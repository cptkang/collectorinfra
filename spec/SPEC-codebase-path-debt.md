...
> **출처**: `plans/70-WIP-codebase-scale-and-path-debt.md`(v4, 2026-08-06 작성) 분석 + 2026-08-20 전수 실측
...
4. 경로·모듈 폐기 제안에 **4항 실측 첨부가 강제**된다(D-161)
...
└── 02_decision.md                  ← [변경] D-161 등재 (D-140 아님 — §0 ①)
...
- 폐기 제안에 **D-161 ② 4항 실측** 첨부 (운영 설정값 · 런타임 가용성 · 브랜치 한정 git log · 역방향 import)
...
### `promotion-rule`
- [ ] `docs/02_decision.md`에 **D-161**으로 등재 (`## D-` 헤더 + 「변경 이력」 표 양쪽)
- [ ] 채번 안내 라인이 "현재 최대 D-142 → 다음 D-143"으로 갱신 *(등재 당시 번호 — 2026-08-24 재부여 후 현행은 "최대 D-162 → 다음 D-163")*
- [ ] `CLAUDE.md` Known Mistakes 원칙에 4항 실측 의무 1~2줄 반영
- [ ] plans/70 본문의 "D-140 예약" 표기가 D-143(현행 **D-161**)으로 정정
...
하나를 회수할지 확인이 필요하다. *(기본: D-143 신규 채번)* → **해소**: D-143으로 등재했으나 2026-08-24 병합 번호 충돌로 **D-161** 재부여.
...
③"예비 코드" 주석 + 폐기 기한. *(기본: ③ — D-161 기한부 존치가 새 규칙의 첫 적용 사례가 된다)*
