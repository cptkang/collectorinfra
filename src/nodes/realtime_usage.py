"""실시간 CPU/메모리 사용률 조회 노드 (Plan 66 — 2단계 하이브리드 데이터 평면).

흐름 (Plan 66 §2, 전부 결정적 — LLM 미사용, D-035):
...
# 존 표기 (선택지 라벨과 동일 입도 — Plan 65 §4.4)
_ZONE_LABELS = {
    "polestar_b0": "은행존",
...
async def realtime_usage_lookup(
    ...
    *,
    ...
    for db_id in db_ids:
        ...
        # ③ 병합 — 미수집·수집 지연·조회 실패 표기 (Plan 65 §1.3-2 파서 주의 ①③)
