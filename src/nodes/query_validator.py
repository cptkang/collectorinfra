...
async def query_validator(
    ...
    *,
    ...
    # 재현·토큰 가변 — 프롬프트로는 못 막는 비결정 오류라 결정적 가드로 재생성을 유도, D-087).
    ...
    # 유도한다(D-087 계열). 알람 뷰의 부모 조인 등 일부 별칭 무필터는 정당하므로
    ...
    # 9. 라우팅 필터 오용 탐지
    routing_errors = _check_routing_filter_misuse(sql)
...
def _find_bare_hangul_tokens(sql: str) -> list[str]:
    """문자열 리터럴·따옴표 식별자·주석을 제거한 뒤 남는 한글 토큰을 찾는다(D-087).
...
def _check_routing_filter_misuse(sql: str) -> list[str]:
