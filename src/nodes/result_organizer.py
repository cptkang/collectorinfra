...
async def result_organizer(
    ...
    *,
    ...
    # 폼필 월 시리즈 결정적 조립(D-113)이 발동해 행이 반환됐으면 부족 판정을 무시한다.
    ...
    # 항목이라 재조회 무의미 — 사유는 D-114 미작성 안내가 노출한다.
    if not is_sufficient and masked_results and state.get("form_month_anchor"):
        logger.info(
            "폼필 월 시리즈 결정적 결과(D-113) %d행 — 부족 판정 무시(재시도 억제)",
            len(masked_results),
        )
        is_sufficient = True
