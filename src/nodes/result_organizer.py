...
async def result_organizer(
    ...
    *,
    ...
    # 폼필 결정적 조립(D-143/D-146)이 발동해 행이 반환됐으면 부족 판정을 무시한다.
    ...
    # 항목이라 재조회 무의미 — 사유는 D-144 미작성 안내가 노출한다.
    # D-146 확장(라이브 실측 2026-07-30 서버목록 양식 3존): 월 시리즈 없는 폼필도
    ...
    if not is_sufficient and masked_results and (
        ...
        logger.info(
            "폼필 결정적 결과(D-143/D-146) %d행 — 부족 판정 무시(재시도 억제)",
            len(masked_results),
        )
        is_sufficient = True
