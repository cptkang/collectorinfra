"""폼필 확인 이력 (Plan 68 Phase 3, D-118) — 양식 시그니처 스코프의 TTL 단기 캐시.

사용자가 역질문 패널에서 확정한 답을 양식 시그니처(헤더 필드 집합 해시) 단위로
Redis에 저장한다. 설계 원칙(plans/68 §2.4 개정):
...
async def load_form_memory_answers(
    ...
    *,
    ...
    if answers and touch:
        ...
        logger.info(
            "폼필 확인 이력 적용(D-118): '%s' %d개 항목 (사용 %d회, TTL %d일 연장)",
...
async def save_form_memory_entries(
    ...
    if await redis.save_form_memory(signature, data, ttl):
        logger.info(
            "폼필 확인 이력 저장(D-118): '%s' %d개 항목 (TTL %d일, 총 %d개)",
...
async def delete_form_memory_entries(
    ...
    *,
    ...
    if field_names is None:
        ...
        logger.info("폼필 확인 이력 전체 삭제(D-118): '%s' %d개 항목", display, len(fields))
    ...
    if fields:
        ...
        if not await redis.delete_form_memory(signature):
            return 0, display
    logger.info("폼필 확인 이력 필드 삭제(D-118): '%s' — %s", display, removed)
