...
# 단위 정규화 대상 숫자 캐스트 타입(D-196). D-160과 달리 여기는 "이미 숫자 캐스트인데
...
def _unwrap_llm_unit_replace(operand: str) -> str:
    """LLM이 만든 ``REPLACE(<식>, '<단위>', '')`` 임기응변을 벗긴다(D-196 후속).
...
def _unit_normalized_expr(operand: str) -> str:
    """단위 접미 문자열을 GB 기준 숫자로 정규화하는 방언 중립 SQL 식을 만든다(D-196).
...
def normalize_eav_unit_casts(sql: str, value_columns: tuple[str, ...] | list[str]) -> str:
    """EAV 값 컬럼의 숫자 캐스트를 단위 인지 정규화 식으로 교체한다 (D-196).
