...
# 절대 월 **범위** 표현(D-164): "1월부터 6월까지" / "2026년 1월~6월" / "1월에서 6월" / "2026-01~2026-06".
...
# 반기 표현(D-164): "(2026년|올해|작년) 상반기/하반기" — 연도 미상은 "미래가 아닌 가장 최근 반기".
...
def _resolve_month_range_expr(text: str, ref: date) -> tuple[str, str] | None:
    """절대 월 범위 표현("1월부터 6월까지"·"2026년 1월~6월"·"2026-01~2026-06")을 해석한다(D-164).
...
def _resolve_half_year_expr(text: str, ref: date) -> tuple[str, str] | None:
    """반기 표현("2026년 상반기"·"작년 하반기"·"상반기")을 (시작, 끝) 월 범위로 해석한다(D-164)."""
...
def resolve_stat_month_range(
    ...
    *,
    ...
    # 절대 월 **범위**·반기 표현이 최우선(D-164) — "2026년 1월부터 6월까지"가 아래 단일 월
...
FORM_MEMORY_NOUN_KEYWORDS = (
    ...
    # D-165: "이 양식에 저장된 내용은?" — '저장된 답/값'만 있어 미탐(라이브 실측 2026-08-25)
    "저장된 내용", "저장한 내용",
)
FORM_MEMORY_VIEW_KEYWORDS = (
    "보여", "조회", "알려",
    # D-165: 의문형("기억하는 내용은 뭐지?") — 명령형만 잡혀 존 역질문으로 흘렀다.
...
# D-165: 채움 동사가 동반되면 이력 조회가 아니라 채움 요청("기억한 값으로 채워줘") — 조회로
# 단락되면 DB 조회가 통째로 이력 응답으로 대체되므로(3중 게이트) 미탐 쪽으로 보수적 판정.
_FORM_MEMORY_FILL_VERBS = ("채워", "채우", "작성", "기입", "반영")
# D-165: 양식 업로드 후 '?'만 입력 — 저장 값 조회 단축키(반각·전각·반복 허용). 정확 일치라 오탐 0.
