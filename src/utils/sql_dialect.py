...
engine 실값은 "db2"/"postgresql"/"mariadb" 정확 문자열이므로 정규화 후 등호 비교로 전 소비처
동작이 동일하다(계획서 v2 실측 — 값 전수는 테스트가 단언). "mariadb"(plans/95)는 DB2가 아니라
행 제한 절이 LIMIT이며 이진 판정 그대로 맞다 — 표현식 방언은 이 모듈 소관이 아니다.
