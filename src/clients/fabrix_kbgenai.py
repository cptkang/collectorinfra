...
class KBGenAIChat(BaseChatModel):
    ...
    system_prompt: str = ""
    timeout: int = 300
    # 호출 1건의 벽시계 총상한(초, D-195). `timeout`은 httpx/requests 의미상 read
    ...
    async def _agenerate(
        ...
        **kwargs: Any,
        ...
        # D-195: 총 소요 상한. httpx는 취소에 즉시 반응(연결 abort)하므로
    ...
    def _stream(
        ...
        **kwargs: Any,
        ...
        # D-195: 동기 스트림도 동일한 벽시계 총상한 (async 쪽 주석 참조)
        ...
        **kwargs: Any,
        ...
        async with httpx.AsyncClient(verify=False) as client:
            async with client.stream(
                ...
                blocked_logged = False
                _content_parts: list[str] = []
                # D-195 진단 계측 — 총상한 발화 시 FabriX 측 상태 판정 근거
                ...
                async for line in response.aiter_lines():
                    _line_count += 1
                    # D-195: 벽시계 총상한. STATUS/SYNC 하트비트가 read 타임아웃을
