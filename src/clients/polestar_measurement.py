"""폴스타 measurement API 클라이언트 (Plan 66 — 실시간 CPU/메모리 사용률).

엔드포인트 (Plan 65 §1 확정 — 2안):
...
핵심 규칙:
    ...
    - resourceIds는 **서버 리소스 ID**(cmm_resource.id). 콤마 구분, 200개/콜 청크
      (Plan 65 §1.3-4 실측: 200대 yd 814ms·gp 2,460ms/37KB — 안전 범위).
    - 청크는 병렬 호출 + **전체 타임아웃 가드**(per-call만으론 무력화 — Known Mistakes).
    - 응답 shape는 Plan 65 §1.3-2 확정본: 최상위 date(호출 시각) + data.measurement[]
...
# 지표 → (definitions, type) 파라미터 (Plan 65 §1.2 — 지표별 1콜)
