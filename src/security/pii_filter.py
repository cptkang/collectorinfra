...
# ── 계좌번호(851) 광폭 매칭 의심 형태 (D-122 후속3) ─────────────────────────────
...
def is_block_dump_enabled() -> bool:
    ...
    설정 로드 실패 시 안전하게 ON으로 간주한다(원인 특정이 우선 — D-122 후속1).
    """
    try:
        from src.config import load_config
...
def scan_account_suspects(
    ...
    """계좌번호(851) 광폭 매칭 **의심** 형태(날짜·타임스탬프)를 스캔한다 (D-122 후속3).
...
def scrub_pii(text: str) -> str:
    ...
    점으로 치환해 서버 계좌번호(851) 광폭 매칭을 회피한다(D-122 후속3 — 값·자릿수
...
def diagnose_blocked_prompt(
    ...
    """차단된 요청의 프롬프트를 **섹션별**로 스캔해 원인 후보를 특정한다 (D-122).
    ...
    for name, text in sections.items():
        ...
        # (D-122 후속3 — "계좌번호 차단인데 계좌 유사 문자열 부재" 실측 대응)
...
def _diagnose_reasons_vs_prompt(
    ...
    (D-122 후속5) "차단 사유(policy_id)"와 "프롬프트의 어떤 값" 사이를 직접 연결하는
    표적 대조. 셋 중 하나로 판정한다:
    ...
    *,
    ...
    Args:
        ...
        prompt_sections: {섹션명: 텍스트}. 주면 전체 스캔 대신 섹션별 진단
            (:func:`diagnose_blocked_prompt`)으로 원인 블록까지 특정한다(D-122).
    ...
    # 원문 응답 전체(D-122 후속1) — 차단 응답은 안내문뿐이라 짧다. 정규화 파서가 모르는
    ...
    # 확인 경로는 FabriX 테넌트 관리 콘솔에서 이 ID로 조회하는 것뿐이다(D-122 후속4).
    ...
    # 전송 원문·응답 전문 파일 덤프(D-122 후속1) — 사유·대조 결과를 함께 기록해
