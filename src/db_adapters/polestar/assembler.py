...
class MonthSeries:
    ...
    # 사용자가 질의에서 명시한 기간(시작, 끝) — 상대(M+k) 양식에서 앵커 산출에 쓴 값(D-164).
...
def recognize_month_series(
    ...
    기준(2~7월)으로 침묵 폴백한 라이브 실측(2026-08-25, D-164)의 대칭 보완.
...
def month_anchor_payload(ms: MonthSeries) -> dict:
    """MonthSeries → state `form_month_anchor` dict(단일·멀티 경로 공용 shape, D-147/D-164).
