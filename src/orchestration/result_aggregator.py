...
async def result_aggregator(
    ...
    *,
    ...
    if len(finalized) == 1:
        ...
        # HITL 폼필(D-148): 역질문 페이로드는 API 응답으로, pending은 체크포인터 state로.
        # 폼필은 D-147 단일 task 고정이라 이 단일 분기로 충분하다.
        ...
        # 존 역질문(D-140 후속2): 단일 task 고정(게이트가 복합 계획 제외)이라 이 분기로 충분.
...
async def _finalize_task(
    ...
    *,
    ...
    # 존 역질문(D-140 후속2): 페이로드를 최종 응답까지 운반(form_fill_clarification 동형).
    ...
    if organized is not None:
        ...
        try:
            ...
            # HITL 폼필(D-148): 역질문 페이로드·대기 상태를 최종 응답까지 운반.
...
def _build_output_state(state: AgentState, task: dict, res: dict) -> dict:
    ...
    return {
        ...
        # 폼필 산출물(D-143/D-148): 파이프라인 res 우선(top-level state는 orchestration
