...
async def replanner(
    ...
    *,
    ...
    if _tasks_now and all(t.get("direct_response") for t in _tasks_now):
        logger.info("replanner: 전 task가 결정적 direct_response — 재계획 스킵(D-147/D-148)")
        return {"needs_replan": False, "replan_history": replan_history, "current_node": "replanner"}

    # 존 역질문(D-140 후속2)은 정의상 이번 턴의 최종 응답(사용자 존 선택 대기) — LLM
    ...
    if any(
        ...
        logger.info("replanner: 존 역질문 대기 — 재계획 스킵(D-140 후속2)")
        return {"needs_replan": False, "replan_history": replan_history, "current_node": "replanner"}
