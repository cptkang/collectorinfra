...
# 2행 병합 헤더 결합 시 그룹 헤더와 서브 헤더를 잇는 구분자 (D-112)
...
def _analyze_sheet(ws: Worksheet) -> dict[str, Any] | None:
    ...
    # 쓰인다. 시트 이름(Sheet1)만으로는 양식 종류를 알 수 없는 실물 양식이 많다(D-113).
    ...
    return {
        ...
        # header_row + 1 계약에 의존하므로 2행 블록에서도 이 계약을 유지한다(D-112).
...
def _detect_header_row(
    ...
    기존 소비처의 `data_start_row = header_row + 1` 계약이 그대로 유지된다(D-112).
...
def _detect_header_block(
    ...
    2) 기준 행의 인접 행이 병합 구조로 결합된 2행 헤더(그룹 헤더 + 서브 헤더)이면
       두 행을 열 단위로 결합해 복합 필드명("그룹|서브")을 생성한다(D-112).
...
def _try_combine_header_rows(
    ...
    보수적 3중 게이트(D-112 — 기존 단일 헤더 템플릿 오결합 방지):
