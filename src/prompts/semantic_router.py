...
# ══════════════════════════════════════════════════════════════════════════
# 3단 계획 필요 신호 (plans/103 §3.2 · plans/111 §5.4 · `TIER3_PLAN_LOOP_ENABLED`)
# ══════════════════════════════════════════════════════════════════════════
#
# 플래그 on일 때만 시스템 프롬프트 **맨 뒤에 덧붙인다** — off면 렌더가 종전과 바이트 동일하다.
# 판정은 LLM 구조화 출력이다(D-004 — 원문 키워드 분류 신설 0). 나눌지 말지의 경계는
# plans/111 §2.2 실측이다 — 과분해: 같은 데이터 안의 필터·안티조인·집계 /
# 미분해: 담당 교대·산출물 둘·앞 결과가 뒤 조건 결정.
SEMANTIC_ROUTER_PLAN_SIGNAL_SECTION = """
## 계획 필요 판단 (needs_plan)

출력 JSON 최상위에 "needs_plan"(true 또는 false)을 함께 적으세요.
- true: 답하려면 **따로 실행해야 하는 조회가 둘 이상**일 때입니다 — 앞 조회의 결과가 뒤 조회의 대상이나 조건을 정하거나(예: 알람이 있는 서버를 먼저 찾고 그 서버들의 성능 지표를 조회), 담당 조회가 서로 다르거나(알람 · 성능 지표 · 실시간 프로세스), 결과물이 둘일 때(예: 전체를 파일로 주고 그중 일부를 표로 따로)입니다.
- false: 한 번의 조회로 답할 수 있으면 false입니다. 같은 데이터 안의 조건 필터 · 제외 조건 · 집계 · 정렬 · 상위 N · 비교는 한 번의 조회입니다.
- intent가 data_query 또는 alarm_query가 아니면 false입니다.
"""  # noqa: E501
