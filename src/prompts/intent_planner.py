...
3. **process_query**: **현재/실시간** 프로세스 조회이면 → `process_query`
   - **시간성으로 구분**: "**현재** 프로세스", "**실시간** 프로세스", "지금 실행 중인 프로세스", "top 프로세스",
     "프로세스 리스트 확인" → `process_query` (실시간 API).
   - "프로세스 **이력**", "프로세스 **추세**", "지난 N일 프로세스", 과거 시점 프로세스 → `data_query` (DB 이력).
   - 애매하면(시간성 신호 없는 단순 "프로세스 리스트") 사용자가 모니터링 중인 현재 상태를 원하는 것으로 보아 `process_query`로 분류.
