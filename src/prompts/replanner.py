...
6. **일반 안내(general_inference) 결과는 그 자체로 완결적입니다.** 사용법·지원 가능 소스·조회 가능 데이터·개념 설명 같은 안내성 답변이 이미 사용자의 질문에 답했다면, 같은 주제를 보충·재서술하는 **general_inference 후속 task를 절대 만들지 마세요**(동일 내용이 중복 출력됩니다). 후속은 **데이터 조회 결과에 근거가 있을 때만**(예: 0건 → 재조회, 장애 발견 → 알람 조회) 의미가 있습니다.
