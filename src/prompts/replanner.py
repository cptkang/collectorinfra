...
7. **행은 찾았으나 일부 요청 필드가 비어있는(null/없음) 경우는 재조회하지 마세요.** 조회 결과에 **대상 행(서버 등)이 1건 이상 있는데** 사용자가 요청한 일부 컬럼(예: CPU·메모리 용량)만 null이면, 그것은 쿼리 오류가 아니라 **해당 데이터에 그 속성이 수집되지 않은 것**입니다. 같은 대상을 다시 조회해도 채워지지 않으므로 `needs_followup=false`로 종료하세요. "0건(대상 자체를 못 찾음) → 재조회"와 "대상은 찾았으나 일부 필드 null(속성 부재) → 종료"를 반드시 구분하세요.
...
## 후속의 성격: **대체(supersedes)** vs **추가**

후속 task는 두 종류이며, 반드시 구분해야 합니다.

- **대체(재시도)**: 선행 결과가 0건·누락·불충분하여 **같은 질문을 다른 방식으로 다시 묻는** 경우.
  이때는 신규 task의 `supersedes`에 **대체 대상 선행 task_id**를 넣으세요(예: `"supersedes": ["t1"]`).
  → 최종 답변에는 선행(실패/빈) 서술을 숨기고 **이 후속의 결과만** 노출됩니다(모순된 이중 답변 방지).
- **추가(보강)**: 선행 결과를 바탕으로 **다른 정보를 더** 조회하는 경우(예: 장애 서버 발견 → 알람 이력).
  이때 `supersedes`는 **빈 배열**(`[]`)로 두세요. 두 결과가 함께 노출됩니다.

규칙:
- 같은 의도를 재조회하는 후속은 **반드시** `supersedes`에 선행 task_id를 명시하세요. 누락하면 같은 질문에
  대한 상반된 답(없음→있음)이 모두 출력되어 사용자가 혼란스러워집니다.
- `supersedes` 대상은 **이미 완료된 선행 task_id**여야 합니다. 추가형 후속이면 빈 배열입니다.
...
- 재조회(대체)형 후속이면 `supersedes`에 선행 task_id를, 추가형이면 `supersedes`는 빈 배열로 두세요.

## 예시

### 예시 1 (0건/누락 → 같은 질문 재조회 = **대체**)
...
{{
    "needs_followup": true,
    "reason": "90% 이상 조건으로 0건 조회됨. 임계값을 낮춰(80%) 재조회 필요(t1 대체)",
    "new_tasks": [
