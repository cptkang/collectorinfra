...
`## D-161`으로 등재한다. 원 계획이 D-140을 예약했으나 소진돼 재채번한 건이다.

**Acceptance:**
- [x] `## D-161` 헤더 1건 + 「변경 이력」 표 1행
- [x] ②-3항에 **브랜치 한정 조회 의무** 포함(`git branch -a --contains` / `merge-base --is-ancestor`)
- [x] 채번 안내 라인이 "현재 최대 D-143 → 다음 D-144"로 갱신
      *(이후 D-144 등재로 "최대 D-144 → 다음 D-145"로 진행 — 정상 갱신. **2026-08-24 병합 충돌로 D-143→D-161·D-144→D-162 재부여** → 현행 "최대 D-162 → 다음 D-163")*
- [x] D-140~142가 plans/70 예약분이 **아니었음**을 안내 라인에 부기(재발 방지)

**Verify:**
- [x] `grep -c "^## D-161" docs/02_decision.md` = 1
...
## ✅ Checkpoint R
- [x] D-161 등재 확인, 이후 모든 폐기 제안이 이 규칙 아래 실행됨
...
**Files:** `.venv/`(정리), `plans/70-WIP-codebase-scale-and-path-debt.md`
**Scope:** XS
...
**Acceptance:**
- [x] `plans/INDEX.md`에 **75건 전건** 등재 (번호·제목·상태·최종 수정일)
- [x] 기존 `plans/README.md`는 변경하지 않음(성격이 다름 — 초기 구현 목차)
- [x] 상태는 파일 내 표기에서 기계적으로 추출(추정 금지)

**Verify:**
- [x] `ls plans/*.md | wc -l`과 인덱스 행 수 일치
- [x] 상호 참조 링크 유효성 검사
...
### V2: 골드셋 확충 — ⚠️ **부분 완료(DB 미기동으로 차단)**

**Description:** 현 26건은 커버리지 76.9%의 표본으로 얇다. 특히 `unhandled` 1건은
거부 경로 설계 근거로 부족하다.

**Acceptance:**
- [x] 카테고리별 최소 표본 목표를 **정의** — `testdata/text2sql_gold/README.md` 「표본 목표」
      (목표 32건 / 현재 26건 / 부족 +6)
- [ ] 목표 **충족** — ⛔ 차단: DBHUB(localhost:9099) 미기동으로 `--check-gold` 불가
- [ ] `unhandled`·`outside` 보강 — ⛔ 차단(위와 동일 사유)
- [x] **기존 26건 불변** — 항목 추가 0

**Verify:**
- [x] `--dry-run` 스키마 위반 0건 유지
- [x] 카테고리 분포 확인 — 26건 불변(20 inside / 6 outside, 76.9%)

**Files:** `testdata/text2sql_gold/{b0,gp,yd}.yaml`
**Scope:** S
**주의:** 골드 SQL은 SELECT 전용. `--check-gold`는 실 DB 접속 필요 → 별도 판단

**차단 사유(실측 2026-08-20):** `curl --max-time 5 http://localhost:9099/sse` → HTTP 000(미기동).
골드 SQL은 채점의 **정답**이라 미검증 항목을 넣으면 그 오답이 이후 모든 평가의 기준이 된다.
`--dry-run`은 스키마만 검사하므로 미검증 항목도 통과시켜 **카테고리 분포와 커버리지율만
좋아 보이게** 만든다 — 측정이 나빠지면서 지표는 좋아지는 최악의 조합. 기존 항목들도
`notes` 기록상 전부 `--check-gold` 실측(최대 15차)으로 확정됐다.
**해제 조건:** DBHUB 기동 후 README 「착수 조건」의 3단계 절차 수행.
...
- [x] 구 이름 `AliasChoices` 하위호환 유지 (제거 아님 — 폐기 기한만 D-161에 부여)
