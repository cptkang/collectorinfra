...
**Acceptance:**
- [ ] `docs/flag_audit.md`에 **43행** 누락 0
- [ ] 열 = 플래그 · 생성 D-번호 · 코드 기본값 · **`.env` 실제값** · 프로덕션 참조 수 · 최종 변경일(브랜치 한정) · 판정
- [ ] 판정은 존치/상수화/삭제/기한부 중 하나로 **전 행 확정**
- [ ] 참조 0건·1~2건 플래그가 별도 표시

**Verify:**
- [ ] 계획서 §4 재현 명령 결과와 표의 참조 수가 일치
- [ ] `.env` 실제값 칸이 전부 채워짐
...
**Acceptance:**
- [ ] 안내 라인은 "채번 규칙 + 현재 최댓값"만 남김
- [ ] 예약/결번/재부여 이력은 표로 분리 (정보 손실 0)
- [ ] 기존 정보가 전부 표로 이관됐음을 확인

**Verify:**
- [ ] 이관 전후 예약 번호 집합이 동일 (`grep -oE "D-[0-9]{3}"` 비교)
