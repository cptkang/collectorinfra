...
- 5경로 구동(§7 경로 커버리지):
  `--path {graph,orchestration,multidb,deep_agent,semantic_router}`
  `semantic_router`는 사다리 3단(기준 경로 · D-225) · `deep_agent`는 사다리 1단(부가 경로 opt-in) —
  둘 다 확정 단이 그 단이 아니면 전 항목 graceful 스킵
...
# semantic_router = 사다리 3단(기준 경로 · D-225 ①)
# deep_agent = 사다리 1단(부가 경로 opt-in · D-225 ②)
# 둘 다 graph와 같은 그래프를 돌리되, 확정 단이 실제로 경로 이름과 같은 단인지 검사한다
# (plans/70 V1) — 아니면 "그 단을 쟀다"는 말이 성립하지 않으므로 조용히 통과시키지 않고 스킵한다.
# 경로 이름이 곧 사다리 단 값(`LadderTier.value`)이다.
PATHS = ("graph", "orchestration", "multidb", "deep_agent", "semantic_router")
_LADDER_CHECKED_PATHS = ("deep_agent", "semantic_router")
    ...
    def predict(self, item: GoldItem, flags: dict) -> PredictionResult:
        ...
        try:
            ...
            async def _run() -> dict:
                ...
                if self.path not in ("graph", *_LADDER_CHECKED_PATHS):
                ...
                if self.path in _LADDER_CHECKED_PATHS:
                    # build_graph가 사다리 단을 확정·기록한다(O1/O2). 요청한 단이 아니면 이 실행은
                    # 다른 단을 잰 것이므로 결과를 그 단의 수치로 보고해선 안 된다.
                    from src.observability.ladder import current_ladder  # type: ignore

                    snap = current_ladder() or {}
                    if snap.get("tier") != self.path:
                        raise _LadderTierMismatchError(
                            f"사다리 {self.path} 미확정 — tier={snap.get('tier') or '미확정'} "
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    parser.add_argument("--path", choices=PATHS, default="orchestration",
                        help="실행 경로 (semantic_router=사다리 3단 기준 경로 확정 검사 · "
                             "deep_agent=사다리 1단 부가 경로 opt-in 확정 검사 · "
                             "기본 orchestration=단일 DB 파이프라인 직접 구동, 사다리 단 무관)")
    ...
    if args.ab:
        ...
        if args.ab == "candidate_count":
            ...
            # env/.env에 켜져 있는 값(예: 로컬 TEXT2SQL_SEMANTIC_COMPOSE=true)이 그대로
            # 남아 baseline도 variant와 동일 조건으로 실행된다(2026-07-15/07-20 실측).
