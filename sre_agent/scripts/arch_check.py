...
MODULE_LAYER_MAP: dict[str, Layer] = {
    # SREAgent 모듈 매핑 (src/sre_agent/ 레이아웃)
    # 파일 경로 기준(src.sre_agent.*)과 임포트 경로 기준(sre_agent.*)을 모두 등록해야
    # from/to 양쪽 계층 판정이 누락되지 않는다.
    "src.sre_agent":                  "application",
    "src.sre_agent.settings":         "config",
    "src.sre_agent.toolset_profiles": "config",
    "src.sre_agent.diagnosis":        "application",
...
# 내부 모듈로 취급할 임포트 prefix — 새 최상위 패키지 추가 시 여기에 등록한다
INTERNAL_PREFIXES = ("src.", "sre_agent.")


def extract_imports(file_path: Path) -> list[ImportInfo]:
    """파일에서 내부 모듈 import 문을 추출한다."""
    try:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            ...
            if node.module and node.module.startswith(INTERNAL_PREFIXES):
                imports.append(ImportInfo(
                    module=node.module,
                    line=node.lineno,
...
def check_project(project_root: Path) -> CheckResult:
    ...
    # 이 레이아웃은 src/ 없이 최상위 sre_agent/ 아래 파이썬 패키지가 위치한다.
    src_dir = project_root / "sre_agent"
    result = CheckResult()
...
def print_text_report(result: CheckResult, verbose: bool = False) -> None:
    ...
    if verbose:
        ...
        src_dir = Path(project_root) / "sre_agent"
        for py_file in sorted(src_dir.rglob("*.py")):
