...
async def test_t1_schema_endpoint_returns_catalog(monkeypatch, tmp_path):
    ...
    assert len(items) == 349  # plans/107 intent_frame 설정 +5 · plans/111 COMPOSITE_TASK_FRAME_ENABLED +1
...
def test_t2_group_and_field_counts():
    ...
    → TEXT2SQL_ALARM_DETERMINISTIC(활성 알람 결정적 조립 옵트인 · 커밋 1454898)으로 **313**.
      당시 이 단언 갱신이 누락돼 기준선이 빨간 상태였다 — D-198 작업에서 소급 반영.
    → LLM_FABRIX_TOTAL_TIMEOUT(D-198 FabriX 호출 총 소요 상한 · 기본 300s)으로 **314**.
      그룹 수 불변(llm 기존 그룹).
    → 2026-09-09 **322**: COMPOSITE_{SEQUENTIAL_GATE,SCOPE_POSTCHECK,PRIOR_SCOPE_BY_DB}_ENABLED
    ...
    → 2026-09-21 plans/107 신규 그룹 `intent_frame`(`INTENT_FRAME_ENABLED`·`CANONICAL_QUERY_{MODE,
      CONSUMERS}`·`REWRITE_{GATE,VERIFY}_MODE`) +5 = **348** · 그룹 **25** · general 불변(19).
      작업 직전 343/24 실측 후 가산.
    ...
    assert len(index) == 349  # plans/111 COMPOSITE_TASK_FRAME_ENABLED +1
