...
class TestRecognizeMonthSeries:
    ...
    def test_month_range_query_sets_anchor_end(self):
        """라이브 실측(2026-08-25, D-185): "1월부터 6월까지" → M=1월, M+5=6월(지난달 폴백 아님)."""
        ms = recognize_month_series(
            _cpu_form_mapping(),
            context_text="CPU 사용현황",
    ...
    def test_llm_time_range_fallback_reaches_anchor(self):
        ...
        피벗 stat_month에만 있던 D-136 2단 폴백의 앵커 대칭(D-185)."""
        ms = recognize_month_series(
            _cpu_form_mapping(),
            context_text="CPU 사용현황",
    ...
    def test_anchor_payload_shape_is_shared(self):
        """단일·멀티 경로가 공용 payload를 쓴다 — requested/source 포함(D-185)."""
...
class TestFormFillNotes:
    ...
    def test_note_states_query_basis_when_requested(self):
        """질의 기간으로 앵커를 잡았으면 안내문에 그 근거를 명시한다(D-185)."""
        from src.nodes.output_generator import _append_form_fill_notes
    ...
    def test_note_flags_mismatch_between_request_and_filled_months(self):
        """요청 개월 수 ≠ 양식 칸 수(1~3월 요청, 6칸)면 침묵하지 않고 불일치를 명시한다(D-185)."""
        from src.nodes.output_generator import _append_form_fill_notes
...
class TestResponseTableMarkdownSafety:
    ...
    def test_reference_info_block_injected(self):
        """D-186(라이브 실측 2026-08-25): 프롬프트에 연도가 없어("1월~6월"+M/M+1 칼럼) LLM이
    ...
    def test_year_guard_flags_summary_only(self):
        """D-186 사후 가드: 요약 문단의 기준 밖 연도만 경고, 표 본문의 도입일자 연도는 오탐 아님.
    ...
    def test_normalize_markdown_tables_fixes_separator_width(self):
        """D-187(라이브 실측 2026-08-26): GFM은 구분선 셀 수 ≠ 헤더 셀 수면 표로 인식하지 않아
...
class TestFormFillHitl:
    ...
    def test_hitl_pending_db_ids_survive_orchestration_output_state(self):
        """D-186(라이브 실측 2026-08-25): 존 체크박스 런은 항상 오케스트레이션을 타는데
...
class TestFormMemoryCommands:
    ...
    def test_memory_gate_question_forms_and_shortcut(self):
        """D-186(라이브 실측 2026-08-25): 의문형·'저장된 내용'·'?' 단축키는 이력 조회로,
    ...
    def test_memory_panel_payload_and_transport(self):
        """D-187: 조회 응답에 삭제 패널 컨텍스트가 동봉되고 subagents→result_aggregator를
    ...
    async def test_memory_delete_route_helper(self, monkeypatch):
        """D-187: 패널 삭제 요청은 파이프라인·LLM 없이 결정적으로 처리되고, signature가 세션의
