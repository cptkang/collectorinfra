"""실시간 사용률 데이터 평면 테스트 (Plan 66 — 게이트·클라이언트·노드 병합).

전부 mock — 네트워크·DB 미사용. 응답 shape는 Plan 65 §1.3-2 확정본을 그대로 고정한다.
"""
...
# Plan 65 §1.3-2 확정 응답 shape (사용자 교정본 그대로)
_FIXED_PAYLOAD = {
