...
class TestPathDelegation:
    ...
    def test_multi_path_delegates(self):
        from src.nodes.multi_db_executor import _extract_sql

        content = "```SQL\nSELECT 1 FROM t\n```"
        assert _extract_sql(content) == "SELECT 1 FROM t"

    def test_single_path_delegates(self):
        from src.nodes.query_generator import _extract_sql_from_response

        content = "```SQL\nSELECT 1 FROM t\n```"
        assert _extract_sql_from_response(content) == "SELECT 1 FROM t"
