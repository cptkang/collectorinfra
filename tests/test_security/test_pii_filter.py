...
class TestLogFilterBlockDetail:
    ...
    def test_full_raw_response_in_log(self, caplog, no_dump):
        """정규화 파서가 모르는 키도 원문 응답 JSON 전문으로 로그에 노출된다(D-122 후속1)."""
        with caplog.at_level(logging.WARNING):
            log_filter_block_if_any(
                result={
...
class TestPolicyTargetedDiagnosis:
    """서버 사유(policy_id)의 룰을 프롬프트에 역적용하는 표적 대조 (D-122 후속5).
...
class TestRevisedRules2026:
    ...
    # 인프라 데이터에서 계좌번호(851) 룰에 걸리는 실형태 카탈로그(D-122 후속3 —
...
class TestAccountSuspects:
    """계좌번호(851) 광폭 매칭 의심 형태 — 날짜·타임스탬프 (D-122 후속3)."""
...
class TestResponseShapeParsing:
    """가이드 실측 응답 shape — snake_case·FR-200/400·APIM (D-122 후속2)."""
