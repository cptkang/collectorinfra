...
class TestSev3RepeatInterval:
    def test_sev3_uses_shorter_interval(self):
        ...
        fp = "fp-sev3"
        assert w._is_duplicate_fingerprint(fp, now=1000.0, severity=3) is False  # 최초
        assert w._is_duplicate_fingerprint(fp, now=1030.0, severity=3) is True   # 30s<60 중복
        # 61s 후 → sev3 TTL(60) 만료 → 재통보 허용(중복 아님)
        assert w._is_duplicate_fingerprint(fp, now=1061.0, severity=3) is False

    def test_non_sev3_uses_common_interval(self):
        # 동일 시점(61s 후)이라도 sev2는 공통 14400s TTL → 여전히 중복
        w = _worker(repeat=14400, sev3=60)
        fp = "fp-sev2"
        assert w._is_duplicate_fingerprint(fp, now=1000.0, severity=2) is False
        assert w._is_duplicate_fingerprint(fp, now=1061.0, severity=2) is True

    def test_sev3_within_interval_still_duplicate(self):
        w = _worker(repeat=14400, sev3=60)
        fp = "fp-sev3-dup"
        assert w._is_duplicate_fingerprint(fp, now=1000.0, severity=3) is False
        # 59s 후 → sev3 TTL(60) 이내 → 중복
        assert w._is_duplicate_fingerprint(fp, now=1059.0, severity=3) is True
    ...
    def test_default_equal_intervals_no_change_regression(self):
        ...
        assert w._is_duplicate_fingerprint(fp3, now=1000.0, severity=3) is False
        assert w._is_duplicate_fingerprint(fp3, now=1000.0 + 100, severity=3) is True
        assert w._is_duplicate_fingerprint(fp2, now=1000.0, severity=2) is False
        assert w._is_duplicate_fingerprint(fp2, now=1000.0 + 100, severity=2) is True
