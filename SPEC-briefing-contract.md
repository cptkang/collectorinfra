...
> 모듈 id: **`briefing-contract`** (`CAPABILITY-MAP-50.md`) · 착수 결정: **D-194 예정**(미등재 — §Open).
...
7. `plans/50` §0.3 G6 항목이 **해소**로 갱신되고 D-194가 등재된다.

## Open Questions — 구현 시 확정(2026-09-02)

O-1 기본안 채택(`심각(신뢰도 high) — 게이트 PAGE · 조사 상향 · 시그니처 oom_kill`) · O-2 `[근거]` 폐기 확정 · O-3 D-194는 6모듈 완료 시점에 한 번에 등재(모듈 1 랜딩과 같은 세션이라 근거 분산 없음).
...
| **O-3** | D-194를 이 모듈 랜딩 시 등재할지, 6모듈 완료 후 한 번에 할지 | **이 모듈 랜딩 시**. 계약 정본 지정은 이 모듈에서 확정되는 결정이라 뒤로 미루면 근거가 흩어진다 |
