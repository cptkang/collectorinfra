...
| `composite-task-progress` | 복합 질의 단계 이벤트 — `emit_task_progress` 공통 헬퍼를 2단 레벨 루프·1단 도구 러너가 호출 · deep_agent 재개/합성 마일스톤 | `sse-progress-contract`(custom event를 `progress{kind:"task"}`로 나른다) |
