...
> **상태**: **구현 완료(위임 방식) — 2026-09-09 재판정(`plans/50` §0.8 · D-207)**. collectorinfra 측 CW-A(push `noise_gate/application/nodes/investigation_trigger.py`)·CW-B(pull `src/nodes/fault_diagnosis.py`)·CW-C(`fault_escalation_enabled` 승격) 완료(D-124 · 3-E 후속 모드 D-137), §4.8 L1 메시지 보강 완료(D-108 `message_enrichment_enabled`), §8 권고 생성 완료(`sre_agent` D-138 · human-gated · 실행 경로 부재 테스트 고정). **코드 잔여 0** — 남은 관할은 §8.3 **B-3 자동 조치 거버넌스**(범위 외 · 착수 금지 블로커)뿐이며, §4.8 L3 부분은 Plan 60 §18 E8(=Plan 66 R11)로 이관됐다. **`-WIP` 태그 해제(2026-09-09 사용자 확정 U-G — 파일명 `64-automated-incident-investigation-and-response.md`)** · B-3 거버넌스 관할은 유지. *종전 표기(이력)*: 계획 (미구현). 사용자 확정(2026-07-21): ①산출물=Plan 60 훅 + 신규 Plan 64, ②조사 범위=**L3 실호스트 명령까지 즉시 대상**(→ §7 보안결정 선행), ③조치=**권고만·운영자 승인**(D-003 유지). **통합 재편(2026-07-24 · D-118)**: 조사 실행 본체(ReAct 조사 루프·severity_judge·브리핑 조립·조치 권고 생성)는 **`sre_agent/` 독립 패키지(HolmesGPT — `plans/sre-agent/02`)가 담당**하며, 본 계획의 `investigation_graph` 자체 구현(§3)은 **대체됨**. 본 계획의 잔존 유효 범위 = 요구·조사 절차 사양(§1·§4)·거버넌스(§8)·**collectorinfra 측 소비 배선** — 섹션별 상태와 재편 Wave는 **§0** 참조.
...
> **2026-09-09 실측(`plans/50` §0.8.6 ①)**: CW-A·CW-B·CW-C **전부 구현 완료** — D-124(3-A/3-B) · D-137(3-E 즉시통보+후속 브리핑). 아래 표는 착수 근거로 보존한다. 계획군 잔여의 단일 소유는 `plans/50` §0.8.2가 정본이다.
...
| 날짜 | 변경 | 사유 |
|---|---|---|
| 2026-09-09 | **헤더 상태 재판정 — 구현 완료(위임) · 코드 잔여 0 · B-3 거버넌스만 관할** | 사용자 지시(*"50, 64, 66번 등 유사한 계획을 종합 분석하여 정리"*) → `plans/50` §0.8(D-207) 실측: CW-A~C(D-124·D-137)·§4.8 L1(D-108)·§8 권고(D-138) 전부 랜딩. 헤더 "계획 (미구현)"은 2026-07-21 시점 표기가 남은 것. §0.2에 실측 주석. E8(§4.8 L3)은 60 §18/66 R11 소유, 자동 조치 B-3은 착수 금지 유지. `-WIP` 해제는 U-G — **같은 날 사용자 확정·리네임 완료**. 함께 확정된 U-H로 §4.8 L3의 게이트 동기 probe는 폐기되고 post-gate 비차단 보강만 남는다(60 §18.3). |
