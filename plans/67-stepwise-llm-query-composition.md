...
> **상태**: **완료(v19, 2026-09-10) — E1 A/B 재측정 실행(D-127 건별 승인·동일 커밋 09183c8·caffeinate·슬립 0)으로 판정 확정: stepwise 기본 OFF 확정(EX 11/15 vs 9/15, 승 0·패 2, 지연 2배). 계획 잔여 0** — 후속 별건 3건(b0 EAV 예제 실측·hi 조인 키 즉시 적용·taxonomy 프롬프트 노출)은 계획 범위 밖 옵트인/외부 접속 대기로 남긴다. (v17 이전 서술) 전 트랙 구현 + E1 A/B 평가 실행(2026-08-05, v16) — 단, 측정 조건 불일치로 판정은 잠정·재측정 대기(v17) — E1 판정: 공통 채점 10건 EX **동률 7/10**(stepwise 승 1·패 1) → **stepwise 기본 OFF 유지**(ON 전환 근거 없음, 승 패턴인 기간 필터 성능 통계형은 S-IR 확장의 실효 근거로 보존). ⚠ 두 팔이 다른 커밋(`b04c5dd` 전/후)·다른 시간대에서 실행됐고 baseline 구간의 90%가 시스템 슬립이라 **지연 비교는 무효**, EX 결론도 재측정 전까지 확정 인용 금지(v17·D-128). 부수 성과: 실 Gemini(thinking 계열) content 블록 리스트로 **라이브 경로 전멸이던 잠복 결함을 공용 유틸+직접 소비처 전수 교정**(bd29707·b04c5dd). 잔여 후속: v15 후속 등재 목록(gp 픽스처 골드 보강은 2026-08-05 완료 — `09_plan67_e1_gold_missing_servers.sql`, check-gold 15/15 OK).
> **개정 이력**:
...
> - v19(2026-09-10) — **E1 A/B 재측정 실행(D-127 건별 승인) → 판정 확정·계획 완료(`-WIP` 해제)**: ①**조건 충족 실증** — 두 팔 모두 커밋 `09183c8` 격리 worktree, `caffeinate -i` 하 연속 실행(baseline 14:00:03~14:03:20 3m17s → stepwise 14:03:20~14:09:27 6m07s), `pmset -g log` 슬립 진입 0건, 모델 gemini-3.5-flash-lite, 설명 자동생성 OFF, `--path orchestration --db gp` 15문항, 팔 전환은 OS env `TEXT2SQL_STEPWISE_DERIVATION`만. ②**결과** — EX baseline **11/15(73.3%)** vs stepwise **9/15(60.0%)**, subset 11 vs 10, 평균 지연 **12.7s vs 24.1s**(슬립 0이라 이번엔 유효), 토큰 3,048 vs 3,774. 카테고리: server_config 6/7→5/7 · unhandled 1/1→0/1 · complex 3/3 · performance 1/2 · alarm 0/2 동일. 차이 2건 **전부 stepwise 패** — gp-002(요청에 없는 `name` 컬럼 추가, 컬럼 5 vs 골드 4) · gp-014(유사 사양 서버 3행 vs 골드 2행 과포함). v16의 승 gp-009는 이번엔 양쪽 PASS(동률). ③**오염 1건(판정 불변)** — stepwise 팔 gp-015에서 Gemini 429 RESOURCE_EXHAUSTED → 루프 실패·1방 폴백(사유 로그 정상). gp-015는 양 팔 FAIL이라 판정에 영향 없음. 그 외 429 0건, 두 팔의 `[저장]`·감사 로그는 v17 교정대로 stderr 분리(JSON 순수). ④**판정: stepwise 기본 OFF 확정**(v16·v17 잠정 → 확정, D-128 갱신). 승 패턴이 사라지고 패 2건이 '과포함'(컬럼·행) 계열 — S-IR 확장이 아니라 루프의 마감 규율 문제라 ON 전환 근거 없음. ⑤**부수 관찰(후속 별건, 미착수)** — (a) `[단계적도출] 미등록 도구 호출: search_value_index`: `SYNONYM_VALUE_RETRIEVAL` OFF라 바인딩되지 않은 도구를 LLM이 호출(프롬프트 도구 목록과 바인딩 집합 정합 점검). (b) gp-010 stepwise SQL이 `relation "cmm_resource" does not exist`로 실행 실패(스키마 한정 누락 — 양 팔 FAIL 항목). 산출물 `eval_results/e1_20260910/`(run_meta.txt에 커밋·시각). 격리 worktree는 제거.
> - v18(2026-09-10) — **v15 후속 등재 목록·사용자 측 2건 실측 정리 → 잔여 = E1 재측정 1건**: ①`TEXT2SQL_PROMPT_KNOWLEDGE_RENDER`·`QUERY_INTENT_LLM_ASSIST`의 RELOADABLE_KEYS 판정 **등재**(소비처 실측 — 전자는 `knowledge_render_enabled()`가 요청 시 `load_config()`를 읽고 `/admin/settings/reload`가 `cache_clear()`를 호출, 렌더 캐시는 플래그별 키; 후자는 `synonym_registrar` partial(그래프 재빌드)+`routes/query.py`의 `app.state.config` 판독. 런타임 실증: cache_clear 전 불변·후 반전. 카탈로그 테스트 47건 그린). ②severity_map 정본 상위집합 확장은 **이미 완료** 확인(`config/knowledge/_base/catalog.yaml` 해소/resolved 포함 8항 = semantic_models 4파일 동일). ③agg 표기 field_validator 흡수 **완료** 확인(`src/semantic/ir.py` `_normalize_agg`). ④`QUERY_INTENT_LLM_ASSIST` ON 전환은 운영 `.env`에 **이미 true**(인터뷰 대기 항목 해소). ⑤plans/69 충돌 확인 — 69는 v9로 전량 완결(잔여 0), 충돌 없음. ⑥**사용자 측 2건 실측**: Redis 기동 확인(`collectorinfra-redis` 6380 = `.env` REDIS_PORT) — 질의 이력은 b0 12·gp 26·yd 17건 기적재, 활성 DB `polestar`만 0건이라 프로필 예제 12건 **적재 완료**(`query_history_seed.py load --db polestar`, 병합 모드). DB 적재는 `eval_text2sql.py --check-gold --db gp` **15/15 OK**(결함 후보 0 — Phase 0-2 판정 충족, 골드 0행 위양성 없음). ⑦**E1 재측정 런북 확정(승인 대기)**: 동일 커밋(현 HEAD) 격리 worktree · `caffeinate -i` 하 두 팔 연속 실행 · 팔 전환은 OS env `TEXT2SQL_STEPWISE_DERIVATION` 만 · `--path orchestration --db gp --json --sql-dump` · 설명 자동생성 OFF(`SCHEMA_CACHE_AUTO_GENERATE_DESCRIPTIONS=false`) · 산출물 `eval_results/e1_<date>/` · 실행 전 `LLM_GEMINI_MODEL` 현행값(gemini-3.5-flash-lite) 유효 확인. **D-127 건별 승인 전 미실행**. 미이관 후속(별건 유지): b0 EAV 예제 실측(`polestar_b0.yaml:456`, DB2 접속 필요)·hi 조인 키 즉시 적용(프로필 few-shot 동반 교정+b0 실측 전제)·taxonomy LLM 프롬프트 노출(옵트인 별건).
> - v17(2026-08-05) — **v16 E1 A/B의 측정 조건 불일치 사후 확인 → "완료·확정"을 잠정으로 강등(D-128 재갱신)**: 두 팔이 **동일 조건이 아니었음**을 사후 실측. ①**다른 커밋** — stepwise 08-04 18:00:34~18:11:10(`b04c5dd` **이전**), baseline 21:25:45~23:55:16+(`b04c5dd` 커밋 21:24:28의 77초 뒤 기동). `b04c5dd`가 고친 `result_organizer`·`schema_analyzer`·`semantic_compiler`·`description_generator` 4파일은 **두 팔 공유 경로**라 stepwise 수치는 결함 코드 산출(과소평가 가능). ②**baseline 구간 90%가 시스템 슬립** — 벽시계 2:29:31 중 2:14:09(배터리 Maintenance Sleep, `pmset -g log`가 로그 공백과 초 단위 일치: `21:26:10 user_request` = `21:26:10 Entering Sleep … 1028 secs`), 실가동 ≈15분. `avg_latency_ms`는 macOS `perf_counter`가 슬립 중 정지해 슬립 미포함(측정 16분 ≈ 실가동 15분 정합)이나 슬립 단절의 재시도·재접속 영향은 배제 불가 → **평균 지연 64.6s vs 38.2s 비교 무효화**. EX 결론(기본 OFF)은 방향 불변으로 유지하되 **잠정**, 동일 커밋·`caffeinate -i` 연속 실행 재측정 전까지 확정 근거 인용 금지. ③**산출물 오염 근본수정** — `e1_*.json`이 유효 JSON이 아니었음(감사 structlog 30/29줄 + `[저장]` 알림 1줄이 stdout 혼입). `scripts/eval_text2sql.py`에서 감사 structlog·진행 알림을 **stderr로 분리**, 기존 산출물은 무손실 정규화(`e1_*_audit.log` 분리·지표 불변).
> - v16(2026-08-05) — **E1 A/B 평가 실행(트랙 S 성적표 — v17에서 잠정으로 강등)**: 로컬 gp 샌드박스 15문항, 격리 worktree(7d4237e+파서 수정 2커밋), 모델 gemini-3.5-flash-lite(구 2.5-flash-lite 서버 퇴역 404로 인터뷰 교체), 설명 자동생성 OFF. ~~동일 조건~~ → **조건 불일치(v17 참조)**. **결과**: 공통 채점 10건 EX **7/10 동률** — stepwise 승 gp-009(지난달 기간 필터 성능 통계 — S2 라이브에서 확인한 pattern-B 시간 필터 갭의 개선 실증), 패 gp-002(서버 구성 나열형). 전체 EX baseline 70.0%(7/10) vs stepwise 63.6%(7/11), subset 70.0% vs 72.7%. ~~평균 지연 64.6s vs 38.2s~~(v17에서 무효 처리). gp-014는 stepwise만 검증 소진 실패(HAVING 서버 식별 컬럼 가드), gp-015 baseline은 일시 ReadError로 우열 판정 불가. **SMQ 정확도(1/6 대비)·커버리지(34.6% 대비) 축은 orchestration 경로 하네스에서 smq_scored=0으로 미산출** — EX 축만으로 판정. **판정: stepwise 기본 OFF 유지**(EX 우위 없음 — D-128 "활성화는 E1 후" 조건 미충족). 미채점: gp-007/008/011(+baseline gp-014) 골드 0행(픽스처 리터럴 서버명 부재). **측정 중 실사고 2건**(known_mistakes 등재): ①`.env` 퇴역 모델(D-021 금지 계열 잔존)로 2.5h 실패 공회전 ②content 블록 리스트 — S2의 지점별 정규화가 비대칭이어서 공용 유틸 진입부(bd29707)+직접 소비처 4곳(b04c5dd, semantic_compiler `parse_smq_response` 사전 strip이 baseline 전건 SKIP 원인) 전수 교정으로 **실 Gemini 라이브 경로 복구**. 산출물: `eval_results/e1_20260804/`(미추적 보존), D-022 재검토 등재(005fe3f)·mcp 가드 제거는 f15ac46 기커밋 확인.
