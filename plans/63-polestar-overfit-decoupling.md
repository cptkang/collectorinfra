...
> **상태**: 진행 중 — Step0(cee7cdf)·**P1 완료**(affca22, D-088)·**P4-1 완료**(a75ce5f, D-088 확장 — overfit_check 71토큰) · **P2 진행 중**(D-089): Stage 1 완료 — `src/db_adapters/` 어댑터 계층+레지스트리+부트스트랩, POLESTAR 템플릿 2종·`_check_routing_filter_misuse` 이동, `get_adapter().system_template/validator_checks` 배선(단일 경로), servername/hostname 가드는 infra 호출로 utils 잔류. 검증: test_db_adapters 7·subset 610 무회귀·overfit schema-literal 136→111·arch_check error 0. 남은 P2: pivot 조립기 클러스터 이동(Stage 2)
