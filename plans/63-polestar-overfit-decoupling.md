...
> **상태**: **완료** (전 트랙 P1~P4, 2026-07-20, 커밋 affca22~9fd1917). 후속 EX 라이브 측정은 **DB 데이터 미적재로 유효 검증 불가**(2026-07-21 실측: gold·pred SQL 모두 0행 → 빈 결과 동치로 위양성 통과, orchestration "26/26"은 무효). 하네스 버그 2건 수정(graph용 InMemorySaver 주입, 빈 골드결과 위양성 가드) 후에도 데이터 없이는 이동-불변 판정 불가 — **이동-불변 근거는 오프라인 골든 동치(text2sql 109)+전체 pytest 무회귀로 유지**, 데이터 적재 시 재측정 권장. ⑤편입 체크리스트 CLAUDE.md 반영은 관례상 `/revise-claude-md` 위임(docs/18 기록). — Step0(cee7cdf)·**P1 완료**(affca22, D-088)·**P4-1 완료**(a75ce5f, D-088 확장 — overfit_check 71토큰) · **P2 완료**(ac94f65+다음커밋, D-089): Stage 1(어댑터 계층+레지스트리+부트스트랩, POLESTAR 템플릿 2종·`_check_routing_filter_misuse` 이동+훅 배선)·Stage 2(pivot 조립기 클러스터 이동, 호출부 3곳 어댑터 직접 임포트). servername/hostname 가드는 infra 호출로 utils 잔류(P3 프로필화). 검증: test_db_adapters 7·subset 604 무회귀·overfit schema-literal 136→79·기준선 71→44 토큰·arch_check error 0. · **P4-2/P4-3 완료**(D-091): `testdata/generic_mon/`+`tests/test_generic_path/`(공통 템플릿·폴스타 리터럴 무오염·어댑터 미발동, E2E 옵트인) — 하네스가 공통 템플릿 D-085 예시의 잔여 `stat_date` 누수 검출·중립화(overfit 79→78, 기준선 43토큰). 편입 체크리스트 ⑤프로필/모델 추가(docs/18). · **P3 완료**(D-090): 메트릭 어휘 격리는 P2로 달성, 무선언 DB는 공통 LLM 경로(P4-2 검증). `GENERIC_LLM_MAPPING`(기본 OFF) 옵트인 범용 기간 힌트 배선(폴스타 리터럴 0, 선언 우선 EX 동치), LLM 자동등록 차단 유지(D-068 6차). 잔여 폴스타 리터럴(43토큰)은 전부 폴스타 게이트로 비누수 — 프로필 완전 이관은 라이브 검증 가능 시 후속. **Plan 63 전 트랙 완료** — EX 하네스는 라이브 DB/LLM 미접속으로 3경로×26 skip(오프라인 골든 동치 확인)
