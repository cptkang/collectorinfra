...
- **정본 1단(`deep_agent`) 경로** — 지난 run 은 2단으로 강등돼 돌았다(`degraded_reason=flag_off`). **이번 실험에서도 재지 않는다**(§7.1 **L-1**) — 1단 구성이 아직 갖춰지지 않았다(사용자 확정 2026-09-16). `--preflight` 가 단을 판정해 보고하고, 리포트 1절 경고(O-c)가 그 사실을 매 run 에 싣는다
