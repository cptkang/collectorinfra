...
| **B** | E2 크로스-호스트 상관 | E4(위상 가중) | M | L1 | B-6(=존 경계 확정) | ✅ **구현 완료 (D-109 + 정밀화 D-112: 필드 Jaccard + 위상 가중 + 클러스터 메타 감사)** |
| **B** | E3 동적 baseline | 독립 | M | L1 | B-3(해소) | ✅ **구현 완료 (D-110 순수 Python HW + 2차 D-113 STL 분해·statsmodels optional·HW 폴백)** |
| **C** | E5 변경 상관 | 피드 | M~L | L1/L2 | B-2(=폴스타 이력 확정) | ✅ **1차 구현 완료 (D-111, cmm_resource_lifecycle_history·promote)** |
...
- **L-2 (E1 강화) — 의미적 근접중복 주석** ✅ **구현 완료(D-114)**: SHA1 지문이 다르지만 의미적으로 같은 재발(메시지 표현만 다른 경우)을 **임베딩 유사도**로 감지해 **재발 count 병합 후보로 주석**(GPTrace 패턴). **억제 판정은 결정적 지문(compute_fingerprint) 유지**, 임베딩은 관측성 보강만. **폐쇄망 로컬 임베딩 모델 반입 = 블로커 B-7**(옵트인 기본 off·운영 활성화는 보안 협의 선행).
...
- **L-4 (E4 융합) — 토폴로지+텍스트 임베딩(DiLink식)** ✅ **구현 완료(D-114)**: E4 의존성 그래프에 **알람 텍스트 임베딩을 융합**해 연쇄·root 귀속 정밀화(root NAME↔알람 텍스트 유사도 → `noise_ctx["root_text_similarity"]` **주석**·cascaded/root 판정 불변). **폐쇄망 임베딩 반입 = 블로커 B-7**(옵트인 기본 off·운영 활성화는 보안 협의 선행).
...
>
> **★ 택일 완료 — (나) 확정(2026-09-09 · 사용자 · `plans/50` §0.8.4 U-H · D-207 후속)**: **(a) 게이트 동기 경계 probe를 폐기**하고 **(b) post-gate 비차단 보강만** 구현한다. D-117 ②("둘 다")는 (b)만으로 **개정**된다. 경계 상향(§18.4-3)은 L1 신호 판정(D-104 수준)으로 남는다. §18.6의 `gate_l3_probe_enabled`·`gate_l3_probe_timeout_seconds`는 **신설하지 않는다**(설계 이력으로만 보존). (가) 경량 동기 MCP 도구는 (나) 운영 실측 후 재검토 조건부.
...
| 날짜 | 변경 | 사유 |
|------|------|------|
| 2026-09-09 | **§18.3 지연 예산 택일 완료 — (나) 확정**: (a) 게이트 동기 probe 폐기 · (b) post-gate 비차단 보강만. D-117 ② 개정, `gate_l3_probe_*` 설정 신설 취소 | 사용자 확정(`plans/50` §0.8.4 U-H · D-207 후속). 회귀 0·범위 최소, (가)는 운영 실측 후 조건부 재검토 |
...
| 2026-07-22 | **Wave B(E2·E3) 구현 완료 + D-109~110 등재** | 사용자 Wave B 착수 지시(Wave A는 사용자가 직접 커밋 HEAD e530c73). B-6(존 경계)·B-3(순수 Python HW) 확정 반영. **E2(D-109)**: 신규 domain `correlation.py`(stdlib·signature_tokens[server 제외]·jaccard·ClusterState·match_cluster[동점 first_ts]), 워커 `_detect_correlated_storm`(온라인 그리디·첫 도착=대표·db_id 존 스코프·min_cluster_size번째부터 억제·버퍼 sweep+상한)·`_detect_storm` 독립 병존, 게이트 step7.5(correlated→SUPPRESS·storm 사유 구분), sig_label은 domain scan_signature_severity(정책 순수성). **E2 1차=필드 Jaccard만**(위상 가중 후속). **E3(D-110)**: 신규 domain `anomaly.py`(math/statistics·순수 Python Holt-Winters)+infra `polestar_metric_baseline.py`(cmm_metric_stat_h 읽기전용·Redis 캐시), 배선=enricher gather 5번째 코루틴→AlarmState.anomaly_severity→analyzer 후처리 상향 가드(dynamic_baseline_enabled AND enable_ai_severity_boost·상향 전용), **게이트 무변경**. 화이트리스트 게이팅(`kind in METRIC_SOURCE_BY_KIND`, Known Mistakes #2)로 1차 CPU/메모리. prometheus_client 미배선(§5.2=cmm_metric_stat 확정). 실측: definition_name(Utilization) assembler와 일치·period=24 일간 1차. **회귀 0 검증**: `tests/test_alarm/` **600 passed**, `arch_check --ci` exit 0, flags-off 회귀 확장(E2·E3 off→비트동일). Known Mistakes #1(E4 NAME)은 E2 1차 N/A(위상 가중 시 재확인)·#2(화이트리스트)는 E3 준수. |
| 2026-07-22 | **Wave C(E5) 1차 구현 완료 + D-111 등재 — E1~E6 전 항목 완료** | 사용자 Wave C 착수 지시. **선조사(폴스타 DB 실측)**: B-2(a) 확정 — 폴스타 개발 DB(실 제품 스키마 394테이블)에 변경이력 테이블 실재·적합(primary=`cmm_resource_lifecycle_history`: resource_id·event_time·lifecycle_type·description). 보조=core_config_history·sms_agent_patch_history. dev 데이터는 합성 placeholder(실운영 편입 시 event_time 단위·조인 재검증). **E5(D-111)**: 신규 infra `change_feed.py`(fetch_recent_changes·읽기전용·graceful)+domain `change_correlation.py`(overlay_changes·순수·덕타이핑). `_NOISE_CTX_KEYS`에 change_nearby/change_candidates, 게이트 step9 promote("변경 근접(원인성)"·**억제 아님·승격만**·change 모듈 미import). signals 미확장(§7.2). 1차=cmm_resource_lifecycle_history만(보조 union·외부 CI/CMDB·변경 실행/롤백 후속). **회귀 0 검증**: `tests/test_alarm/` **634 passed**, `arch_check --ci` exit 0, flags-off E5 섹션. **복구 이력(중요)**: 작업 중 사용자 GitHub Desktop 원격 병합(ux_improvement, HEAD e530c73→188eace)이 미커밋 Wave B를 `stash@{0}`로 대피시킴 → 팀리드가 Wave B 스태시 복원(600 passed 확인)→E5 재적용(3-way)→전체 재검증(634 passed)으로 **무손실 복구**. ux_improvement가 D-101~104를 등재해 Plan 64/§14.4 예약과 충돌(그 계획 착수 시 재부여 필요), D-105만 예약 유지. |
| 2026-07-23 | **E2 정밀화 2건 구현 완료 + D-112 등재 (D-109 후속 완결)** | 사용자 인터뷰 확정(E3 주간 계절성은 2차 보류). **①클러스터 메타 감사**: `match_cluster`→`(idx,score)`·`_detect_correlated_storm`→`(correlated,meta)`, decision_store `correlation_meta` 최상위 필드(대표 지문·member_seq·유사도·§10 signals 밖·E1 recurrence 전례). **②위상 가중**: E4 `DependencyGraph.id_of`/`is_related` 추가, `match_cluster(adjacent,topo_weight)` 인접 클러스터 보너스(correlation.py는 topology 미import·워커-주입 §10), 워커 그래프 자체 캐시(hot-path 클라이언트 open 회피), 옵트인 `correlation_topology_weight_enabled`(기본 off·회귀 0), **B-6 존 경계 불변**. **Known Mistakes #1 재확인(실측·의무 이행)**: `id_of(server_name)`는 엣지 로더 자식 행 NAME만 해소 → 순수 root 서버는 name 부재→id_of None→위상 보너스 미발동·**필드 Jaccard로 정상 군집(정확성 버그 아닌 우아한 열화)**. E4 enricher의 IN 조회 name 보강은 워커 경로 미적용 — 워커 root name 보강은 FI 후보. **회귀 0 검증**: `tests/test_alarm/` **665 passed**, `arch_check --ci` exit 0, flags-off 위상 off 섹션. 부수: gate 감사 대역 kwarg 누락(TypeError 삼킴→감사 누락) 2건 수정. |
