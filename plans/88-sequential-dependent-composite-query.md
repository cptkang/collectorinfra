...
- v10(2026-09-10) — **플래그 7종 코드 기본값 on**(사용자 지시: "`.env`는 ignore 파일이라 적용되지 않을 수 있다 — 설정값을 기본 동작으로"). `CompositeConfig` 7종 `False → True`(`plans/80` §5.4-③ 명시적 예외 · 근거 4항 config 주석 · D-203 항목 8). `PATH_PARITY` 기본 false 불변. off 전제 테스트 3건 명시 고정 · 도움말 라벨 기본 on 관례로 정정 · `.env`/`.env.example` "명시일 뿐" 주석. §11.3의 off 관측 단계(1-a·1-a' 등 "접두 없음")는 이제 `COMPOSITE_…=false` 접두를 **명시**해야 재현된다.
