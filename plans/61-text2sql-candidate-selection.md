...
> **상태**: **부분 구현 — 트랙 B·C 착수 완료(2026-07-14, §12 참조)**. E1 하네스(D-072)·트랙 B(E5-1 완료·E5-2 소비 연결·E5-3 상한 config화)·**트랙 C 전체(E6-1~4, D-076: 시맨틱 모델 3종·semantic_compiler·coverage_router 3경로·골든 34건)** 구현 완료. D-072·D-075·D-076 등재. 트랙 A(E2~E4)·E5-4는 측정 게이트 대기(미착수). 전 단계 **회귀 없는 옵트인 증분**(기본 OFF, Plan 61 테스트 153건 통과·회귀 0).
> **개정 이력**:
...
>
> **2026-07-14 업데이트(트랙 C 착수)**: D-072·D-075·D-076 `docs/02_decision.md` 등재 완료. 트랙 C(E6) 전체 구현(E6-1 시맨틱 모델 3종·E6-2 semantic_compiler·E6-3 coverage_router 3경로 배선·E6-4 골든 34건)·E5-2 소비 지점 연결 완료. 전 기능 `TEXT2SQL_SEMANTIC_COMPOSE=off` 기본, OFF 시 회귀 0(트랙 C 격리 후 test_nodes 실패 diff 0 실측). 트랙 A(E2~E4)·E5-4는 측정 게이트 대기(미착수 유지).
...
| **E2. 다중 후보 생성 (D-073)** | ❌ **미구현** | `src/nodes/candidate_generator.py` 부재. `TEXT2SQL_MULTI_CANDIDATE`/`CANDIDATE_COUNT`/`CANDIDATE_STRATEGIES` 플래그 미노출(`Text2SQLConfig`에 없음). `AgentState.sql_candidates` 부재 |
| **E3. 복잡도 분기** | ❌ **미구현** | `TEXT2SQL_COMPLEXITY_GATE` 플래그·분류 로직 부재 |
| **E4. 실행기반 후보 선택 (D-074)** | ❌ **미구현** | `src/nodes/candidate_selector.py` 부재. `TEXT2SQL_SELECTION` 미노출 |
...
| **E5-2. 값 검색 승격** | ✅ **연결 완료(소비 지점)** | 인프라 `src/schema_cache/value_index.py` + `redis_cache.save/load_column_value_index`. **소비 지점 연결**: `semantic_compiler.check_coverage._validate_literals`가 SMQ 필터 리터럴을 value_index로 검증(모델 밖 실측값 미검증 시 커버리지 밖 → 컴파일러 우회 환각 차단). coverage_router가 `cfg.synonym.value_retrieval` 시 value_index 전달. `SYNONYM_VALUE_RETRIEVAL=false` 기본 OFF(OFF 시 검증 스킵). 잔여: 값 인덱스 런타임 **주입(state 적재)** 은 후속(현재 검증 훅만 배선) |
| **E5-3. 사전 위생·거버넌스** | 🔶 **부분(상한 config화만)** | D-051 상한 15의 config 노출 완료(`SYNONYM_MAX_SYNONYM_SUPPLEMENT_TABLES`, `schema_analyzer`가 `cfg.synonym.max_synonym_supplement_tables`로 소비 — 모듈 상수는 기본값으로 잔존). **미구현**: 유사어 메타(등록출처·사용횟수·최종사용일·신뢰도), 장기 미사용 감쇠/정리, 충돌 우선순위 규칙 |
...
- 구현·노출됨(기본 OFF): `SYNONYM_FUZZY_MATCH`, `SYNONYM_VALUE_RETRIEVAL`(E5-2 소비 연결됨), `SYNONYM_SEMANTIC_MATCH`(자리예약), `SYNONYM_MATCH_CONFIDENCE_MIN`(기본 0.85), `SYNONYM_MAX_SYNONYM_SUPPLEMENT_TABLES`(기본 15 — E5-3 config화), `TEXT2SQL_SEMANTIC_COMPOSE`(**소비 연결됨** — query_generator·multi_db_executor 진입부 가드), `TEXT2SQL_SEMANTIC_FALLBACK`(기본 `llm` — 과도기), `TEXT2SQL_FALLBACK_CONFIDENCE_MIN`(자리예약, 트랙 A 게이트용). `.env.example` 반영 완료(별도 줄 주석 규칙 준수).
- 미노출(트랙 A 미착수): `TEXT2SQL_MULTI_CANDIDATE`, `TEXT2SQL_CANDIDATE_COUNT`, `TEXT2SQL_CANDIDATE_STRATEGIES`, `TEXT2SQL_COMPLEXITY_GATE`, `TEXT2SQL_SELECTION`.
- config 클래스: `SynonymMatchConfig`(env_prefix `SYNONYM_`)·`Text2SQLConfig`(env_prefix `TEXT2SQL_`) 신설.
...
4. **E5-3 잔여**: 유사어 메타(등록출처·사용횟수·최종사용일·신뢰도)·감쇠·충돌 우선순위 규칙(미착수).
5. **트랙 A(E2~E4)**: E1 커버리지율 측정 결과 커버리지 밖 비율이 정당화할 때 착수(§0.3 조건부). 착수 시 `semantic_fallback` 기본값 `candidate_then_human` 전환 + coverage_router 3단 폴백(신뢰도 게이트·HITL) 완성.
6. **E5-4**: E5-1 잔여 미매칭율 측정 후 판단(착수 게이트).
7. **커버리지 확장(후속)**: 현재 커버리지 내 필터는 ⊆{resource_type, ALARMSEVERITY} 보수적. HAVING 서버필터·동적 날짜·집계(상위 N)·LOB 속성을 dimension 카탈로그 사람 승인 루프(D-012)로 점진 확장 — E1 커버리지율 측정으로 우선순위 결정.
