...
> **★ 완결(시점 문서) · 2026-09-21** — [`plans/110`](./110-WIP-scenario-test-consolidated.md)(시나리오 테스트 통합 계획)으로 잔여를 이관하고 종료했다(INDEX 「이관 조항」 D-208 2차 적용 · 파일명 상태 태그 해제). 잔여(§12 게이트 · Wave S5~S7 · Y-8 · §19 Y-11·Y-12·O-e·V28~V31 등)와 **현행 실행 가이드 · Windows 부록 A**는 110 「▶ 실행 가이드」·§3.1·부록 A가 소유한다. 이 문서는 설계 근거·시점 기록으로 보존한다 — 아래 상태 표기는 **이관 직전 원문**이다.
>
> **작성일**: 2026-09-11
...
- **무효 판정**: 오케스트레이터가 안 떠서 1단이 내려간 프로파일(`degraded_reason=orchestrator_unavailable`·`package_missing`)은 **INVALID**다(10절 사유). 운영자 선택으로 비기준 단이 된 경우(`intent_flag_on`·`semantic_routing_off`)는 리포트 상단 경고만 남는다(D-221 ⑤ · 기준 단은 3단 `semantic_router` — D-225). 종전 어휘 `flag_off`는 D-225로 폐기됐다 — 1단 확정(`none`)은 부가 경로 opt-in이라 경고가 아니다.
