...
| "실시간 **디스크** 사용률" | DB | 지원 지표(CPU/MEM) 외 — Plan 70 §1 범위 |
...
- 디스크/네트워크 지표 확장.
- `process_api_base_urls_csv` → 제네릭 통합 rename(Plan 70 §1.3-5) — 프로세스 API 소비처 회귀 검토 필요라 본 계획에서는 신규 섹션 추가로 갈음, rename은 별도 턴.
...
- ~~미수집 판독 기준(1차)~~ → **반증 실측(사용자, 2026-07-24)**: yd에서 '미수집' 표기된 서버의 ID를 콘솔에서 조회해 **단건 API 호출하면 정상 반환**됨 — "수집 대상 아님" 가설로는 설명 불가. 갱신된 유력 가설(우선순위순):
  ...
  2. **API 배치 상한**: 한 콜의 resourceIds 대수가 일정 수를 넘으면 응답 measurement가 절단될 가능성(Plan 70 실측은 응답 크기만 확인, 항목 수 미검증).
