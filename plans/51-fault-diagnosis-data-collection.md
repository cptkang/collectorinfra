...
> **상태**: **완결(시점 문서) — 2026-09-09 잔여 `plans/91-TODO-fault-investigation-residual-consolidation.md`로 이관(1-5 플레이북 편입 · 1-11 L2 벤더 협의 · D-208) · `-WIP` 해제.** 착수·완료 추적은 91에서만. *직전 표기*: **부분 구현 — 재판정(2026-08-31 · `plans/85` §4 A-2)**. **L1(§5.1) 7종은 구현 완료**(`mcp_server` 폴스타 도구 8종 · D-122). **L3 접근 경로는 B로 확정**(2026-08-31 · **D-189** — §5.3 개정, 원 권고 A는 격하). 조사 실행은 `sre_agent`가 담당한다(D-118 위임 — 자체 수집기를 만들지 않는다). **잔여**: ①**L2 6항목 조사**(§5.2 — 벤더 협의) ②**§6 장애 유형별 플레이북·§4 카탈로그의 프롬프트 편입** (문서 자산은 완성돼 있으나 코드·프롬프트 소비 0건 — 가장 값싼 잔여. **2026-09-09 정정**: 편입점은 D-197로 생겼다 — `AgentSettings.investigation_guidance_extra` → `build_guidance()` 말미, 기본 `None`. **잔여는 내용 편입**: `sre_agent/application/investigation_guidance.py`에 알람 kind별 결정적 문구로 §6 6유형을 싣는다. 소유는 본 계획 — `plans/50` §0.8.2 ③) ③§3 분석 기법의 명시적 반영. 절별 대조표는 `plans/85` §4 A-2 참조.
...
> ### ★ 확정 (2026-08-31 · **D-189** — 아래 원 권고를 개정)
>
> **채택 = B(허용목록 read-only 명령 실행).** 아래 원 권고(1순위 A)는 **의사결정 이력으로 보존**하며
> 현행 결정이 아니다. 전환 사유는 D-189 참조 — 요지는 셋이다.
>
> 1. **B는 이미 구현·실동작 중이다** — `sre_agent/toolset_profiles.py::VM_DIAG_ALLOW`(light 20종 +
>    heavy 12종 가드 형태). HolmesGPT 채택(D-118) 시점에 bash toolset이 딸려 들어왔고, MVP 실 조사를
>    완주했다(D-123).
> 2. **A는 코드 자산 0건**이고 벤더 협의(P0-4) 리드타임이 통제 밖이다(`plans/66` §1.5 ③ 실측).
> 3. **B의 위험은 §9 요구를 초과하는 통제로 방어된다** — 부하 가드 L-1~L-3(`timeout 20 nice -n 10`
>    접두 **강제**, `top -b -n 1` 1회 고정), deny에도 **같은 가드 접두를 적용**해 우회 차단,
>    L-4 `(db_id, host)` in-flight 거부, 전 수집 감사 jsonl. **prefix allowlist는 지침 주입과 달리
>    실효 강제다** — 지침만 넘기던 시절엔 프로덕션 호출부가 0건이라 강제가 0이었다(2026-08-27 실측).
>
> **A의 처리**: 폐기가 아니라 **격하**한다 — *"폴스타 벤더가 read-only 진단 스냅샷 API를 제품 기능으로
> 제공하고 그것이 `VM_DIAG_ALLOW`가 얻는 신호를 대체·보강함이 실측될 때"* **추가 채널로 편입**을
> 재검토한다. 그때까지 A는 이 절에 **미채택 대안**으로만 존치한다(D-161 승격–폐기 동반 — 폐기할
> 코드가 0건이라 삭제 대상은 없다).
>
> **C(로그 전송)는 여전히 미채택**이다 — 과거 로그 포렌식이 필요해지면 그때 별도 결정한다.

<details>
<summary>원 권고(2026-06-26 · 의사결정 이력 — 현행 아님)</summary>
