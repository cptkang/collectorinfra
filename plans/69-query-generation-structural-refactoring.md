...
> **상태**: v5 — **P0 부분 실행 완료 후 중단(사용자 지시 — 커밋 포인트까지만)**: §1.1 **①~⑧·⑩ 수정·커밋 완료(9건, 각 독립 커밋 + 재현 테스트)**, 영역 스위트 기준선 대조 실패 집합 완전 동일(회귀 0). **이월 = ⑨(S-IR1 도달성 — 프롬프트 바이트 변경·sha256 골든 갱신 수반) + P1~P5 전체.** 재개 시 ⑨부터.
> **개정 이력**:
...
- **D-134 (본 계획)**: 쿼리 생성 구조 리팩토링 — 프롬프트 조립 공유 빌더(경로 대칭), sql_dialect 단일화, IR 계층 분리(nodes↔tools 순환 해소), 실행·감사 기록 헬퍼 통합, 멀티 검증 강화 옵트인. D-066/D-067/D-088/D-089 경계 준수 명시. ※예약은 `docs/02_decision.md:8` 안내 라인에 반영됨(v4 재확인 — 현 최대 **D-136**·다음 D-137, **D-132는 등재 완료**되어 미등재 예약은 D-134뿐). 등재 직전 `## D-` 헤더·「변경 이력」 grep 재확인.
