...
> **상태**: v9 — **계획·후속·별건 전량 완결. 잔여 0건.** P0~P5 구현·D-134 등재·문구 통일 적용·후속 3건에 이어 마지막 별건(EAV 검증 리터럴 이관)까지 완료(2026-08-05).
> **개정 이력**:
> - v9(2026-08-05) — **잔여 별건 완료(843e624)**: 공용 계층 마지막 폴스타 리터럴 3토큰(`configuration_id`·`stringvalue_short`·`server.Server`) 제거. 실측 결과 두 리터럴이 **이미 프로필에 선언**돼 있어(`direct_join.config_column`·`value_column`·`value_joins[0].eav_attribute/entity_column) 어댑터로 함수를 옮기는 대신 **선언 참조로 전환**(훅 신설 불필요 — Plan 63 §9 준수). `_validate_forbidden_joins` 검출 조건·메시지 4곳을 `_eav_bridge_hint`로 공유 조립, 공용 폴백은 DB-agnostic 일반명(`prompt_blocks.py:349` 선례). 선언 결측 시 안내 생략·패턴 1 검출 스킵(자리표시자 조립은 없는 컬럼 지시가 재시도 프롬프트에 실려 금지). `check_left_join_where_demotion`은 docstring 예시만 일반화(코드 무변경 — overfit이 docstring을 스캔 대상으로 삼는 특성). 부수 이득: 금지 조인 검출이 폴스타 전용 → 임의 EAV 프로필 대응으로 일반화. 검증: 폴스타 3프로필×6시나리오 18건을 HEAD 모듈과 대조해 **메시지 바이트 완전 동일**(생성 12건·불일치 0), 픽스처 shape 드리프트 보정(목이 실 프로필 선언 누락), overfit 기준선 `src/sql_validation.py` 항목 제거(3토큰 전량 스테일·게이트 대상 0건 실측), 전 게이트 그린.
> - v8(2026-08-04) — **후속 3건 완료(사용자 지시)**: ①피벗 wrapper 제거(e54d785 — 소비 테스트 6곳 신 진입점 전환·9조합 바이트 동일 선실증, `build_multi_resource_pivot_sql` 소멸, block 기본값 상수 통일) ②SQL 검증 코어 분리(874cba6 — `src/sql_validation.py` 신설 900L·순수성 실측 20심볼 이동, query_validator 173L 축소+재수출 shim, tools→nodes 0건·nodes→tools 단방향 확정·지역 임포트 3곳 모듈 승격. 목적지는 tools의 DB-agnostic 하드 인바리언트 실측으로 독립 모듈 채택, arch/overfit 편입 — 기준선 delta는 경로 이동 3토큰+스테일 순감 1토큰) ③W-6 멀티 재료 조달(d90f260 — cache_manager 경로 재사용·4재료 대칭 수록·독립 try 5지점, 재료 부재 시 바이트 불변·스냅샷 12키 무갱신, 싱글톤 순서 의존 발견→빈 캐시 대역 고정. 길이 실측 DB당 현실치 +1,949자(13.6%)·설명 상한 +9,909자(69%) — 재료 조건부·상한 발현 시 절단 상한은 단일·멀티 공통 후속으로 두고 전 재료 수록 채택). 전 단계 게이트 그린(기준선 6건 대조·골든·arch/overfit).
> - v7(2026-08-04) — **문구 통일 사용자 건별 인터뷰·적용 완결**: 채택 6건 — 1단계 W-1·W-3·W-4·W-7·S-1(fbb2ca4 — 멀티 system 2키만 스냅샷 갱신, 단일 8키·멀티 human 2키 무갱신으로 W-7·S-1 프롬프트 무영향 실증, W-1 inline 분기·style 파라미터 소거) + 2단계 U-1(2413753 — `build_unmapped_fields_block(hangul_alias=)` 공유 빌더 통합, 멀티 바이트 완전 불변(9조합 실측·스냅샷 12키 무갱신), 단일은 폼필 미매핑 경로만 ②③⑤ 변경, 'Template B' 미정의 참조·`_[h,d,m]` 열거 해소. 폴스타 리터럴은 어댑터 상수(`METRIC_PIVOT_TABLE/KEYS`) 조달 — 호출부 상수 원안은 qg overfit 기준선 부재로 --ci 위반이라 실측 근거 이탈, 공용 계층 리터럴 순감). **W-5 반려**(FK 헤더 현행 유지·`relationships_header` 존치). 승인 결과 434b02f, §2 통합 보류도 U-1로 해소 — §0.3-2 2단계 절차 종결.
...
- 문서: `plans/67-WIP-stepwise-llm-query-composition.md`(§1.2 경직성 — 기능 트랙과의 경계), `plans/63`(3계층·overfit), `docs/02_decision.md`(D-035·066·067·076·088·089·099·128·131), `docs/18_known_mistakes.md`(비대칭·초기화·일괄 갱신·worktree 기준선 원칙), `spec.md:721`(행 수 상한 — §0.2-5 모순)
