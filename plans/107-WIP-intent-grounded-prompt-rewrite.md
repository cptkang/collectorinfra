...
> **작성일**: 2026-09-20 · **개정**: 2026-09-21 (**v2** — 코드 재실측 + 문헌 재검토 / **v2.1** — `plans/94` 계열과의 경계·소유권 정리 / **v2.4** — 94 계열 통합(`plans/110`)에 따른 참조 갱신 / **v2.5** — 구현(W-1·W0.5·W0·W1·W2·W5 섀도·W3 메커니즘 · §12) / **v2.6** — 권고 결정 반영: 1단 판정 입력 결함 수정 · U-10 부분 재개방 · G-9 근거 정정 · §12.9)
> **상태**: **부분 구현**(2026-09-21 · §12) — W-1·W0.5·W0·W1·W2 랜딩 · W5 섀도까지 · W3 메커니즘까지(기본 off) · **W2.5·W4 미착수**. 사용자 확정 게이트 G-1~G-7·G-9 전건 확정(§11). **U-10은 v2.6에서 `semantic_compiler`만 재개방·수정**(`binding.py`는 운영 비활성 경로라 보류 · §12.9). 남은 것은 측정 산출물 2건(G-5 W5 보고 · G-6 U-7)과 로컬 MLX 효과 측정(§12.9) · 파일명 `-WIP`
> **성격**: 현행 실측 + 구현 계획 + **구현 현황(§12)**
...
> **측정·수정 가족(v2.1 추가 — §6에 처분 명시 · v2.4: 아래 다섯 건은 `plans/110`으로 통합·종료)**: **`plans/110`**(시나리오 테스트 통합 계획 — 이 계획의 E2E 회귀·`rewrite_trace` 수집·수용 기준은 **110 §3.1이 소유** · 원 정의 94 §19) · `plans/94`(하네스 설계 정본 · 시점 문서) ·
...
| **W0** | 골든셋·회귀 케이스 (**v2.1 축소 — 소유권 분할**) | `testdata/routing_gold/rewrite.yaml`(가칭 · **이 계획 소유**) — **슬롯 단위 채점만** 담는다: **파서·라우터 출력 스냅샷 + 기대 슬롯**으로 `merge_frame`·`rewrite_needed`·렌더를 **LLM·서버 없이** 채점(현 `testdata/routing_gold/`에는 `routing.yaml`·`decomposition.yaml` 2건만 있고 `rewrite.yaml`은 없다 — 실측 2026-09-21). 범주 ①§2.4 오염 **4건** ②지시어 후속 ③위치 전환 ④되묻기 재개 ⑤멀티 의도·순차 의존(plans/88) ⑥멀티 DB(불변식: `plans/79` §1.1) ⑦**게이트 `pass_through` 케이스**.<br>**E2E(서버 왕복) 회귀는 신설하지 않는다** — `plans/110` **§3.1**이 소유한다(원 정의 `plans/94` §19 · §6). 오염 4건 중 **3건은 94 카탈로그에 이미 단언이 있다**(F-01·F-03·F-04 / B-10·A-02 / G-02 — §6 94 행) | 0 | — |
...
| **W0.5** | 승격 전후 판정이 달라지는 케이스를 **전수 열거**하고, 각각이 "원문 기준이 옳다"를 만족할 것 · 1·2단에서 §2.4 LIMIT 사례와 **동형 결함이 재현되지 않을 것** · **(v2.1)** `plans/110` §3.1 **V28**(원 94 §19.4) — 오염 4건 대응 시나리오가 회귀 0 |
...
| W3 | 소비자별 기존 평가 대비 **비열화**(`eval_routing` `multi_preserved` · `eval_text2sql` · **`plans/110` §3.1 V29**(원 94 §19.4)) — 기준값은 전환 전 기준선 측정으로 정한다(v1 G-8 삭제 — 게이트가 아니라 산출물). **(v2.1)** 시나리오 판정은 **하네스(원 94 → v2.4부터 `plans/110`)가 소유**하므로 이 계획은 기준선 run id와 비열화 폭만 적는다 |
| W4 | 되묻기 재개 후 **재질의 없이** 실행 완료 · 답변 외 슬롯 불변 |
| W5 | 섀도 기간 R1·R2·R6 검증 실패율 + **게이트 `pass_through` 비율** 보고 → enforce 전환은 사용자 승인. **(v2.1)** 이 비율은 하네스가 읽어야 하므로 **적재는 이 계획(§4.9 `rewrite_trace`) · 수집·리포트는 `plans/110` §3.1 O-e**(원 94 §19.3)가 소유한다 — `plans/94` §17 **O-b**가 *"제품이 싣지 않으면 감사 로그로 못 가져온다"* 를 이미 확정(`analyze.LLM_COST_UNMEASURABLE`)한 것과 같은 함정이다 |
...
| **`plans/98`**(v2.1) | **제품측 코드 수정 장부 — 앵커 경합 2건 확정.**<br>①**CU-18**(잔여 · **98이 계속 소유** — v2.4: 98 종료로 잔여 소유는 `plans/110` §3.3 · 98 G-10 소관) = `assembler.build_alarm_history_sql`(`assembler.py:1474`) 호출부(`:1562`)가 `resolve_query_limit` 결과를 **쓰지 않는다** — **「소비 누락」 축**이다(`plans/98:371-377`: *"고친 값이 소비되지 않는다" 유형*). §2.3의 `resolve_query_limit`은 **같은 슬롯의 「입력 오염」 축**이고 **축소 방향은 D-066으로 닫혀 있다**(확대 방향만 조건부 잔존 · W-1·U-10) → **107은 CU-18을 손대지 않는다.** 다만 **같은 슬롯의 반대편 축이므로 한 차수로 묶어 실행하면 검증이 한 번에 끝난다**는 실행 권고는 남긴다.<br>②**CU-15**(잔여 · 게이트 **G-11** 대기 — D-153 후속1 충돌 · v2.4: 잔여 소유는 `plans/110` §3.3)는 **§2.1 R4**(`subagents.py:344-352` `_inject_demonstrative_hostname`)와 **같은 경로**다 → **이 계획은 그 앵커를 건드리지 않고 98 G-11의 결론을 승계**한다.<br>③**설계 선례(경합 아님)**: CU-8은 `resolved_limit` 승격을 일반화하는 대신 **승격 의존 자체를 없앴다**(`output_generator.py:1082` `_applied_row_limit()` — 소비 시점에 `query_attempts`의 SQL에서 적용 상한을 다시 읽는다) → **W0.5도 "승격"이 유일한 해법인지 슬롯별로 판단할 것.**<br>④**경계 대칭**: 98 §0은 *"`plans/94`의 `scripts/scenario/`·`testdata/scenarios/`를 건드리지 않는다"* 를 선언한다. 이 계획도 **같은 선언**을 한다(§5 W0 · 94 §19에 기록) |
| **`plans/99`**(v2.1) | 재측정 실험 설계. 이 계획은 **독립 run을 요구하지 않는다** — 99 E-3(전 시나리오 재측정 — v2.4: `plans/110` §3.2 재테스트)에 **arm을 추가하지 않고**, 110 §3.1 V29(원 94 §19.4)의 비열화 판정만 기존 run 대비로 한다(추가 예산 0 · D-127 무관) |
...
| **U-10**(v2.3) | **`src/tools/binding.py:161`의 빈 `query` 폴백이 실제로 발동하는지** — 실측 확정분: `resolve_limit(query or context.user_query, default_limit=context.default_limit)`에서 **LLM이 빈 값을 넘기면 `context.user_query`로 폴백**하고, 그 값은 `column_deriver.py:130-142`의 `ToolContext(user_query=…)` = `query_generator` 경로의 `state["user_query"]`(1·2단 **재작성문**)다. `default_limit`은 승격값(`deps.default_limit`)이라 **`semantic_compiler.py:276`과 정확히 같은 비대칭**(축소 닫힘 · 확대 잔존)이고, 여기엔 **빈 `query` 폴백이 하나 더** 얹힌다. 미확정은 *"LLM이 실제로 빈 값을 넘기는 빈도"* 와 *"확대 방향 성립 사례가 있는가"* 다(**추정**) | 섀도 로그로 `resolve_limit` 도구 인자 분포 수집(빈 값 비율) + 확대 방향 성립 사례 탐색 — **W-1**. 성립하면 **G-9 범위를 다시 연다**(그때는 `semantic_compiler.py:276`·`binding.py:161` 2곳이 대상) · **v2.5 판정: 확대 방향 성립**(run `20260918-182507` R2 기준 26쌍 · 단일 task 8쌍 — §12.4). **v2.6 처분(사용자 "권고에 맞게 진행하라")**: `semantic_compiler.py` LIMIT 해석을 `surface_query`로 **수정** · `binding.py:161`은 `TEXT2SQL_STEPWISE_DERIVATION` 기본 off·`.env` 미기재로 운영 실효 0이라 **보류** — §12.9 |
...
|---|---|---|---|
| **G-5** | W5 섀도 보고 — R1·R2·R6 검증 실패율 + 게이트 `pass_through` 비율(`plans/110` §3.1 **O-e**가 수집 — 원 94) | `REWRITE_VERIFY_MODE`를 `enforce`로 올릴지 | **아니다** — 섀도 착수는 확정이다 |
...
## 12. 구현 현황 (**v2.5** · 2026-09-21)

> 사용자 지시(2026-09-21): *"107번 계획을 구현하여 94번 시나리오 테스트시 검증할 수 있도록 하라."* G-1 확정 문구의 "(a) 선행 → 결과 보고 후 (b)(c)" 가운데 **(b)까지**로 범위를 잡았다. 94 §19의 기계 판정 항목이 전부 W2 이후 산출물을 전제하기 때문이다.
> **커밋하지 않았다**(작업 트리). 실 LLM 호출 0 · `RUN_E2E` 0 · 94/110 시나리오 실 실행 0(D-127) — 실행은 사용자 승인 사항이다(§12.7).

### 12.1 실행 단위 상태

| WU | 상태 | 산출(파일) | 검증 |
|---|---|---|---|
| **W-1** | **완료** | §12.2(실측 표) | 전수 grep + 실 run 체크포인트 디코드 |
| **W0.5** | **랜딩**(플래그 없음 — `plans/80` §5.4-③ 예외, 근거는 `query_gen_common.surface_query_for_judgment` 독스트링) | 공통 판정 입력 `src/utils/query_gen_common.py` `surface_query_for_judgment` · 급증 5종 `src/nodes/query_generator.py` `_try_spike` · 순위·최상급 `src/nodes/semantic_compiler.py` `_compile_ab`(`compile_from_nl`·`compile_smq` 인자 `surface_query`) · 멀티 대칭 `src/nodes/multi_db_executor.py` `_try_semantic_compile`·`_generate_sql` · **전체스코프** `src/nodes/query_validator.py`(`validate_sql` 입력) · `multi_db_executor._validate_sql(surface_query=)` | `tests/test_nodes/test_plan107_w05_surface_judgment.py` 21 |
| **W0** | **랜딩** | `testdata/routing_gold/rewrite.yaml` — 12건 · 범주 ①~⑦ 전부 · §2.4 오염 4건 전부 | `tests/test_nodes/test_plan107_rewrite_gold.py` 16 |
| **W1** | **랜딩(섀도)** | `src/domain/intent_frame.py`(`IntentFrame`·`SlotValue`·`merge_frame`·`rewrite_needed`·`verify_rewrite` — `src.domain.*` 외 import 0) · `src/nodes/intent_frame_builder.py`(`build_intent_frame`) · 상태 `raw_user_query`·`display_query`·`intent_frame`·`rewrite_trace`(`src/state.py` — 요청 스코프, 매 턴 초기화) | `tests/test_nodes/test_plan107_intent_frame.py` 46 |
| **W2** | **랜딩** | 렌더 `src/prompts/canonical_query.py` · 감사 `src/security/audit_logger.py` `log_rewrite_trace` · done 페이로드 `src/api/routes/query.py` `_rewrite_trace_fields`(완료 done 4곳 — 텍스트·파일 스트림) | 위 46건에 포함 |
| **W5** | **shadow까지** | `verify_rewrite` · `REWRITE_VERIFY_MODE=shadow`. enforce는 받지 않는다(G-5 — 측정 뒤 사용자 확인) | 위 46건에 포함 |
| **W3** | **메커니즘까지(기본 off)** | `get_prompt_query` + 배선 3곳 — `query_generator._build_user_prompt(prompt_query=)` · `output_generator._build_response_prompt` · `general_inference` 사용자 메시지. 소비자 켜기는 하지 않았다(V29 기준선 선행) | off일 때 사용자 프롬프트 바이트 동일 단언 · `prompt_render_diff --ci` 0 |
| W2.5 | **미착수** | — | G-6 U-7 측정 선행 · 실행은 D-127 건별 승인 |
| W4 | **미착수** | — | `plans/106` H1이 코드 0건 — 선행 없음 |

설정: `src/config.py` `IntentFrameConfig`(`INTENT_FRAME_ENABLED`·`CANONICAL_QUERY_MODE`·`CANONICAL_QUERY_CONSUMERS`·`REWRITE_GATE_MODE`·`REWRITE_VERIFY_MODE` — 전부 기본 off, 받지 않는 값은 경고 후 off) · 관리자 UI 그룹 `intent_frame`(`src/api/settings_catalog.py`) · 도움말 `config/settings_help/intent_frame.yaml`.
W2.5·W4용 플래그(`INTENT_ENRICHMENT_ENABLED`·`CLARIFICATION_ANSWERS_ENABLED`)는 **만들지 않았다**(투기적 코드 금지).

### 12.2 W-1 실측 — §2.3·§4.4 전제의 확정

**(a) `user_query` 상태 읽기 23파일**(계획 시점 22) — W-1 시점 작업 트리 라인:

| 구분 | 위치 |
|---|---|
| **원문**(공통 전단·오케스트레이션 수준) | `input_parser.py:111·118·124·134·150` · `field_mapper.py:76·99` · `semantic_router.py:118·609` · `intent_planner.py:253` · `sequential_runner.py:52·67` · `replanner.py:96` · `deep_agent.py:211` · `subagents.py:770·777·820·839`(`_make_isolated_input` — 교체 **전**) · `result_aggregator.py:356` |
| **재작성문**(1·2단 격리 파이프라인) | `query_generator.py:495` · `query_validator.py:112·120` · `result_organizer.py:331` · `multi_db_executor.py:346·501·547·586·637·649` · `output_generator.py:175·643·714·962·1013·1065` · `general_inference.py:243` · `cache_management.py:52` · `synonym_registrar.py:68` · `fault_diagnosis.py:104` · `subagents.py:1091·1221` · `process_query.py:780` · `host_inspect.py:161` · `entity_locator.py:753` |
| 원문 우선 폴백 | `subagents.py:296`(`original_user_query or user_query`) |
| 쓰기(교체) 지점 | `agent_orchestrator.py:356`(2단 R2) · `deepagents_tools.py:369`(1단) · `subagents.py:1268`(R1 — 단일 DB면 `sub_query_context`, 없으면 `sub_query`) · `result_aggregator.py:660` |

3단 그래프 경로는 전부 원문이다(단일 DB 재작성 0곳 — §0.1 재확인).

**(b) R6(`parsed_requirements["original_query"]`) 소비처**: 쓰기 `input_parser.py:124·263·364`(원문 복사) → `subagents.py:748`·`result_aggregator.py:658`(→ `sub_query`). LLM 프롬프트 `query_generator.py:1606` · `output_generator.py:283·643·961·1014·1066` · `multi_db_executor.py:1135·1395·1416·1517·1577·1835·2005·2332`. 결정적 판정 `schema_analyzer.py:536·567` · `multi_db_executor.py:2274`(`resolve_stat_month_range`) · `subagents.py:372`(CU-15 앵커 — 미접촉).

**(c) §2.3 표면어 판정의 최종 처분**:

| 슬롯 | W-1 판정 | W0.5 |
|---|---|---|
| `resolve_effective_limit` | 승격됨(D-066) | 제외 |
| `resolve_query_limit` | 축소 닫힘 · **확대 잔존 — 실 run에서 성립**(§12.4) | 제외(G-9 · U-10 재개방은 사용자 확정) |
| `is_realtime_usage_query` | 승격됨(D-066 후속7 · `subagents.py:820`) | 제외 |
| **`has_all_scope_keyword`**(판정 보류 → **오염 확정**) | 검증기 안전망(`sql_validation.py:193` · `multi_db_executor._auto_limit_or_none`)이 `resolved_limit`을 읽지 않는다 — LLM SQL에 LIMIT이 없으면 재작성문의 "전체/서버별" 유무로 10,000/기본 상한이 갈린다 | **승격** |
| `refers_to_demonstrative_server` | `intent_planner.py:613`은 원문 · `subagents.py:372`는 R6 | 미접촉(CU-15 앵커 — `plans/110` `98·CU-15`) |
| 순위·최상급(`_is_superlative`·`_resolve_ranking`) | 오염(트랙 C · 운영 `.env` `TEXT2SQL_SEMANTIC_COMPOSE=true`) | 승격 |
| 급증·비교기간·파일시스템·절대임계·기타지표 | 오염 — **네 종이 `_try_spike` 한 함수 안에서 급증 어휘 게이트 뒤에 있다**(급증 → 비교기간 → 파일시스템 → 절대임계 순 조건 진입) | 승격 |
| 시간범위(`resolve_stat_month_range` · `query_generator.py:508`·`multi_db_executor.py:2274`) | 오염 | **미승격** — G-9 목록 밖, 3단 전환 시 재판정 |

W-1이 새로 찾은 표면어 판정(13종 밖): `semantic_compiler`의 `normalize_smq`(상위어·용량·기간 승격) · `_monthly_breakdown_blocked`. 전부 `compile_from_nl`의 `user_query`를 읽는다 — **미승격 · 3단 전환 시 재판정**.

**(d) 실 run 발현 — run `20260918-182507`(2단) 체크포인트 디코드**: thread 353 · task 쌍 518(단일 task 460) · 재작성 254쌍(49%). 판정 함수는 **운영 코드 그대로** 원문과 `sub_query` 양쪽에 돌렸다. **R2(=R6)는 정확한 값**이고, R1(`sub_query_context`)은 체크포인트에 없어 **근사**다.

| 슬롯 | 탈락(단일/복합) | 추가(단일/복합) |
|---|---|---|
| 전체스코프 | 1 / 0 | **8 / 18** |
| 건수 | 0 / 3 | 0 / 1 |
| 순위/최상급 | **3** / 1 | 0 / 0 |
...
→ **카탈로그 빈도(절대임계 21 · 순위 20 · 비교기간 8 · 파일시스템 4 · 급증 1)는 실효 노출이 아니다.** W0.5가 실제로 바꾸는 것은 **단일 task 분기**이고, 실 run 기준 순서는 **전체스코프 9 > 순위/최상급 3 > 절대임계 3(실효 0 — 급증 게이트 뒤) > 나머지 0**이다. 실시간 탈락 1건은 이미 승격(D-066 후속7)으로 막혀 있던 것이 실측으로 확인된 사례다.

### 12.3 W0.5에서 판정이 바뀌는 케이스 전수

| 경로 | 변화 | 판단 |
|---|---|---|
| 3단 그래프 | **없음** — 원문 키가 없어 `current`(=`ctx.user_query`)를 그대로 쓰고, `surface_query` 미지정 기본값은 종전과 같은 SQL이다(바이트 동일 단언) | — |
| 2단 복합 계획 | **없음** — 복합이면 task 스코프 재작성문을 쓴다 | 실 run 복합 절대임계 탈락 4쌍("CPU 90% 넘는"이 목록 task에서 빠짐)이 **올바른 축소**다 — 원문을 읽으면 다른 task 조건이 번진다 |
| 1·2단 단일 · 순위/최상급 3쌍 | "가장 많이 상승한" → "상승폭이 가장 큰"(표면어 목록 밖) — 종전 정렬 없음 → **원문 기준 DESC + 상위 1건** | 원문 기준이 옳다(사용자가 최상위를 물었다) |
| 1·2단 단일 · 전체스코프 확대 8쌍 | "…서버와 그 개월 수" → "…각 서버별 초과 개월 수" — 종전 안전망 10,000 → **원문 기준 기본 상한** | 원문 기준이 옳다(사용자가 전체를 말하지 않았다). 실효는 LLM SQL에 LIMIT이 없을 때뿐 |
| 1·2단 단일 · 전체스코프 탈락 1쌍 | 지시어 후속 "그 서버들 중에…"(원문이 "서버들"을 가짐) → 원문 기준 10,000 | 무해 — 서버 필터로 이미 한정된다 |
| 1·2단 단일 · 절대임계 추가 3쌍 | 재작성이 "넘고"→"초과"로 정규화해 정규식이 잡던 것을 원문 기준에선 못 잡는다 | **원문 기준이 불리한 방향**. 다만 급증 조립은 급증·파일시스템 어휘가 먼저 있어야 진입해 3쌍 모두 실효 0 — 기록만 한다 |
| **1단 다중 호출** | `is_composite`가 항상 False라 task N이 전체 원문을 읽는다 — 다른 task 조건이 번질 수 있다 | 선례(`resolved_limit`·`realtime_usage_intent`)와 같은 한계. 1단 전용 판정은 두지 않았다(D-225 ⑦). 이번 run은 2단이라 실측 없음 |

**소비 시점 재판독(CU-8 선례) 판단** — 전 슬롯 재판독을 채택했다. 상태 필드를 새로 두지 않아도 격리 입력에 `original_user_query`가 이미 있다(`subagents.py:839`).

### 12.4 U-10 판정 — **확대 방향 성립**

- run `20260918-182507` R2 기준 **26쌍(단일 task 8쌍)**에서 재작성문이 전체스코프 표면어("서버별"·"서버들"·"각 서버" — `query_gen_common.py:444` 목록)를 **새로 얻었다**. 건수 표면어 추가 1쌍(복합).
- `resolve_query_limit`은 그 표면어가 있으면 `default_limit`(승격값)에 닿기 전에 `_ALL_QUERY_LIMIT`을 돌려준다(`query_gen_common.py:520-527`). 1·2단에서 `semantic_compiler.py:276`·`binding.py:161`(빈 `query` 폴백)은 재작성문을 읽으므로 **확대 방향 오염이 실재한다.**
- **이번 구현은 이 두 호출부를 바꾸지 않았다.** G-9 확정 문구가 *"성립하면 G-9 범위를 다시 연다"* 이므로 **사용자 확정 사항**으로 올린다. 한계: R1은 미측정이다.
- **v2.6 처분** — §12.9 ②.

### 12.5 계획 대비 이탈 (근거 · 되돌리는 비용)

| # | 이탈 | 근거 | 되돌리는 비용 |
|---|---|---|---|
| 1 | W0.5 우선순위를 카탈로그 빈도가 아니라 **실 run 단일 task 분기**로 판정했다 | §12.2(d) — 급증 계열 4종은 급증 어휘 게이트 뒤라 카탈로그 빈도가 노출이 아니다 | 문서만 |
| 2 | **`has_all_scope_keyword` 승격을 추가**했다 | W-1이 "판정 보류"를 오염으로 확정 · G-9 ㉮ 규칙(미승격 슬롯) · 실 run 단일 9쌍으로 발현 빈도 최상위 | `query_validator.py` 1줄 + `multi_db_executor._validate_sql` 호출 3곳 |
| 3 | 복합 계획이면 원문을 쓰지 않는다(선례보다 엄격) | 원문에 다른 task 조건이 있다 — 실 run 복합 절대임계 4쌍 | `surface_query_for_judgment` 조건 1줄 |
| 4 | 렌더러 입력을 `IntentFrame`이 아니라 **plain 사전**(`slot_dict()`)으로 했다 | `scripts/arch_check.py` — prompts는 utils만 참조한다(§4.10 초안 불가). 출처 키 문자열 일치는 테스트가 고정한다 | 계층 규칙 변경이 필요하다 |
| 5 | 1·2단 `rewrite_trace` 운반을 **thread_id 키 in-memory 보관소**로 했다(키·레코드·수명 bound) | 격리 파이프라인 상태는 최종 state로 합쳐지지 않는다 · `request_id`는 라우트가 state에 싣지 않는다 | 모듈 1곳 |
| 6 | 검증 채널 이름을 `r1`·`r2`가 아니라 **`user_query`·`original_query`**로 했다 | 노드는 R1인지 R2인지 구별할 수 없다(`subagents.py:1268`이 둘 중 하나를 넣는다) | 이름만 |
| 7 | `raw_user_query`는 `INTENT_FRAME_ENABLED`일 때만 싣는다 · 병기 블록 원문 줄은 `display_query`(R3 치환본)가 1순위다 | off 바이트 동일 · 미선택 존 위치어 재주입 방지(D-154) | 라우트 헬퍼 1곳 |
| 8 | `default` 출처 슬롯이 없다(시스템 LIMIT 상향 등은 슬롯으로 만들지 않는다) | 기본값 정책(106 H1.5) 부재 · 만들면 모든 질의가 게이트를 통과하지 못한다 | 빌더 1곳 |
| 9 | **V29는 부분 구현**이다 — 비교 키 `(env, tier, base_profile, arm)` 튜플(`analyze.COMPARISON_AXES`·`select_baseline`)과 고른 기준선의 `summary.meta.regression_baseline` 기록은 들어갔다. **기준선 run id를 사람이 지정하는 수단(CLI)은 없다** — `__main__.py`는 병행 세션(36) 소유라 넣지 않았다. 실제 기준선 선언은 `plans/110` §3.2대로 재테스트 뒤 | 범위 · 파일 소유 | CLI 1개 인자 + `select_baseline` 분기 |

### 12.6 검증 증거

| 항목 | 결과 |
|---|---|
| 신규 테스트 | 113건(W0.5 21 · W1~W5 46 · W0 골든 16 · 94 §19 하네스 30) + 단언 키 전수 쌍 26→30 |
| 전체 `pytest` | **8428 passed · 31 skipped · 0 failed** |
| `arch_check --ci` | exit 0 · ERROR 0 · WARN 81→85(+4 = 노드 4곳 → `intent_frame_builder`, 기존 `prompt_blocks`·`key_bridge`와 같은 패턴) |
| `overfit_check --ci` | exit 0 · 신규 유입 0 |
| `prompt_render_diff --ci` | exit 0 · 바이트 동일 |
| ruff(`uvx --offline`) | 신규 `src/` 모듈 0 · 수정 파일 증가분은 파일 기존 관례(`Optional`·`timezone.utc`) 유지분 |
| mypy(`--python-version 3.12`) | 신규 모듈 0 · 순증 1(`multi_db_executor.py` 기존 `result` 이름 재사용 계열 — 1006행 `QueryResult` 바인딩) |
| 무과금 모의 | `--mock --only F-01,F-03,F-04,A-01,B-10,A-02,G-02` → raw `rewrite_trace` None 12/12 · `bottleneck.md` 고정 문구 출력 |

### 12.7 94(→`plans/110`) 시나리오로 검증하는 법 — **실행은 사용자 승인 사항**

| 판정 | 방법 |
|---|---|
| **O-e**(게이트 통과 비율·검증 실패율) | `python -m scripts.scenario --run --arm tier2_intent_frame --arm tier3_intent_frame` → `bottleneck.md` 「재작성 게이트·검증」 절. arm은 `config/scenarios/profiles.yaml`에 추가했다(프롬프트는 바꾸지 않는다) |
| **V28**(W0.5 전후 회귀 0) | `--only F-01,F-03,F-04,A-01,B-10,A-02,G-02` — 전: W0.5 이전 코드 기준선 run · 후: 현 트리. 대응표 단언은 `test_plan94_s19_rewrite.py`가 카탈로그에 고정한다 |
| **Y-11·Y-12** | 단언 기계(`expect.rewrite.{gate,slots_preserved}`)는 랜딩됐다. **카탈로그에 아직 선언하지 않았다** — 기능이 꺼진 run에서 선언된 턴이 합격 대신 보류로 바뀌므로, O-e 섀도 run으로 턴별 게이트 결과를 본 뒤 선언한다 |
| V29 | 비교 키 튜플과 `meta` 기록은 들어갔다 — 사다리 단·arm이 다른 run은 건너뛰고 사유를 `regression.md`에 남긴다. 사람이 기준선을 지정하는 CLI는 미구현(§12.5 9) |
| 폐쇄망 실 실행 | `.env`·`LLM_PROVIDER` 확인 → 무과금 사전 점검(`--preflight`) → 위 명령. 외부 과금 프로바이더면 `RUN_E2E=1` + 건별 승인(D-127) |

### 12.8 등재 후보 (D-번호 미채번 — 사용자 확정 후 `docs/02_decision.md`)

- **W0.5 판정 입력 규칙**: 표면어 결정적 판정은 `surface_query_for_judgment`(원문 · 복합이면 task 스코프)를 쓴다 — `resolved_limit`·`realtime_usage_intent`의 일반화.
- ~~U-10 재개방 여부~~ → **v2.6 처분**: `semantic_compiler`는 편입(수정) · `binding.py:161`은 보류(§12.9 ②).
- **1단 격리 입력의 원문 부재 처리**: 원문이 없으면 task 질의로 판정한다(§12.9 ①) — 원문 전달(`_AMBIENT_KEYS`에 `user_query`)은 다중 호출 번짐 때문에 채택하지 않았다.
- **`rewrite_trace` 운반 계약**: done 페이로드 `rewrite_trace`(목록) + 감사 이벤트 `rewrite_trace` — 하네스 O-e가 읽는 표면.

### 12.9 v2.6 — 권고 결정 반영 (2026-09-21 · 사용자 *"권고에 맞게 진행하라"*)

| # | 결정(권고안) | 조치 | 검증 |
|---|---|---|---|
| ① | **1단 LIMIT·실시간 판정 입력 결함 수정**(안 (C)) — 1단은 격리 입력에 원문이 없어(`deep_agent._AMBIENT_KEYS`에 `user_query` 없음 · `plans/103` N-1) `_make_isolated_input`이 `resolve_query_limit("", 1000)`으로 **LIMIT을 1000에 고정**하고(원문이 "모든"이어도 · 2단은 10000) `is_realtime_usage_query("")`로 **실시간 의도를 항상 끔**(`subagents.py:1215`·`:1244` 게이트 미발동). 승격값이 표면어보다 먼저 적용돼(`query_gen_common.py:562-564`) task 질의에 "모든"이 남아도 1000이었다 | `subagents._make_isolated_input` — 판정 입력을 `state.get("user_query") or task["sub_query"]`로. **2단은 원문이 있어 종전과 같다.** 원문 전달안(`_AMBIENT_KEYS`에 추가)은 1단 다중 호출에서 "상위 N"이 형제 task로 번져 **채택하지 않았다** | `tests/test_orchestration/test_plan107_tier1_judged_query.py` 6건(1단 "모든"→10000 · 평문→기본 · "상위 5"→5 · 실시간 → True · 2단 원문 기준 유지 · 승격값 우선) |
| ② | **U-10 부분 재개방** — `semantic_compiler.compile_smq`가 IR 부재 시 LIMIT 표면어를 `surface_query`(원문 · 복합이면 task 스코프)로 해석 | `semantic_compiler.py` 호출 1곳 + 독스트링 2곳. 1단은 원문이 없어 효과 0(①과 같은 이유) · 3단은 `user_query`가 원문이라 무변화 → **효과는 2단 단일 task** | `test_plan107_w05_surface_judgment.py` `TestLimitReadsSurfaceQuery` 2건(재작성문의 "서버별" 추가가 LIMIT을 올리지 않음 · 재작성문이 "모든"을 떨어뜨려도 원문 기준 상향 유지) |
| ②′ | `binding.py:161`(빈 `query` 폴백) **보류** | 단계적 컬럼 도출(`TEXT2SQL_STEPWISE_DERIVATION`)이 기본 off·운영 `.env` 미기재라 실효 0. 켜는 시점에 같은 규칙(`surface_query`)으로 편입한다 | — |
| ③ | **W0.5 유지 · G-9 근거 정정**(§11 G-9 행 말미) | 코드 변경 없음 | — |
| ④ | **복합 계획 확장은 `plans/111`로** — 이 계획이 병기에서 복합을 제외한 자리를 task 프레임(원문 조각)으로 채운다(111 C-1~C-3 랜딩 · `COMPOSITE_TASK_FRAME_ENABLED` 기본 off) | 111 §7.1 | 111 테스트 22건 |
| ⑤ | **로컬 MLX 효과 측정**(사용자 지시 *"mlx를 이용하여 2,3단의 효과를 측정해보고 적용하라"*) — `eval_text2sql` 43건(`testdata/text2sql_gold_sandbox/`) × {2단·3단} × {섀도·병기(`query_generator`)} | 하네스 보강: `scripts/eval_text2sql.py`가 2단 `task_results`에서 SQL을 뽑고, 실행별 고유 스레드로 `rewrite_trace`를 수거해 JSON에 싣는다 | **진행 중** — 결과는 이 절과 111 §2.7에 적는다 |
...
| 2026-09-21 | **v2.4** | **94 계열 통합에 따른 참조 갱신**(사용자 지시 *"config간소화 계획 1개, 시나리오 테스트 1개로 통합 정리하고 기존 계획은 확인하여 종료처리하라"* — 94·96·98·99·108 → `plans/110`). 이 계획이 하네스에 요구한 항목의 **소유처만** 94 §19 → `plans/110` §3.1로 바꿨다(헤더 가족 줄 · §5 W0·W0.5·W3·W5 · §6 94·98·99 행 · §11 G-8·G-5). **결정·범위·게이트(G-9 ㉮ · W0.5 범위 · U-10 · 108 G-2 비대칭 사유)는 무변경** · v2.1~v2.3 이력 행은 과거 기록이라 그대로 둔다 · 신규 D-번호 0건 |
| 2026-09-21 | **v2.6** | **권고 결정 반영**(사용자 *"권고에 맞게 진행하라"*) — ①1단 판정 입력 결함 수정(LIMIT 1000 고정·실시간 상시 off → task 질의로 판정) ②U-10 부분 재개방(`semantic_compiler` 수정 · `binding.py` 보류) ③G-9 근거 정정(W0.5 실효 = 2단 단일 task · 1단 무작동) ④복합 확장 → `plans/111` ⑤로컬 MLX 측정 착수 · §12.9 · 신규 D-번호 0 |
| 2026-09-21 | **v2.5** | **구현**(사용자 지시 *"107번 계획을 구현하여 94번 시나리오 테스트시 검증할 수 있도록 하라."*). G-1 (b)까지. ①**W-1** — `user_query` 23파일 원문/재작성문 분류 · R6 소비처 전수 · 판정 보류 2종 확정(전체스코프=오염 · 지시어=CU-15 앵커) · **실 run `20260918-182507` 체크포인트 디코드로 발현 실측**(518쌍 · 재작성 49%) ②**W0.5** — `surface_query_for_judgment`(원문 · 복합이면 task 스코프)로 순위·최상급·급증 5종·**전체스코프(추가)** 승격, 단일/멀티 대칭 ③**W0** 골든셋 12건 ④**W1·W2** 프레임·렌더·`rewrite_trace`(감사 + done) ⑤**W5 섀도** ⑥**W3 메커니즘**(기본 off) ⑦**U-10 확대 방향 성립** → 재개방 사용자 확정 ⑧94 §19 하네스(Y-11·Y-12·O-e·V28~V31 · V29 비교 키 튜플·기준선 기록) — 기록은 `plans/110` §3.1 ⑨이탈 9건(§12.5) · 등재 후보 3건(§12.8) · 신규 D-번호 0 · 파일명 TODO→WIP |
