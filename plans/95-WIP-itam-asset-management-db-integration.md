...
> **v14**(2026-09-21): 사용자 지시 *"중단된 작업을 재개하라."* — **W-10 구현**(`plans/102` X-7 랜딩으로 차단 해소). 존 선택 재개 턴이 대상을 고정할 때
> **존 그룹 없는 활성 DB는 선택으로 좁혀지는 대상이 아니므로 분류 결과에 한해 보존**한다(`semantic_router._keep_zoneless_targets`). 후보가 없으면 분류 호출 0·반환 동일
> **성격**: 구현 계획 · **상태: 부분 구현 — 트랙 A0(W-1~W-3)·W-4 소스 정의·W-5 엔진 값·트랙 S(W-S1~W-S3)·W-8·W-9·W-10·W-12(로컬)·W-12a·W-13 랜딩, 잔여는 **W-4 운영 연결(G-3)과 운영 활성화(`plans/104` 준비도 C1~C7)뿐** — **v13: W-11 종결(G-9 노출 확정)** · **v12: W-5 `db_schema`·W-6·W-7과 G-4·G-5·G-13 조사는 `plans/104`로 이관**, 운영 활성화는 104 준비도 C1~C7 충족 후(§5.1·§5.2·§8)**
...
| **W-10** | ✅ 완료(v14) | `src/routing/semantic_router.py` — `_keep_zoneless_targets`(신규) · `_router_prompt_context`(본 분류와 재료 단일화) · 우선순위 2.5 블록 · `src/routing/db_scope.py` `zone_selection_db_ids`(신규 공용 헬퍼) · `src/orchestration/subagents.py`(격리 입력 키 + task 고정) · 테스트 19건(`tests/test_semantic_routing/test_zone_selection_keep_zoneless.py` 9 · `tests/test_orchestration/test_zone_selection_task_targets.py` 10) | **v14 구현**: 존 선택·범위 선택의 선택지는 **존 그룹**으로 만들어지므로(`ZONE_CLARIFY_OPTIONS` · `partition_execution_groups`) 존 그룹 없는 DB는 애초에 고를 수 없는데, 재개 턴이 `selected_db_ids`로 대상을 통째로 고정해 그 DB가 사유 없이 빠졌다. 이제 **후보(= 존 그룹 없는 활성 DB 중 선택에 없는 것)가 있을 때만** 원문을 1회 분류해 **후보에 한해** 보존한다 — 선택한 존은 그대로 고정(UI 선택 우선 불변) · `MIN_RELEVANCE_SCORE` 동일 적용 · 분류 실패는 선택 존만 + WARNING(침묵 금지). 판정은 `registry.zone_group_of(db_id) is None`(W-9와 같은 기준). **역질문 발동 조건은 바꾸지 않았다**(3단 게이트 `semantic_router.py`는 "대상이 전부 폴스타일 때만" 그대로) — 실제로 혼합 질의에 역질문을 띄우는 것은 질의 텍스트만 보는 라우트 선게이트(`src/api/routes/query.py` `_zone_clarification_or_none`)다. **비트 동일**: 라우팅 산출물 매트릭스 633건(활성 5종 전 부분집합 × 선택 × 분류 변형 3)을 HEAD worktree와 대조 — **존 미배정 후보가 없는 300건은 산출물·분류 호출 수까지 전건 동일**(현 운영 `b0·gp·yd` 포함), 변화 222건은 전부 후보를 분류가 고른 경우이고 선택 존은 순서·내용 그대로, 추가분은 전부 존 미배정 DB다. 분류 호출은 후보가 있는 333건에서만 1회 늘어난다(후보 없는 조합은 0 유지). **플래그 없음**(변화 조합 = 결함 조합 — W-9와 같은 근거). **검증**: 신규 19건 통과(라우터 9 · task 고정 10) · 라우터 테스트를 HEAD에 얹으면 **3건 실패**(보존·dict 형태·실패 경고) / 4건 통과 · task 고정 테스트는 HEAD에서 수집 불가(헬퍼 부재)라 별도 프로브로 전후 대조(위) · `tests/test_semantic_routing`·`test_nodes`·`test_orchestration`·`test_routing_eval` 1,593 passed(1 skipped) · arch/overfit exit 0 · ruff 11=11 · mypy 13=13(신규 0) · **실 LLM 1건(mlx `Qwen3.5-9B-OptiQ-4bit` · 두 평면 루프백 · 과금 0)**: *"전체 서버의 유지보수 계약 만료일과 CPU 사용률을 알려줘"* 재개 턴(선택=`polestar`)에서 `[polestar(선택), itam(보존)]` — 선택 없는 참조 실행은 `[itam, polestar]`(17.3초/3.8초). **task 고정도 같이 고쳤다(팀 리드 요구 (2) 실측 결과)**: ~~2단 비대칭~~ — 3단이 순차 러너로 2단 부품을 재사용하므로 **3단 결함**이었다. 근거: `src/graph.py:162,180`(`sequential_entry` → `sequential_runner`) · 진입 플래그 `composite.sequential_fallback_tiers_enabled` 기본 True(`src/config.py:1130`) · SQL 승인 기본 off · `src/orchestration/sequential_runner.py:28,68`(`_llm_decompose` → `agent_orchestrator`) → `subagents.run_data_query_pipeline`의 task 고정. **수정**: 격리 입력에 라우터 산출물을 접어 싣고(`_make_isolated_input`의 `zone_selection_db_ids` 키 — `target_databases`는 task 격리 규약상 비우므로 별도 키) task 고정이 그 값을 쓴다(`subagents.py:1124`). 판정은 공용 헬퍼 `src/routing/db_scope.py` `zone_selection_db_ids`(선택 존 + 존 그룹 없는 라우터 대상 · **LLM 0회 — 같은 턴 라우터 결정을 결정적으로 재사용**). 실측(같은 입력): HEAD `['polestar_b0']` → 수정 후 `['polestar_b0','itam']`. **3단 순차 러너 경로 자체로도 재현**(순차 표지 질의 + 존 선택 재개 턴 → `sequential_runner` → `agent_orchestrator` → task 고정): HEAD `[['polestar_b0']]` → 수정 후 `[['polestar_b0','itam']]` · 후보 없는 조합(`선택=b0,gp`)은 양 트리 동일. 테스트 10건(`tests/test_orchestration/test_zone_selection_task_targets.py` — 헬퍼 4 · 파이프라인 2 · 격리 배선 1 · 3단 재사용 근거 1 · **순차 러너 2**). 2단 전용 진입(`intent_planner.py:321-328`)은 그대로 둔다(배선 미변경 지시). **(3) 직전 턴 결정 재사용은 불가 — 실측**: 재개 턴 초기 상태가 `target_databases=[]`로 덮고(`src/state.py:481`), 결함 경로의 역질문은 라우팅 전에 질의 텍스트만 보고 뜨며(`src/api/routes/query.py` `_zone_clarification_or_none`), 3단 라우터 게이트는 대상이 전부 폴스타일 때만 떠서 존 미배정 DB를 애초에 담지 않는다 → 라우터는 원문 1회 분류. **(4) 변화는 itam 전용이 아니라 「존 그룹 없는 활성 DB 일반」이다**: 존이 달린 DB는 `polestar_b0`(`config/db_registry.yaml:156`)·`polestar_cm_gp`(`:181`)·`polestar_cm_yd`(`:201`) 셋뿐이고 `polestar`(`:225`)·`cloud_portal`(`:244`)·`itsm`(`:259`)·`itam`(`:279`)은 존이 없다 — 매트릭스 실측에서 **`ACTIVE_DB_IDS=polestar` 단독은 분류 호출이 늘지 않는다**(3케이스 전건 호출 0 · 반환 동일 — 선택이 곧 그 DB라 후보가 비고, 활성 밖 선택은 `semantic_router.py:160` 활성 필터에서 떨어져 고정이 발동하지 않으며, 역질문 선택지 `b0·gp·yd`가 비활성이면 역질문 자체가 뜨지 않는다), `polestar,itam`은 선택이 `polestar`면 itam이·선택이 `itam`이면 `polestar`가 분류 결과에 한해 남고(각 1회), 둘 다 고르면 종전과 같다. **(1) 무플래그 성립 조건 2건을 테스트로 고정**: 후보 없으면 분류 0회 + **반환 dict 전체(키·값) 동일**, 분류 실패는 선택 존만 + WARNING 1줄. **후속 의존(2026-09-21 · 인가 축)**: 보존 경로의 후보 산출(`src/routing/semantic_router.py:532-535`)과 task 고정 헬퍼(`src/routing/db_scope.py` `zone_selection_db_ids` · 소비 `src/orchestration/subagents.py:814,1126`)는 **`active_db_ids`만 보고 인가 축(`allowed_db_ids`)을 보지 않는다** — 현재 라우터에 그 축의 소비처가 0건이라(`semantic_router.py` grep 0 · 소비처는 `state.py:256` · `nodes/general_inference.py:98` · `nodes/fault_diagnosis.py:124` · `orchestration/subagents.py:798` · `orchestration/entity_locator.py:589` · `orchestration/deep_agent.py:146` · 범위 축 UI는 `db_scope.py:175`) 지금은 무해하다. **`plans/104` C-4(D-232 · `AUTH_DEFAULT_ALLOWED_DB_IDS` 범위 확장)가 라우터 대상 결정에 인가를 강제하면 같은 교집합을 이 두 지점에도 적용해야 한다** — 미이행 시 **비허용 존 미배정 DB가 존 선택 재개 턴에 대상으로 다시 실린다**(`[]`의 의미 통일도 함께 — 현재 `general_inference.py:98`은 빈 목록을 전체 허용으로 읽는다). 소유는 104 C-4이고 `collectorinfra-94` 세션이 수행한다(편집 전 통지 확답) — 95는 아직 없는 축을 앞질러 구현하지 않는다. **아래는 v8·v9 이력** — ~~미착수 — 사용자 결정~~ 혼합 질의의 존 역질문 기대 동작에 **기본 가정이 없다**(§8 G-7 기본값은 T2만 규정). 현 운영은 itam 비활성이라 혼합 대상이 생기지 않는다. **v8 재실측**: 비발동 조건은 사본 2곳이다 — 2단 `src/orchestration/subagents.py:282-284`(§0.2 ⑤ 인용 `:264-268`에서 이동) · **3단 `src/routing/semantic_router.py:442-444`**(v7 기준 경로). 발동으로 바꾸면 끝나지 않는다 — 선택지가 폴스타 존뿐(`ZONE_CLARIFY_OPTIONS`)이고 재개 턴은 `selected_db_ids`로 대상을 **고정**하므로(`semantic_router.py:136-158`) 재개 턴에서 itam이 빠진다. 선택지는 최종 보고로 사용자에게 올림. **v9 — 구현 보류 · 요건 확정**: `plans/102` X-7(소유 라우팅) 이후로 둔다(권고 (c)). 사용자 사실(자산관리는 존 없음·단일 시스템)에서 나오는 요건 — *존 역질문이 발동하더라도 itam은 존 선택과 무관하게 대상에 남아야 한다*(재개 턴 `selected_db_ids` 고정에서 itam 탈락 금지). `plans/102` W-9/W-10 행에 같은 요건 기록 · **선행 충족(2026-09-17 · `plans/102` v4)**: X-7(답변 영역 소유 라우팅)이 플래그 `ROUTER_CAPABILITY_OWNERSHIP_ENABLED` 뒤로 기본 off 랜딩됐다 → **W-10 재개 가능**. 102 구현 중 확인된 결함 — 분해 task의 소유 고정은 `selected_db_ids`가 없을 때만 적용돼, 존 선택 재개 턴의 자산 task가 폴스타로 간다(102 §4.1.6) — 이 W-10 요건과 같은 결함이다 |
...
| **v14** | 2026-09-21 | **W-10 구현 — 존 선택 재개 턴에서 존 미배정 DB 보존**(사용자 지시 *"중단된 작업을 재개하라."* · `plans/102` X-7 랜딩으로 차단 해소). `semantic_router`의 존 선택 고정 블록에 보존 판정(`_keep_zoneless_targets`)을 더했다 — 후보(존 그룹 없는 활성 DB 중 선택에 없는 것)가 있을 때만 원문 1회 분류 → 후보만 보존, 선택 존은 그대로 고정, 분류 실패는 WARNING + 선택 존만. 본 분류와 프롬프트 재료를 `_router_prompt_context`로 단일화. 역질문 발동 조건·2단 배선은 미변경(2단 비대칭은 §5.1에 기록). 비트 동일: 매트릭스 633건 중 후보 없는 300건 전건 동일(분류 호출 0 유지) · 변화 222건은 전부 결함 조합. **추가 실측 3건(팀 리드 요구)**: ①3단이 순차 러너로 2단 부품을 재사용해 task 고정에 닿으므로(그래프·플래그 근거 §5.1) "2단 비대칭"이 아니라 3단 결함 — 같은 축으로 `zone_selection_db_ids`(`db_scope`)와 격리 입력 키로 함께 고쳤다(HEAD `['polestar_b0']` → `['polestar_b0','itam']`) ②직전 턴 라우터 결정 재사용은 불가(초기 상태가 `target_databases`를 덮고, 역질문이 라우팅 전에 뜬다) → 라우터는 원문 1회 분류, 같은 턴 task 고정은 그 결정을 LLM 0회로 재사용 ③로컬 샌드박스 `polestar`도 존 미배정이라 변화는 itam 전용이 아니다(`ACTIVE_DB_IDS=polestar` 단독은 후보 0 · 분류 0회 · 반환 동일). 검증: 신규 테스트 19건(순차 러너 재현 2건 포함) · 관련 스위트 1,593 passed · arch/overfit 0 · ruff·mypy 신규 0 · mlx 실 LLM 1건. W-14 문서분은 v12에서 이미 종결(`CLAUDE.md:183` 실측 확인). 남은 것: **W-4 운영 연결(G-3) · 운영 활성화(`plans/104` 준비도 C1~C7)** |
