...
> **상태**: **완결(시점 문서) — 2026-09-09 잔여 `plans/91-TODO-fault-investigation-residual-consolidation.md`로 이관(1-6 E8 post-gate · 1-10 C3 택1 · 1-12 R10 · 1-13 R12 · 1-14 §7-1 · 1-15 DB2 · 1-16 P0-1 · D-208) · `-WIP` 해제.** 잔여 장부 역할은 91이 승계하며 §1.2~§1.6은 시점 기록으로 보존한다. *직전 표기*: **Phase 1~3 구현 완료 — MVP 실증**(2026-07-28 실 Gemini 조사 완주). Phase 4~5·잔여 항목은 §1.4 스냅샷 참조. **2026-08-25 재점검: Plan 66 스코프 코드 진척 0·회귀 0 — 잔여 5건은 전부 코드 외 선행조건 대기**(§1.5). **⚠ 2026-08-31 갱신(D-189): R11(E8)의 P0-4(벤더 협의) 블로커가 해소돼 잔여 5건 중 1건이 착수 가능해졌다** — L3 접근 경로가 B(허용목록 명령 실행)로 확정됐다(§1.5 ③). 착수 게이트는 §7의 사용자·행정 확인 항목. **MVP 재현 절차: `docs/23_plan66_mvp_test_guide.md`**. **2026-09-09 계획군 교차 정리(§1.6 · `plans/50` §0.8 · D-207)**: 실 조사 e2e와 E8은 본 계획 소유로 확정, RCA 축 잔여는 50 Phase C′ 소관.
...
### 1.6 계획군 교차 정리 (2026-09-09 · `plans/50` §0.8 · D-207)

사용자 지시(*"50, 64, 66번 등 유사한 계획을 종합적으로 분석하여 계획을 정리하라"*)로 장애 조사 계획군의 잔여를 **단일 소유**로 재배치했다. 본 계획에 영향이 있는 것만 적는다 — 정본은 `plans/50` §0.8.

| 항목 | 본 계획에서의 위치 | 정리 결과 |
|---|---|---|
| **실 조사 e2e**(R9 실 완주 · `docs/23` §7 레벨 B) | R9 | **소유 = 본 계획(`docs/23`)**. 50은 C′-0(`evidence_correlation_enabled=True` 경로의 `root_cause_hypotheses`·상대시각 타임라인 단언)만 보탠다 — 현 `test_investigation_e2e.py`는 완주·도구 호출·토큰만 단언한다(실측). 실행은 D-127 건별 승인 |
| **E8 L3 게이트 배선**(R11 · 4-B) | Phase 4 | 60 §18·64 §4.8 L3·51 L3·85 티어 3.5가 **전부 이 한 항목**을 가리킨다 — **소유 = 본 계획 4-B**(설계 정본 60 §18). 착수 게이트는 60 §18.3 (a) 지연 예산 **택일**(가: 경량 동기 MCP 도구 / 나: (a) 폐기·(b) post-gate만) — 50 §0.8.4 **U-H → (나) 확정(2026-09-09 사용자)**: (a) 게이트 동기 probe 폐기, **(b) post-gate 비차단 보강만** 구현. 4-B의 "게이트 동기 경계 probe(kind별 USE·≤2s·캐시)" 부분은 범위에서 빠진다(60 §18.3·§18.6 · D-117 ② 개정). `host_diagnostic_collector`·`polestar_host_snapshot` grep 0건(미착수 재확인) |
| **RCA 축 잔여**(Plan 50 Phase C′) | 본 계획 범위 밖(Plan 50 소관) | 50 §0.8.3이 C′-0~C′-4로 구체화. 전부 `sre_agent`/`mcp_server` 내부 증분이라 본 계획의 Phase 배선(3-A~3-E)은 **수정 없음** |
| **Plan 64 상태** | R8(CW-A~C) | 64 헤더가 "계획 (미구현)"이던 것을 **구현 완료(위임) · 코드 잔여 0**으로 재판정(64 §15). B-3 자동 조치 거버넌스는 64 관할·착수 금지 유지(본 계획 §3 「착수 금지」와 동일) |
| **플레이북 편입**(85 티어 1) | — (Plan 51 소관) | 50 G5(D-197)가 편입점(`AgentSettings.investigation_guidance_extra`)만 만들었고 내용은 미편입 — 소유 51, 본 계획과 병렬 가능 |
| **R12 `polestar_host_snapshot`** | 4-C | 변동 없음 — R11 후 결정 |

> 본 절은 §1.2·§1.4·§1.5의 상태를 바꾸지 않는다(R10·R12·R14·R15·⑤·⑥ 그대로). 바뀐 것은 *어느 계획서가 어느 잔여를 적는가*뿐이다.
...
| 4-B | **R11 E8**: ~~폴스타 에이전트 스냅샷 채널 어댑터~~ **허용목록 명령 실행 채널**(D-189 — `noise_gate/infrastructure/host_diagnostic_collector.py`, 게이트 자산 D-139 배치) + ~~게이트 동기 경계 probe(kind별 USE·≤2s·캐시)~~ **폐기(2026-09-09 U-H (나) 확정 · §1.6)** + post-gate 결정적 요지 첨부(sre_agent 미가용 시 폴백) + 측정 기반 dedup 상태지문 | Plan 60 §18 | M (~~P0-4 선행~~ 해소 D-189 · 착수 가능) |
...
| 날짜 | 변경 | 사유 |
|---|---|---|
| 2026-09-09 | **U-H 확정 반영** — E8 실행 모델 (나): 게이트 동기 probe 폐기·post-gate 비차단만(4-B 행 정정 · §1.6) | 사용자 확정(*"권고에 맞게 수정하라"*) · D-117 ② 개정(D-207 후속) |
| 2026-09-09 | **§1.6 계획군 교차 정리 신설**(`plans/50` §0.8 · D-207) — e2e 소유=본 계획(`docs/23`), E8=본 계획 4-B(다섯 계획이 가리키는 단일 항목 · 착수 게이트 U-H 지연 예산 택일), RCA 축 C′는 50 소관, 64 상태 재판정 반영 | 사용자 지시(*"50, 64, 66번 등 유사한 계획을 종합 분석하여 정리"*). §1.2~§1.5 상태 불변 — 잔여의 기재 위치만 정리 |
