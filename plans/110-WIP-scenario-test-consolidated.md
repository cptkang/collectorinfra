...
>
> **상태**: 부분 구현 · 잔여 있음 — 파일명 `-WIP`. 하네스 S0~S4·§15~§18·Y-10과 제품 수정 19건이 랜딩했다. **2026-09-21 `110·N-1`(`--arm`)이 랜딩해 2단·3단 arm 병행 실행의 코드 차단은 풀렸다.** 그러나 **판정 자격을 갖춘 기준선 run은 아직 0건이다**(§0.3) — 남은 것은 실행과 사람 판단이다(§4 A·C). **2026-09-23: 기준 운영 단 = 2단(D-251) · Q-1·G-6 종결 · 다음은 부록 B 기준선 run → CU-4·CU-5 → 재측정(§4 C′).**
>
...
>
> **관련 결정**: D-003 · D-127 · D-208 · D-212(예약만 되어 있고 본문 미등재 — §3.1 `94·S7-b`) · D-215 · D-216 · D-217 · D-218 · D-220 · D-221 · D-222 · D-225(→**D-251로 전면 개정** — 기준 운영 단 2단) · D-231 · D-236 · **D-251**
>
...
- **arm은 한 run에 담지 않는다**(2026-09-21 실측 · run `20260921-175929`): arm 2종 = **서버 기동 4회**인데, MLX 프롬프트 캐시는 앱 서버 밖이라 프로파일이 바뀌어도 비워지지 않는다. 뒤에 도는 arm일수록 실패율이 올라가 (성공 5/8 → 1/8 → 0/1 → 0/1) **단 차이와 자원 고갈이 구별되지 않는다.** `--arm` 하나씩 따로 돌리고, 그래도 지연 결론은 내지 않는다(D-240 ④).
...
- **무효 판정**:
  ...
  - 운영자 선택으로 비기준 단이 된 경우는 리포트 상단에 경고만 남는다(D-221 ⑤). **기준 단은 2단 `intent_orchestration`이다(D-251 · 2026-09-23 — D-225의 3단 기준을 개정)** — 3단 arm(`semantic_router` · 사유 `none`)과 4단(`semantic_routing_off`)이 경고 대상이다.
...
**하네스는 이제 제품을 잴 수 있지만, 아직 판정 자격을 갖춘 기준선 run이 없다.**

> **2026-09-23 갱신(D-251)**: 기준 운영 단은 **2단 `intent_orchestration`**이고 3단은 2단 대비 비교 arm이다. 아래 원문의 "기준 경로(3단)"·"재서 고른다"는 D-225 시점 서술이다. 폐쇄망 run은 4건(§0.3)이고 여전히 전부 판정 자격이 없다. 다음 할 일은 부록 B의 기준선 run → C′(CU-4·CU-5) → 재측정이다.
...
| `20260922-093837` | Windows `GE-IT19Y040` · fabrix 워커 + vllm `Qwen3.5-9B` 오케스트레이터 · `868c7f6`(**저장소에 없음**) dirty | 220건 / 388턴 | **2단 `intent_flag_on`**(arm 없음 · 3단 미측정) · 인증 on | **88턴(22.7%)이 120초 타임아웃** — 사망 위치 `result_aggregator` 50 · 2차 `agent_orchestrator`(재계획 후속) 30 · 1차 `agent_orchestrator` 8. 계약 적용 기능 합격률 19.3%(58/300). 완주 턴 서술 노드 지연은 **응답 길이와 r=0.912**(24.2ms/자 · 500자 미만 p50 7.1s 대 1,500~3,000자 48.0s). 재계획 후속 실행 28/361턴(7.8%) 중 21턴 타임아웃·합격 3 | 없음(2단 단독 · dirty · 커밋 부재 · 반복 1) | 이 문서 §3.3 CU-4·CU-5 · §7 2026-09-23 |
...
- ~~**2단 전용 결함 처분 규율**(108 G-2): 기준 경로는 3단이다(D-225). 2단에만 있는 결함에는 투자하지 않는다(D-161). 단 **1단에서도 발현하는 결함에는 이 선례가 전이되지 않는다**(107 G-9).~~ **폐기(2026-09-23 · D-251 ③)** — 기준 운영 단이 2단이므로 2단 결함이 곧 기준 경로 결함이다. 3단은 2단 대비 비교 arm이고, 3단에만 있는 결함은 `plans/103`(기능 동등성)이 소유한다.
...
| **`108·§5.5`** | 표적 재현 7건(가이드 ④ 3.5단) | 108 §5.5 | `--only` + `--arm` | 가이드 ④ 기대표 | ~~`110·N-1`~~ 해소 | **1차 실행(2026-09-21 · run `20260921-175929` · 로컬 MLX 9B · env=sandbox · 14건/18턴)** — **검증됨 2건**: ①`CU-A1` H-03 산출 Excel이 올바른 시트 `리소스상태`에 **54행**(종전 결함은 헤더만 1행) ②`CU-H1` B-07 SQL에 `DECIMAL` 10건·`AS INT` 0건(D-199 가드가 NUMERIC→DECIMAL 치환 → 구 단언 `\bnumeric\b`가 구조적 충족 불가였던 것이 풀림). **판정 불가 3건**: `CU-B2`(R3-03 — 샌드박스에 존 보유 DB가 0이라 `build_zone_clarification`이 `None`을 반환해 게이트 **비발동**. 탐지 함수 `find_unregistered_zone_terms('판교존…')→['판교존']`은 정상 · 네 진입 배선도 대칭 실측 → **폐쇄망에서만 판정된다**) · `D-04`(양 arm timeout) · `R3-03C`(양 arm timeout). ⛔ **2단·3단 비교는 성립하지 않았다** — 실패가 실행 순서대로 단조 증가했다(`baseline+tier2` 성공 5/8 → `baseline+tier3` 1/8 → `optin_alarm+tier2` 0/1 → `optin_alarm+tier3` 0/1). 대부분 `input_parser`에서 1~10초 만에 죽어 진짜 지연이 아니다. 단일 공유 MLX 서버가 **서버 기동 4회 연속 부하**를 못 버틴 것이다(장비 여유 메모리 77MB 실측). **MLX로는 arm을 한 run에 담지 말고 arm마다 따로 돌린다**(⑩) | **부분 실행 · 재실행 필요** |
...
| **`98·CU-4`** ≡ `96·S-b` · **`98·CU-5`** ≡ `96·S-a` | `replanner` 결정적 조기 종료 · 서술 비용(`result_aggregator` 안의 `output_generator._generate_text_response` — **2·3단 공통**). ~~둘 다 2단 전용 노드다~~(2026-09-23 정정 — CU-5 비용 본체는 3단 `output_generator` 노드도 부른다 · `src/graph.py:687`) | 98 §3 · 96 §5 · **D-251 ④⑤** | `replanner.py:69-94` · `output_generator.py:241-319`·`:475-586` | 노드 p50 · 발동률 · `bottleneck.md` 「응답 서술 비용」·「재계획 후속 실행」 | ~~`110·Q-1`~~ · ~~`98·G-6`~~ **둘 다 종결(D-251)** · **`108·재테스트` 기준선 run 선행**(D-251 ⑦ · §2.3) | **착수 대상 — 기준선 뒤 랜딩.** 확정 사양: ①CU-5 = G-6 (c) — 표가 있는 조회 응답 전체(폼필 미리보기 표 포함)의 표는 코드가 결정적으로 렌더, LLM은 2~3줄 요약만(D-187 ② 번복) ②CU-4 = 전 task 성공·행 반환·미완 표지 없음이면 LLM 없이 `needs_replan=False`, 0건·부분 실패·순차 의존(D-203)은 LLM 평가 유지 ③신규 플래그 없이 기본 동작(D-162) ④재측정은 같은 arm 2종. 선행 측정(2026-09-23): 서술 지연 대 응답 길이 r=0.912(20260918 r=0.935) · 재계획 후속 실행 7.8%(28/361턴) 중 타임아웃 21·합격 3 · LLM 후속 제안 후 D-063 전부 폐기 20회 |
...
| 키 | 물음 | 입력·선행 | 현황 · 권고 |
|---|---|---|---|
| ~~**`110·Q-1`**~~ **종결(2026-09-23 · D-251)** | 98 CU-4·CU-5에 108 G-2 규율(2단 전용 결함 보류)을 적용하는가 | 사용자 인터뷰: *"2단은 기본 운영 단으로 보고 3단은 비교하여 2단 대비 성능을 확인할 수 있도록 한다."* | **적용하지 않는다** — 기준 운영 단이 2단으로 바뀌어 108 G-2 규율 자체가 폐기됐다(§2.4). CU-4·CU-5는 착수 대상(§3.3) · `98·G-6`도 (c)로 종결 |
...
```
A. 지금 · 사람 판단 ─ ~~108·G-6(판정 계약)~~ **승인·랜딩(2026-09-21)** · 99·H-1(커밋 범위) · ~~110·Q-1~~·~~98·G-6~~ **종결(2026-09-23 · D-251)**
      │
...
C. 폐쇄망 실행 ─ --preflight(E-0·E-1) → 표적 재현(3.5단 · J-1 · J-2 · arm 2종)
      ...
      │   → ~~단 선택 판단~~(**D-251: 기준 운영 단 = 2단 · 3단은 비교 arm**) → 98 §7·§8.5 효과 측정
      │   · 구동 안내서: **부록 B**
      │
C′. 기준선 직후 · 제품 코드(D-251 ⑦) ─ **CU-4(replanner 결정적 조기 종료) · CU-5(G-6 (c) 표 결정적 렌더 + LLM 2~3줄 요약)**
      │   → 같은 arm 2종으로 재측정(C와 같은 명령) → 기준선 대비 판정
      │
...
- **C의 첫 run이 단별 기준선이 된다.** 이전 run 3건과는 직접 비교하지 않는다(§0.3). ~~운영 단 선택(`plans/102` L-5)은 이 비교가 나온 뒤에 사용자가 정한다.~~ 운영 단은 **2단으로 확정**됐다(D-251 ① · 2026-09-23). 3단 arm은 2단 대비 성능을 재는 비교 축이다.
- **C′는 C 뒤에만 간다**(§2.3 · D-251 ⑦). 기준선 run 전에 CU-4·CU-5를 넣으면 효과를 귀속할 수 없다.
...
- 이 문서가 새로 올리는 물음은 `110·Q-1` 하나다(§3.4). ~~확정되면 그때 채번 규칙에 따라 등재한다.~~ **2026-09-23 사용자 인터뷰로 확정 → D-251 등재**(98 G-6·108 G-2 처분 · D-225 전면 개정 포함).

## 7. 변경 이력

| 날짜 | 내용 |
|---|---|
| 2026-09-23 | **D-251 등재 — 사용자 인터뷰 결과 반영**(원문 *"2단은 기본 운영 단으로 보고 3단은 비교하여 2단 대비 성능을 확인할 수 있도록 한다."*). `110·Q-1`·`98·G-6` 종결, §2.4 2단 전용 결함 규율 폐기(108 G-2), CU-4·CU-5를 착수 대상으로(사양 확정 · 기준선 뒤 랜딩), §4에 C′ 단계(기준선 → CU-4·CU-5 → 재측정) 신설, **부록 B 기준선 구동 안내서** 신설. D-225 개정의 코드 변경(기본값·`ladder.py`·preflight/report)은 arm 프로파일이 사다리 플래그 3종을 명시해 기준선 측정값에 영향이 없어 즉시 랜딩했다(D-251 「(c) 판정」). |
| 2026-09-23 | **run `20260922-093837` 분석 · 하네스 대리 지표 2종 랜딩**(요청 "110번 구동 결과를 분석하여 성능을 개선하라"). 출처 4종: run id `20260922-093837` · 커밋 `868c7f6`(저장소에 없음 · dirty) · **2단**(arm 없음) · 인증 on — 기준선 자격 없음(§0.3 행 추가). **제품 코드는 고치지 않았다** — 병목 1·2위(서술 비용 CU-5 · 재계획 CU-4)가 모두 열린 게이트에 걸려 있다: §2.3 재측정 전 생성 경로 불변(108 G-1 (b)) · `98·G-6` 서술 정책(사람) · `110·Q-1` · §2.4 신규 `enable_*` 금지(D-162) · D-187 ② 표 결정적 생성 보류(사용자 확인). 대신 판단 입력을 만들었다: `bottleneck.md`에 「응답 서술 비용(대리 지표)」·「재계획 후속 실행」 절(`scripts/scenario/analyze.py` `narration_cost`·`replan_followup` · 테스트 7건 · D-221 부기). CU-4·CU-5 행과 Q-1에 실측을 적고 Q-1 전제(2단 전용)를 CU-5에 한해 정정했다. 정확도 불합격은 전부 기존 행(CU-6 H-10·H-11·H-15·K-04 · CU-9 H-06·H-12·H-13 · P-8 A-01 · CU-14 R3-03 · CU-B1 E-07·D-03(추정 — 응답이 "알람 데이터는 현재 스키마에 없다"로 시작) · 108 §1.6 상태 잔존 I-03~06과 같은 증상의 I-01·I-02(추정))에 대응했고 신규 결함 행은 없다. |
...
## 부록 B. 기준선 run 구동 안내서 — 폐쇄망 사용자용 *(2026-09-23 · D-251 ⑦)*

> **목적**: 2단(기준 운영 단)과 3단(비교 arm)의 **첫 판정 자격 기준선**을 잰다. 이 run이 끝나야 CU-4·CU-5(§3.3)를 넣고 같은 명령으로 재측정한다(§4 C′). **CU-4·CU-5가 들어간 커밋으로 돌리면 기준선이 아니다.**

### B.1 커밋 상태 — 돌리기 전에 확인

| 조건 | 확인 방법 | 어기면 |
|---|---|---|
| 작업 트리 clean | `git status --short` 출력 0줄 | `run.json` `meta.dirty: true` → 기준선 자격 없음(§0.3) |
| 커밋이 **이 저장소에 있음**(원격에 push된 커밋) | 개발망에서 `git cat-file -t <sha>` = `commit` | run 20260922-093837처럼 `868c7f6`를 추적할 수 없다 |
| **D-251 ①(기준 단 2단) 랜딩분 포함 · CU-4·CU-5 미포함** | `git grep -n "is LadderTier.INTENT_ORCHESTRATION" src/observability/ladder.py` 1건 · `replanner.py`에 결정적 "충분" 조기 종료 없음 | ①이 없으면 리포트 경고 대상이 뒤집혀 읽힌다(측정값은 같다) · CU-4·CU-5가 있으면 기준선이 아니다 |
| 대리 지표 2종 포함 | `git grep -n "def narration_cost" scripts/scenario/analyze.py` 1건 | `bottleneck.md`에 「응답 서술 비용」·「재계획 후속 실행」 절이 없다(`--analyze`로 나중에 다시 낼 수는 있다) |

### B.2 명령 (PowerShell · 저장소 루트)

```powershell
git status --short                      # 0줄이어야 한다
git log -1 --format="%H %s"             # 이 SHA 를 기록한다
# ① 무과금 전개 확인 (서버 없음 · 수 초) — 조합 4개(baseline·optin_alarm × 두 arm)가 보이는가
python -m scripts.scenario --dry-run --arm tier2_intent --arm tier3_router
# ② 사전 점검 — 사다리 단 · 설정 에코 · DB 조회 2건
#    --preflight 는 --arm 을 읽지 않는다: 여기 「사다리 단」은 운영 .env 기준이다. arm 별 단은 ③ 뒤 run.json 에서 본다
python -m scripts.scenario --preflight
# ③ 실 실행 (내부망 두 평면 = fabrix 워커 + vllm 오케스트레이터면 승인 없이 돈다 · D-222)
python -m scripts.scenario --run --arm tier2_intent --arm tier3_router
# 끊기면 같은 run_id 로 이어서 (가이드 ⑤)
python -m scripts.scenario --run --resume <RUN_ID> --arm tier2_intent --arm tier3_router
```

- 두 arm은 사다리 플래그 3종(`ENABLE_DEEPAGENTS_PACKAGE=false` · `ENABLE_INTENT_ORCHESTRATION` · `ENABLE_SEMANTIC_ROUTING`)을 **모두 명시**해 자식 서버 OS env로 주입한다. 그래서 운영 `.env`의 사다리 값·코드 기본값과 무관하게 단이 정해진다(D-251 「(c) 판정」).
- **`.env`를 고쳐 한 단씩 돌리지 않는다**(가이드 ③ — 실행 시각 교란).
- 두 평면 중 하나라도 외부(gemini 등)로 해석되면 멈추고 보고한다(D-127).

### B.3 끝난 뒤 확인할 지표

| # | 지표 | 어디서 | 판정 |
|---|---|---|---|
| 1 | arm별 확정 단 | `run.json` `profiles[].tier` · `degraded_reason` | `…+tier2_intent` = `intent_orchestration`/`intent_flag_on`, `…+tier3_router` = `semantic_router`/`none`. **두 arm이 같은 단이면 arm이 무너진 것 — 무효** |
| 2 | 출처 | `run.json` `meta.commit` · `meta.dirty` · `meta.provenance_mixed` | 커밋이 저장소에 있고 `dirty: false` · 섞임 없음 |
| 3 | **응답 서술 비용** | `bottleneck.md` 「응답 서술 비용 (대리 지표)」 | arm별로 피어슨 r · ms/자 · 길이 구간 중앙값. **3단(`output_generator`)도 r이 높으면 CU-5가 두 단 공통 병목이라는 추정이 실측으로 확정된다**(현재는 코드상 추정) |
| 4 | **재계획 후속 실행** | `bottleneck.md` 「재계획 후속 실행」 | 2단 arm의 후속 실행 비율과 그중 타임아웃·합격 수(20260922: 7.8% · 21 · 3). CU-4 재측정의 비교값 |
| 5 | **타임아웃 노드 분포** | `raw.jsonl` `unevaluated_reason == "timeout"` 행의 `node_path` 끝 노드 | 20260922 2단: `result_aggregator` 50 · 2차 `agent_orchestrator` 30 · 1차 8. 3단 arm의 분포를 함께 적는다 |
| 6 | **`input_parser` p50** | `bottleneck.md` 「기여도 상위 노드」 | 20260922 6,785ms · 20260918 6,931ms(`96·S-d` — 두 단 공통 전단) |
| 7 | **`API_QUERY_TIMEOUT` 실제값** | `run.json` `profiles[].effective_settings.API_QUERY_TIMEOUT` | 20260922은 **120**. 운영 `.env`는 60으로 기록된 적이 있다(`docs/18` 2026-09-23). **재측정 run과 같은 값이어야 비교가 성립한다** — 다르면 타임아웃률 비교는 판정 불가 |
| 8 | 기능 합격률(계약 적용) · 무효율 | `report.md` 2절 · 10절 | arm별로 적는다. 무효율 > 5%면 기준선 자격 없음 |

결과는 §0.3 표에 행을 추가하고(출처 4종 — 장부 규칙 5), `94·V29 기준선`에 run id를 기록한다.

### B.4 운영 `.env` — 1단 off 반영 절차 (사용자 작업 · D-251 ②)

이 절은 기준선 run과 **독립**이다(arm이 사다리 값을 덮으므로 순서는 상관없다). 운영 서버 반영 절차다.

1. 백업: `Copy-Item .env .env.bak-20260923`
2. `.env`에서 다음 세 줄이 **각각 별도 줄**로 있게 한다(인라인 주석 금지 — Known Mistakes):
   ```
   ENABLE_DEEPAGENTS_PACKAGE=false
   ENABLE_INTENT_ORCHESTRATION=true
   ENABLE_SEMANTIC_ROUTING=true
   ```
   세 값을 모두 명시한다 — 코드 기본값이 바뀌어도(D-251: 2단 미입력 = on) 운영 경로가 흔들리지 않게 한다.
3. 서버 재기동.
4. 기동 로그 확인: `오케스트레이션 사다리 확정: tier=intent_orchestration degraded_reason=intent_flag_on resolved_by=explicit_env` 1줄. D-251 ① 랜딩분이 배포돼 있으면 그 뒤에 "기준 경로가 아닌" WARNING이 **없어야** 한다(2단이 기준이다). `tier=deep_agent`가 나오면 2번이 반영되지 않은 것이다.
5. 스모크: 웹에서 단순 질의 1건 → 응답 정상.
6. 되돌리기: `Copy-Item .env.bak-20260923 .env` 후 재기동(1단 복귀).
