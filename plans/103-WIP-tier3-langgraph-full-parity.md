...
> **⚠ 기준 개정(2026-09-23 · D-251)**: 기준 운영 단은 **2단 `intent_orchestration`**으로 바뀌었다(D-225 ① 전면 개정 · 사용자 *"2단은 기본 운영 단으로 보고 3단은 비교하여 2단 대비 성능을 확인할 수 있도록 한다."*). 이 계획(3단 기능 동등성 · D-226)은 **계속 진행**하되 목적은 "3단을 운영 기준으로 올리기"가 아니라 **"3단을 2단 대비 비교 arm으로 쓸 수 있게 하기"**다. 본문의 "3단 기준"·"L-5 운영 전환의 선행" 서술은 D-225 시점 기록이다.
...
| **P0-3** K-3 | ◐ | SSE 두 라우트(`/query/stream` · `/query/file/stream`)가 `parent_ids` 깊이 ≥2(서브그래프 안) 종료를 `done` 판정에서 뺀다(`_is_subgraph_event`) · `_STREAM_KNOWN_NODES`에 루프 노드 7종 · 진행 요약 매핑 | **인터럽트 감지(`awaiting_approval`)·폴백 `ainvoke` 재실행 제거 미착수** — 인터럽트를 내는 경로가 P3(SQL 승인 `interrupt()`)뿐이고, 루프는 SQL 승인이 켜지면 진입하지 않는다. 현행 `interrupt_before` 승인 흐름을 바꾸면 P0 "무변경" 원칙에 걸린다. **1·2·4단 무변경 확인(2026-09-22)**: `final_response`를 내는 노드(`deep_agent`·`result_aggregator`·`output_generator`·`general_inference`·`fault_diagnosis`·`cache_management`·`synonym_registrar`·`approval_gate`)가 전부 **루트 직속**(깊이 1)이라 새 판정에 걸리지 않는다 — 2단 `subagents`는 컴파일 그래프를 `ainvoke`하지 않고 노드 함수를 직접 부르고, 1단 `deep_agent`의 내부 그래프 상태에는 `final_response`가 없다. 깊이 ≥2에서 `final_response`가 나는 것은 3단 `task_run` 서브그래프뿐이고 그것이 이 판정의 대상이다. `tests/test_api/test_stream_subgraph_done.py`(실 LangGraph로 깊이 규칙 고정) 포함 916건 통과. **실 그래프 실측(2026-09-22~23 · 로컬 MLX · `astream_events`)**: 1단 `deep_agent` · 2단 `result_aggregator` · 3단 `output_generator` 모두 `final_response`를 실은 종료가 **루트 직속(깊이 1) 1건**이고 종전 규칙과 같은 자리에서 잡힌다(폴백 `ainvoke` 필요 0). 라우트 실측은 `tests/test_api/test_stream_done_root_only.py` 4건 — 실 LangGraph 이벤트로 두 라우트 × (1단형·계획 루프형)에서 `done` 1회·폴백 호출 0을 고정했고, 판정을 빼면 두 라우트 모두 서브그래프 부분 답으로 닫힌다(회귀 검출 확인) |
...
| **P2-1** | ✅ | 라우터 `needs_plan`(플래그 on일 때만 프롬프트 말미 절 + 구조화 서브클래스 `PlanRouterDecision`·`OwnershipPlanRouterDecision` · `chain` 비면 안 됨 → 계획 필요) · 진입 `plan_loop_entry`(데이터·알람 의도 + `needs_plan` 또는 순차 표지 · SQL 승인·양식 제외) · `plan`(= `_plan_turn`) · **`normalize` 단일 출구**(111 D-1) | 2단 분리 라우터(`ROUTER_TWO_STAGE_ENABLED`) 경로는 `needs_plan`을 내지 않는다 — 순차 표지 OR만 남는다. **신호 재현율 실측(2026-09-23 · 로컬 MLX 9B)**: 복합 질의 2건 중 1건만 `needs_plan=true`였다(*"…알람이 있는 서버들의 7월 CPU 사용률"* → false · *"서버 목록 보여주고, 그중 …알람 이력도"* → true) — 소형 모델에서는 표지 없는 복합이 루프에 들어오지 못한다. 순차 표지 OR가 전환기 보완이고, 내부망 재측정 전까지 기본 off를 유지한다 |
