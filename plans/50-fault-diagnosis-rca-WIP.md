...
> **★ v2.2 해소 실측(2026-09-02 · D-194)** — 아래 6건은 전부 구현·테스트로 닫혔다. 갭 서술은 착수 근거로 보존한다.
>
> | 갭 | 해소 | 검증 |
> |---|---|---|
...
> ③「채번 이력」 표를 모두 grep해 최댓값+1. **2026-09-01 실측 최댓값 D-193 → 다음 D-194**이나,
...
> | **U-E** *(v2.1 신설)* | 소유권 확정(§0.4-b)을 `docs/02_decision.md`에 **지금 등재**할 것인가, 착수 시점에 할 것인가 | **착수 시점**. 계획서 예약은 효력이 없으나(§14) 코드 착수 전 등재는 결정을 먼저 굳혀 실측 여지를 줄인다. 다만 사용자가 지금 확정을 원하면 D-194로 등재 가능 |
