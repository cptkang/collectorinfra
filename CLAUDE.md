...
| 2026-06-29 | Plan 52/53 계획서가 노이즈 게이트를 `D-040`으로 명시했으나 `D-040`은 이미 사용 중이었음(등재 시 충돌 위험) | `docs/02_decision.md`의 D-번호는 **본문 `## D-NNN` 섹션(D-001~D-038)** 과 **하단 「변경 이력」 표의 행(D-039·D-040은 2026-06-23에 표 행으로만 등재, 본문 섹션 없음)** 두 곳에 흩어져 있어, `## D-NNN` 헤더만 grep하면 D-039/D-040을 빈 번호로 오인 | 다음 빈 D-번호 산정 시 **`## D-` 헤더와 「변경 이력」 표 행을 모두** 확인. 계획서의 D-번호 표기는 작성 시점 기준이라 그 후 changelog가 번호를 선점했을 수 있음 → 등재 직전 실제 최대 번호를 재확인하고, 충돌 시 다음 빈 번호로 등재하며 사유를 본문에 명시(예: Plan52 노이즈 게이트는 D-040 선점으로 D-041 등재) |
