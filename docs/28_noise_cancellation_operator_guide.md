...
> 매처·심각도 상한·만료·사유가 감사에 남는다. `NOISE_SILENCE_ENABLED`가 켜져 있어야
> 게이트가 실제로 적용한다(기본 off). **심각도 3은 어떤 규칙으로도 침묵되지 않는다.**
> **이력(2026-09-22 · D-247)**: 이 수정 전 코드는 켜도 워커 경로에서 침묵 단계가 평가되지 않았다 — 워커가 넘기는
> 활성 규칙 목록이 알람 그래프 상태에 선언돼 있지 않아 게이트에 닿지 않았다(`plans/112` §9.4). 선언을 보강해
> 이제 켜면 적용된다. 그 전에 만든 규칙은 **이번 배포부터** 억제를 건다 — 켜기 전에 활성 규칙 목록을 먼저 확인할 것.
