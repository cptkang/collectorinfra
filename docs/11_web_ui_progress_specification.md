...
총 16개 노드가 등록되어 있으며(단일 파이프라인 12개 + 다중 의도 오케스트레이션 4개), 질의 유형 및 활성 백엔드(`ENABLE_DEEPAGENT_ORCHESTRATION`)에 따라 실제 실행되는 노드 조합이 달라진다.
...
| `intent_planner` | 의도 분석 | (다중 의도 경로) 사용자 질의를 처리할 작업(의도) 단위로 분해한다. 몇 개의 작업으로 분석되었는지와 각 작업의 종류·지시를 표시한다. |
| `agent_orchestrator` | 작업 실행 | (다중 의도 경로) 분해된 작업들을 의존성에 따라 순서/병렬로 실행하고, 작업별 완료/실패 상태를 갱신한다. |
| `replanner` | 재계획 | (다중 의도 경로) 1차 실행 결과를 평가하여 후속 작업이 필요한지 판단한다. 동적으로 추가된 작업과 그 사유를 이력으로 표시한다. |
| `result_aggregator` | 결과 통합 | (다중 의도 경로) 여러 작업의 결과를 하나의 자연어 응답으로 통합한다. 본문에는 내부 작업 라벨을 노출하지 않는다. |
...
### 4.13 다중 의도 오케스트레이션 노드 (Plan 48/49)

`ENABLE_DEEPAGENT_ORCHESTRATION` 경로에서는 `field_mapper → intent_planner → agent_orchestrator → (replanner 루프) → result_aggregator` 노드가 실행되며, 처리 현황 패널에 사용자 의도가 몇 개의 작업으로 분석·실행되었는지를 표시한다. 작업 목록의 agent 식별자는 사용자용 라벨(`agentLabels`: data_query→"DB 조회", alarm_query→"알람 조회", cache_management→"캐시 관리", synonym_registration→"유사어 등록", general_inference→"일반 안내")로 변환하여 노출한다.

**`intent_planner` — 의도 분석**

| 필드 | 표시 라벨 | 형태 | 설명 |
|------|----------|------|------|
| `task_count` | 의도 분석 | 뱃지 (`N개 작업으로 분석됨`) | 질의가 분해된 작업 수. |
| `tasks` | 작업 목록 | 목록 | 각 작업의 순번·종류(사용자용 라벨)·지시(`sub_query`)·상태. |

**`agent_orchestrator` — 작업 실행**

| 필드 | 표시 라벨 | 형태 | 설명 |
|------|----------|------|------|
| `tasks` | 작업 진행 | 목록 | 각 작업의 상태 뱃지(완료/실패/진행 중/대기). 루프 재진입 시 최신 상태로 갱신된다. |

**`replanner` — 재계획**

| 필드 | 표시 라벨 | 형태 | 설명 |
|------|----------|------|------|
| `replan_history` | 재계획 이력 | 목록 | 회차별 추가 작업 수와 사유. `state.replan_history`에 루프 누적되어 종료 후에도 보존된다. |
| `needs_replan=false` | 재계획 상태 | 뱃지 (`추가 작업 없음 (완료)`) | 후속 작업 없이 종료된 경우 표시. |

**`result_aggregator` — 결과 통합**

| 필드 | 표시 라벨 | 형태 | 설명 |
|------|----------|------|------|
| `status` | 상태 | 뱃지 (`응답 통합 완료`) | 통합 완료 상태. 본문 응답에는 `### 작업 N (agent)` 라벨을 노출하지 않는다(처리 현황으로 이전). |
