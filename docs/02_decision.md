...
- **D-번호 채번**: `## D-` 헤더와 하단 「변경 이력」 표를 **모두 grep**하여 실제 최댓값+1 부여 (현재 최대 D-088 → 다음 D-089).
- 본문 섹션 없는 번호(재사용 금지): D-039·D-040·D-060·D-077(변경 이력 표 행으로만 등재), D-052(D-051에서 replanner 인프라성 에러 가드용 예약), D-078~D-081(결번), **D-089~D-091(Plan 63 폴스타 과적합 분리 잔여 트랙 예약 — 착수 시 등재: D-089 어댑터 분리, D-090 어휘 매핑 LLM 전환, D-091 범용성 회귀 하네스)**.
