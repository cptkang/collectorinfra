...
- **D-048.7** (향후·옵션, E5) **deepagents Advisory Enricher** — agentic 분석은 **보조(`signals` 승격 전용)** 만, 판단은 결정적. vLLM(트랙 B, D-037) 옵트인 전제, 미가용 시 **semantic-routing(FabriX 1회 분류) → 결정적 게이트** 3중 폴백. 환각→오억제는 R-12로 통제(강등 경로 부재).
...
- **DASHBOARD SSE**(E3 부분 구현): API 경로(`analyze_alarm_*`)는 `alarm_bus` 주입으로 DASHBOARD/TICKET SSE 푸시 동작. **워커 경로는 cross-process라 in-memory `alarm_bus` 공유 불가** → 로그 폴백 + 일배치 큐만 동작. 워커→UI 실시간 푸시는 Redis pub/sub 브리지 필요(후속).
