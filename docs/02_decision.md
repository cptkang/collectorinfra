...
- **결정일**: 2026-07-30 | **상태**: 확정 (S2 구현 완료 · 기본 OFF 옵트인 — 활성화 판단은 S3 평가 후) | **번호**: plans/67 예약분 등재(안내 라인 명시 — D-129~131·133이 먼저 등재되어 번호만 아래).
...
- **잔여**: 피벗 wrapper 소비 테스트 4곳 신 진입점 전환, tools/validation→nodes 단방향 임포트 해소, W-6(멀티 유사어·설명 재료 조달 — 별건).
...
## D-137. LLM 호출 거버넌스 — 프롬프트당 계측 + 무손실 호출 절감 4종 (Gemini 과다 호출 분석 후속)

- **결정일**: 2026-08-04 | **상태**: 확정 (구현 완료) | **번호**: 등재 직전 `## D-` 헤더·「변경 이력」 표 재확인(최댓값 D-136 → **D-137**).
- **배경**: Gemini API 과다 호출(관측 ~600회/일) 원인 분석 — checkpoints.db 188턴·audit 1,852건 포렌식 실측 결과, 오케스트레이터는 프롬프트당 중앙값 2회(최대 5회, 병렬 tool call 0건)로 절제되어 있으나 ① **워커 호출 계측 전무**(600회 실측 검증 자체가 불가), ② flash-lite 빈 응답 재개 루프(D-093 우회책)가 사고 턴에서 모델 5회+도구 2회 유발, ③ SSE 폴백이 그래프 **전체 재실행**(×2), ④ 재개 시 동일 sub_query 파이프라인 재실행(D-062는 사후 합성일 뿐), ⑤ 평가 하네스·스모크가 운영 키 공유(일 120~447건 자동 제출 — 4건/초 배치 ×132회 실측). 정확도 저하를 수반하는 완화책(candidate_count 하향·도구 실행 하드캡·task 서브에이전트 즉시 비활성화)은 **채택하지 않음**.
- **결정**: ① **LLM 호출 계측**(`src/observability/llm_call_counter.py` 신설) — 생성자 콜백을 모델 생성 6지점(src/llm.py) 전부에 부착, 요청별 contextvar 카운터를 query API 4개 진입점에 배선, `LLM_CALL_WARN_THRESHOLD`(기본 30, OS env 직독) 초과 시 **차단 없는 경고 1회**. `task` 내장 서브에이전트는 **첫 호출 감지 warning**(조건부 봉인 1단계 — 실측 사용 0건이라 비활성화는 보류, 사용 관측 시 재론). ② **SSE 폴백 체크포인트 회수 우선**(query.py 텍스트·파일 스트림 2곳) — 스트림 완주(END 도달) 확인 시 저장된 final_response 재사용(LLM 0회), 미완주·회수 불가 시에만 기존 ainvoke 폴백 유지(멀티턴 직전 턴 final_response 재사용 오답 가드 포함). ③ **턴 내 동일 sub_query 메모이제이션**(deepagents_tools.py) — 요청 스코프 collector 재사용(구조적 격리·초기화 코드 0줄), 성공 결과만 메모, deepcopy 반환, 합성 경로(D-062) 무변경. ④ **평가·스모크 키 분리** — `EVAL_GEMINI_API_KEY`(.encenv 관리) 설정 시 `scripts/eval_text2sql.py`·`run_pipeline_test.py`가 운영 키 대신 사용(미설정 시 기존 동작). ⑤ **ORCHESTRATOR_MODEL 상향** — gemini-2.5-flash-lite → gemini-2.5-flash(.env, 빈 응답 재개 루프의 근원 제거 — .env.example에 flash-lite 금지 주석).
- **검증**: 신규 테스트 31건(카운터 6·SSE 회수/계측 배선 9·메모 11·평가 키 5) 통과, 프롬프트 sha256 골든 그린(프롬프트 무변경 — D-134 게이트), `arch_check --ci` 0(src.observability=utils 계층 등재), 기존 실패는 전건 클린 HEAD 동일 재현으로 무관 입증. 실 LLM 호출 0(D-127).
- **주의**: ① 임계치 env는 OS 환경변수 직독(pydantic env_file 미주입 — 운영 변경 시 프로세스 env로). ② `/query/stream` 승인 의사 LLM 보조(옵트인)는 SSE 생성기 밖 실행이라 미계측. ③ 메모는 턴 내 한정 — 턴 중 manage_cache 후 문자 동일 재조회는 갱신 전 결과 반환(재개 중복 방지의 의도된 트레이드오프, 문구가 다르면 재실행). ④ worktree 검증 시 `.env`뿐 아니라 **`.encenv`도 복사 필수**(config env_file 2원 — 미복사 시 Gemini 키 부재로 그래프 빌드 테스트 위양성 실패).
- **관련**: D-062(중복 적재 사후 합성)·D-092/D-093(빈 응답 재개 루프)·D-127(과금 게이트)·D-129/D-135(설정 카탈로그·encenv 관리 키)·D-134(동작 불변 게이트).
...
| 날짜 | 결정 ID | 변경 내용 |
|------|---------|----------|
| 2026-08-04 | D-137 | **LLM 호출 거버넌스 — 프롬프트당 계측 + 무손실 절감 4종 등재** — 호출 카운터(`src/observability`·모델 생성 6지점 콜백·API 4진입점 배선·임계 30 경고, task 첫 호출 감지 — 비활성화는 보류), SSE 폴백 체크포인트 회수 우선(2곳·LLM 0회), 턴 내 sub_query 메모(collector 재사용), `EVAL_GEMINI_API_KEY` 평가 키 분리, ORCHESTRATOR_MODEL flash 상향(빈 응답 재개 루프 근원 제거). 정확도 저하 완화책(candidate 하향·도구 하드캡·task 즉시 봉인)은 불채택. 신규 31건·골든 그린·arch 0·실호출 0(D-127). |
