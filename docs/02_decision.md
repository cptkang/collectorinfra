...
- **D-번호 채번**: 이 파일의 **①`## D-` 헤더 ②하단 「변경 이력」 표 ③아래 「채번 이력」 표**를 모두 grep하여 실제 최댓값+1 부여 (현재 최대 **D-167** → 다음 **D-168**. **D-158은 미등재 예약** — 재사용 금지). **계획서에만 적힌 예약은 효력이 없다** — grep 대상은 이 세 곳뿐이므로 예약은 ③에 행으로 등재할 것(D-161 참조).
...
## D-166. 응답 표 렌더 안전망(GFM 구분선 정규화) + 양식 저장 값 패널(항목별 삭제 버튼)
...
- **폐쇄망 확인**: ①19열 응답 표 렌더(원문 노출 0) ②양식 업로드 후 `?` → 목록 아래 패널, [선택 삭제] → "1건을 삭제했습니다 … 남은 항목 N건" + 패널 갱신, [전체 삭제] confirm → "모두 삭제" + 패널 없음, 로그 `form_memory_delete 처리(D-166)`. 배포 7파일(src 6 + app.js) + grep 심볼 `_normalize_markdown_tables`/`build_form_memory_panel`/`_form_memory_delete_or_none`/`renderFormMemoryPanel`/`form_memory_panel`(routes 10곳 카운트 ≥ 12).
...
- **관련**: D-151(HITL·확인 이력)·D-165('?' 단축키)·FIX-20/23(이력 명령 환각 차단)·D-100(표 전 칼럼 규칙)·D-143 후속2(pre-gate 대칭 선례)

## D-167. 알람 서버 식별 역조회 — hostname → 폴스타 등록 서버명·IP·존 (템플릿 platformName 미지원 대응)
- **결정일**: 2026-08-26 | **상태**: 구현 완료(개발망 테스트 통과) — 폐쇄망 실기 검증 대기
- **배경**: 폴스타 알람 소켓 연계(`KbAlarmSocketLinkageProcessor2$MessageContext`)가 `${platformName}`을 지원하지 않아(SpEL `EL1008E`, 발송 자체 실패) 운영 템플릿이 `"serverName":"${hostname}"`로 바뀌었다(`ipAddress`·`resourceAncestry`·`conditions`도 미지원으로 제외). Plan 46 §6.1의 변수 가정이 실제와 달랐던 두 번째 사례(첫 번째=D-163 severity). 결과: ①웹 UI 헤더가 hostname만 보여 공동존(name ≠ hostname)에서 서버 식별이 어렵고 ②이력·노이즈 컨텍스트의 `SVR.NAME = server_name` 완전 일치(D-035)가 공동존에서 0건이 될 위험. 사용자 요청은 헤더에 등록 서버명·IP·OS 노출.
...
- **관련**: D-163(severity 라벨·같은 원인군)·D-035(SVR.NAME 매칭)·D-046(hostname 리졸버 원형)·D-022(EAV 조인 규칙)·D-049(incident 재발행 카드)·Plan 46 §6.1·Plan 59 §17(존)
...
| 날짜 | 결정 ID | 변경 내용 |
|------|---------|----------|
| 2026-08-27 | D-167 | **OS 배지 실데이터화 + 헤더 병합** — 식별 SELECT에 `OSType` 스칼라 서브쿼리(D-022 재검토 정본 조인 키) 추가, UI 1행을 `[심각도] 등록명 — 알람명 (자원)`로 병합(사용자 검토: 서버·알람이 한 줄에서 읽히도록), 메타 줄에 발생/수신 시각. |
| 2026-08-27 | D-163·D-167 | **`dead_letter_enabled`·`server_identity_enabled` 플래그 제거** — 둘 다 기본 on이고 끌 이유가 없으며(실패·타임아웃은 graceful) D-162 §6 "신규 enable_* 플래그 0"·flag_audit "참조 1~2건·기본 ON → 상수화" 판정에 정확히 해당(사용자 지적). 타임아웃·TTL·스트림 키·상한은 운영 파라미터로 유지. 카탈로그 280→278. |
| 2026-08-26 | D-167 | **알람 서버 식별 역조회(hostname → 폴스타 등록 서버명·IP·존)** — 폴스타 템플릿이 `${platformName}`(EL1008E)·`ipAddress` 등을 지원하지 않아 알람에 hostname만 실리게 된 데 대응. `application/server_identity` 단일 함수를 워커·API 양쪽에 배선, 결정적 승격 규칙(hostname을 준 경우만 등록명 교체·IP는 빈 경우만·모호 시 생략)으로 이력 매칭·지문·통보·SSE가 등록명을 쓰게 하고, UI 헤더를 3줄(등록명 / hostname·IP·OS·존 / 알람명·자원)로 재구성. OS(EAV)는 2차. |
| 2026-08-26 | D-166 | **응답 표 렌더 안전망 + 양식 저장 값 패널(항목별 삭제)** — 라이브 실측: 19열 응답 표가 GFM 구분선 셀 수 불일치로 원문 노출(→ `_normalize_markdown_tables`, 렌더만 보장·내용 불변), 칼럼 순서 뒤섞임은 "검증 후 교체" 후보로 보류(사용자 확인), '?' 조회 응답에 `form_memory_panel` 동봉 → 프론트 체크박스/[선택 삭제]/[전체 삭제] → `form_memory_delete` 구조화 요청을 라우트가 파이프라인·LLM 없이 처리(세션 signature 일치 시만). 테스트 3건 신규. |
