...
| D-196 | **예약 유지**(팀장 로컬 미커밋 작업 점유) | `plans/54` 관제 대시보드 — 미등재·재사용 금지 |
| D-195 | **예약 유지**(팀장 로컬 미커밋 작업 점유) | `plans/87` 예약 — 미등재·재사용 금지 |
...
- **결정일**: 2026-07-24 | **상태**: 확정 (계획 · **미구현 — 착수 시 구현**, Plan 60 §18) | **⚠ ① 개정됨(2026-08-31 · D-189)**: 접근 경로가 **A(폴스타 에이전트 확장) → B(허용목록 명령 실행)**로 바뀌었다. 아래 「보안 인터뷰 확정」 ①의 A안 문구는 **의사결정 이력으로 보존**하며 현행 결정이 아니다. ②~④(실행 모델·허용목록·통제)와 그 아래 「결정」·「불변식」은 전부 유효. **R11의 P0-4(벤더 협의) 블로커는 해소** | **번호**: 등재 최댓값 D-116·D-115는 Plan 65 예약 유지 → **D-117**(등재 직전 `## D-` 헤더·변경 이력 표 재확인 완료).
