...
> **D-101~D-104는 `ux_improvement` 브랜치(골든셋 실데이터 수렴 작업)에서 병합된 결정이다.**
> 원래 D-084~D-087로 기록됐으나 `multiintent`가 그 번호를 다른 결정에 이미 사용해(번호 충돌),
> 팀 규약대로 팀장 번호를 우선하고 본 결정들을 최댓값(D-100) 뒤로 재부여했다(2026-07-22 머지).
> 병합된 코드 주석의 해당 D-번호도 이 매핑으로 갱신됨: 구 D-084→D-101, D-085→D-102, D-086→D-103, D-087→D-104.

## D-101. 이번 턴 원문 위치 힌트의 결정적 DB 고정 + 맥락 주입 오염원 차단 (구 UX D-084)
- **결정일**: 2026-07-16 | **상태**: 확정(사용자 승인 — 2a+2b 동시 적용) | **병합**: ux_improvement→multiintent(2026-07-22, 구 D-084)
- **결정**: 폼필 뒤 텍스트 질의 "은행존 알람"이 intent_planner LLM 분해에서 직전 위치와 병합돼 sub_query가 "김포 은행 공동존…"으로 오염→gp 오라우팅(침묵적 오답). (2a) `_build_context_block(ctx, user_query)`가 이번 턴 원문에 위치 표면어(`LOCATION_HINT_TERMS`, input_parser 단일 출처) 있으면 직전 위치/DB 줄을 **미주입**(입력에 없으면 병합 불가 — 구조적 차단, 서버 엔티티·요약은 유지). (2b) 단일 task+힌트 해소 성공 시 `_apply_turn_hint_pinning`이 `target_db_hints`를 `resolve_priority_db_ids`(D-065 계열)로 해소해 DB 집합 고정, classify_dbs는 정제 질의 재사용하되 DB 결정권만 결정적 로직이 가짐.
- **근거**: LLM에 "무시하라" 지시(프롬프트) 대신 충돌 정보를 입력에서 제거 — D-047/D-055/D-065에서 반복 확인된 대로 LLM 규칙 준수는 비결정적. LLM 경계 버그는 mock 재현 불가 → 실환경 재테스트.
- **한계**: 복합 계획은 2b 미적용(2a가 오염 차단). "은행"이 위치 아닌 의미로 쓰인 질의는 드묾(수용).
- **관련**: D-065, D-053, D-055, D-047, D-066

## D-102. 기간 표현 "지난 N개월"의 결정적 범위 해석 (폼필 기간 필터 침묵 누락 수정, 구 UX D-085)
- **결정일**: 2026-07-16 | **상태**: 확정(폐쇄망 실측 버그 수정) | **병합**: ux_improvement→multiintent(2026-07-22, 구 D-085)
- **결정**: 폼필 "지난 3개월간 통계"가 metric 조인에 기간 필터 없이 실행돼 전체 보관 기간 평균 반환(침묵적 오답). 원인: `resolve_stat_month`가 "지난 N개월"을 인식 못하고 반환형(단일 YYYYMM)이 범위를 표현 못함. → **`resolve_stat_month_range`로 대체**(rename으로 잔존 호출부 강제 검출), 반환형 `(시작,끝)` 튜플. "지난/최근/과거 N개월"=직전 완결 월부터 과거 N개월(진행 중인 달 제외, D-076 후속4 유지), 단일 신호는 `(m,m)`. 렌더러(`build_multi_resource_pivot_sql`·`build_stat_month_block`)는 `StatMonth=str|tuple|None` 받아 등호/BETWEEN 겸용(`_normalize_stat_month`). 세 경로(query_generator·multi_db_executor·compile_smq) 동시 갱신(D-066).
- **근거**: 결정적 경로(폼필)는 파서가 유일 기간 해석 지점 → 파서 확장이 근본. 폼필만 LLM 폴백 강등안은 D-068 3차 변동성 부활로 기각.
- **한계**: "지난 반년"·"작년"·한글 수사는 미인식(None→전체 평균, 종전 동일).
- **관련**: D-068(3차), D-076(후속4), D-066
- **머지 주의(D-085 충돌)**: `multiintent`의 D-085는 "LEFT JOIN 강등 가드"로 별개 — 코드에 두 의미가 공존하니 범위 관련 참조만 본 D-102.

## D-103. 사용률 집계의 크래시 면역(DOUBLE 캐스트) + 값 타당성 게이트 (SQL0413N, 구 UX D-086)
- **결정일**: 2026-07-16 | **상태**: 확정(폐쇄망 3단계 데이터 실측으로 경계 확정) | **병합**: ux_improvement→multiintent(2026-07-22, 구 D-086)
- **결정**: D-102로 "지난 6개월" 범위가 동작하자 b0(DB2) 폼필이 `SQL0413N` 변환 오버플로로 실패(3개월은 정상 — 데이터 의존). 원인: 행 캐스트 `DECIMAL(15,4)`(상한 ~1e11)가 쓰레기 행(실측 max_val=5.5e13)에서 오버플로. (1) **크래시 면역**: DB2 행 캐스트를 `DOUBLE`(~1e308, 원리적 불가), 집계 캐스트 `DECIMAL(31,2)`(스케일 2 고정 유지·정밀도만 확장). (2) **값 타당성 게이트**: `UTILIZATION_VALID_RANGE=(0,1000)`을 metric CASE에 `AND s.<val_col> BETWEEN 0 AND 1000` 필드별 추가, `definition_name='Utilization'`일 때만(`_utilization_guard`). 상한 1000 근거: 실측 만재 피크(≤100.1) 통과+쓰레기(≥1.2e9) 차단, 그 사이 값 전무. (3) 적용: `_metric_select_line`(단일 출처)+`decimal_cast_example`(LLM 안내)+db_profiles 4종 few-shot 각 15곳. PG도 게이트 대칭(크래시 캐스트는 DB2만).
- **근거**: 행 캐스트는 DOUBLE(오버플로 원리적 불가), 값 오염은 의미 게이트로 분리 대응. 경계는 추정 아닌 실측(상한 100은 정상 만재 100.0…1~100.1을 잘라냄).
- **관련**: D-102(이 버그의 노출 계기), D-065(후속2)/D-068(7차), D-066, D-003(읽기전용)
- **머지 주의(D-086 충돌)**: `multiintent`의 D-086은 "선행 task 스코프 주입"으로 별개 — DOUBLE/게이트 관련 참조만 본 D-103.

## D-104. 생성 SQL의 자연어(한글) 토큰 잔존 결정적 차단 (validator 가드, 구 UX D-087)
- **결정일**: 2026-07-20 | **상태**: 확정(EX 평가 실측, 재현 2회) | **병합**: ux_improvement→multiintent(2026-07-22, 구 D-087)
- **결정**: 생성 SQL이 지시어("해당"→재실행 시 "현재") 자연어 조각을 구조 영역에 남겨 DB 구문 오류(validator 통과 후 실행 실패, 재시도도 같은 유형). LLM 비결정 오류(D-055 플레이스홀더의 SQL 본문판) — 프롬프트로는 차단 불가. → `query_validator._find_bare_hangul_tokens(sql)`: 주석·문자열 리터럴·따옴표 식별자 제거 후 남는 한글 토큰 검출(따옴표 안 별칭 "CPU 평균"·값 '서울'은 허용). 검출 시 검증 실패(토큰 명시+재작성 지시)→재시도 재생성. 단일(query_validator 4.5)·멀티(`_validate_sql_simple`) 양쪽 공유(D-066, 헬퍼는 query_validator 단일 출처).
- **근거**: 비결정 LLM 오류는 결정적 가드로 차단, 금지 규칙 범위는 좁게 못박음(따옴표 안 허용, D-055 교훈).
- **관련**: D-055, D-047, D-066, D-072
- **머지 주의(D-087 충돌)**: `multiintent`의 D-087은 "validator CTE 주석 비대칭"으로 별개 — 한글 토큰 관련 참조만 본 D-104.
