...
- **D-번호 채번**: 이 파일의 **①`## D-` 헤더 ②하단 「변경 이력」 표 ③아래 「채번 이력」 표**를 모두 grep하여 실제 최댓값+1 부여 (현재 최대 **D-179** → 다음 **D-180**. **D-158·D-163~D-168은 미등재 예약** — 재사용 금지. **D-169~D-174는 2026-08-27 본문 등재 완료 · D-175~D-179는 bug_fix 병합 시 원 D-163~D-167에서 재부여해 본문 등재 완료(2026-08-27)**). **계획서에만 적힌 예약은 효력이 없다** — grep 대상은 이 세 곳뿐이므로 예약은 ③에 행으로 등재할 것(D-161 참조).
...
| D-175~D-179 | **채번 완료**(2026-08-25~27 본문 등재 · bug_fix 병합 시 원 D-163~D-167에서 재부여) | D-175 폴스타 심각도 라벨·dead-letter·카탈로그 그룹 / D-176 폼필 기준월 범위·앵커 폴백 / D-177 폼필 HITL 3건 / D-178 표 렌더 안전망·저장 값 패널 / D-179 알람 서버 식별 역조회 |
...
## D-175. 폴스타 심각도 라벨 정규화 + 워커 dead-letter + 설정 카탈로그 그룹 전수 등재
...
## D-176. 폼필 월 시리즈 기준월 — 절대 월 범위 정규식 + 앵커 산출에 LLM 기간 폴백 대칭 배선
- **결정일**: 2026-08-25 | **상태**: 구현 완료(개발망 테스트 통과) — 폐쇄망 실기 검증 대기
...
## D-177. 폼필 HITL 3건 — 답변 턴 존 보존 실효화 + 이력 조회 게이트 확장('?' 단축키) + 응답 연도 환각 차단
- **결정일**: 2026-08-25 | **상태**: 구현 완료(개발망 테스트 통과) — 폐쇄망 실기 검증 대기
...
- **관련**: D-151(HITL 폼필)·D-157(FIX-26)·FIX-20/23/24(이력 명령 게이트)·D-176(기준월)·D-092(per-task 최종화 스코프)·Known Mistakes 2026-08-25

## D-178. 응답 표 렌더 안전망(GFM 구분선 정규화) + 양식 저장 값 패널(항목별 삭제 버튼)
...
- **폐쇄망 확인**: ①19열 응답 표 렌더(원문 노출 0) ②양식 업로드 후 `?` → 목록 아래 패널, [선택 삭제] → "1건을 삭제했습니다 … 남은 항목 N건" + 패널 갱신, [전체 삭제] confirm → "모두 삭제" + 패널 없음, 로그 `form_memory_delete 처리(D-178)`. 배포 7파일(src 6 + app.js) + grep 심볼 `_normalize_markdown_tables`/`build_form_memory_panel`/`_form_memory_delete_or_none`/`renderFormMemoryPanel`/`form_memory_panel`(routes 10곳 카운트 ≥ 12).
...
- **관련**: D-151(HITL·확인 이력)·D-177('?' 단축키)·FIX-20/23(이력 명령 환각 차단)·D-100(표 전 칼럼 규칙)·D-143 후속2(pre-gate 대칭 선례)

## D-179. 알람 서버 식별 역조회 — hostname → 폴스타 등록 서버명·IP·존 (템플릿 platformName 미지원 대응)
- **결정일**: 2026-08-26 | **상태**: 구현 완료(개발망 테스트 통과) — 폐쇄망 실기 검증 대기
- **배경**: 폴스타 알람 소켓 연계(`KbAlarmSocketLinkageProcessor2$MessageContext`)가 `${platformName}`을 지원하지 않아(SpEL `EL1008E`, 발송 자체 실패) 운영 템플릿이 `"serverName":"${hostname}"`로 바뀌었다(`ipAddress`·`resourceAncestry`·`conditions`도 미지원으로 제외). Plan 46 §6.1의 변수 가정이 실제와 달랐던 두 번째 사례(첫 번째=D-175 severity). 결과: ①웹 UI 헤더가 hostname만 보여 공동존(name ≠ hostname)에서 서버 식별이 어렵고 ②이력·노이즈 컨텍스트의 `SVR.NAME = server_name` 완전 일치(D-035)가 공동존에서 0건이 될 위험. 사용자 요청은 헤더에 등록 서버명·IP·OS 노출.
...
- **관련**: D-175(severity 라벨·같은 원인군)·D-035(SVR.NAME 매칭)·D-046(hostname 리졸버 원형)·D-022(EAV 조인 규칙)·D-049(incident 재발행 카드)·Plan 46 §6.1·Plan 59 §17(존)
...
| 날짜 | 결정 ID | 변경 내용 |
|------|---------|----------|
| 2026-08-27 | D-179 부기 | **알람 헤더 배지 툴팁 상세화** — ①OS 배지: 라벨은 계열명(Linux…) 유지, 툴팁을 EAV `OSVerson`(폴스타 원본 철자) 상세 버전으로(식별 SELECT에 스칼라 서브쿼리 1개 추가, `ServerIdentity.os_version`, 캐시 키 `alarm:identity:v2:`로 세대 분리) ②소스 배지: 김포/여의도/은행 대신 레지스트리 `family` 제품명("폴스타")을 표시하고 위치·db_id는 툴팁("폴스타 — 공동존 김포; polestar_cm_gp")으로 — 알람 소스 확장 시 `db_registry.yaml` family 등록만으로 배지가 따라오도록(`source_labels_for`, 하드코딩 0). `app.js?v=4` 캐시 버스팅. 테스트 +5. |
| 2026-08-27 | D-175~D-179 병합 | **bug_fix → multiintent 병합** — 본 브랜치 결정 5건을 원 D-163~D-167에서 **D-175~D-179로 재번호**(팀장 예약 D-163=Plan 77·D-164~168=Plan 78 우선, 참조 37파일 157건 일괄 치환). 병합 시 Plan 78/79 신규 `composite`·`router`·`host_authz` 그룹이 `GROUP_ORDER` 미등재(D-175 부기와 동일 유형)로 어드민 UI 누락 → 등재 + 전수 단언 상수 갱신(그룹 24·항목 294). |
...
| 2026-08-27 | D-179 | **OS 배지 실데이터화 + 헤더 병합** — 식별 SELECT에 `OSType` 스칼라 서브쿼리(D-022 재검토 정본 조인 키) 추가, UI 1행을 `[심각도] 등록명 — 알람명 (자원)`로 병합(사용자 검토: 서버·알람이 한 줄에서 읽히도록), 메타 줄에 발생/수신 시각. |
| 2026-08-27 | D-175·D-179 | **`dead_letter_enabled`·`server_identity_enabled` 플래그 제거** — 둘 다 기본 on이고 끌 이유가 없으며(실패·타임아웃은 graceful) D-162 §6 "신규 enable_* 플래그 0"·flag_audit "참조 1~2건·기본 ON → 상수화" 판정에 정확히 해당(사용자 지적). 타임아웃·TTL·스트림 키·상한은 운영 파라미터로 유지. 카탈로그 280→278. |
| 2026-08-26 | D-179 | **알람 서버 식별 역조회(hostname → 폴스타 등록 서버명·IP·존)** — 폴스타 템플릿이 `${platformName}`(EL1008E)·`ipAddress` 등을 지원하지 않아 알람에 hostname만 실리게 된 데 대응. `application/server_identity` 단일 함수를 워커·API 양쪽에 배선, 결정적 승격 규칙(hostname을 준 경우만 등록명 교체·IP는 빈 경우만·모호 시 생략)으로 이력 매칭·지문·통보·SSE가 등록명을 쓰게 하고, UI 헤더를 3줄(등록명 / hostname·IP·OS·존 / 알람명·자원)로 재구성. OS(EAV)는 2차. |
| 2026-08-26 | D-178 | **응답 표 렌더 안전망 + 양식 저장 값 패널(항목별 삭제)** — 라이브 실측: 19열 응답 표가 GFM 구분선 셀 수 불일치로 원문 노출(→ `_normalize_markdown_tables`, 렌더만 보장·내용 불변), 칼럼 순서 뒤섞임은 "검증 후 교체" 후보로 보류(사용자 확인), '?' 조회 응답에 `form_memory_panel` 동봉 → 프론트 체크박스/[선택 삭제]/[전체 삭제] → `form_memory_delete` 구조화 요청을 라우트가 파이프라인·LLM 없이 처리(세션 signature 일치 시만). 테스트 3건 신규. |
| 2026-08-25 | D-177 | **폼필 HITL 3건 — 답변 턴 존 보존 실효화 + 이력 조회 게이트('?' 단축키) + 응답 연도 환각 차단** — 라이브 실측: ①`_build_output_state`에 존 키 부재로 FIX-26이 활성(오케스트레이션) 경로에서 한 번도 작동 안 함(→ `selected_db_ids`·`target_databases` 추가, 연결 경로 테스트) ②의문형·"저장된 내용" 미탐(→ '?' 단축키 + 최소 키워드 확장 + 채움 동사 가드 + 힌트) ③응답 프롬프트에 연도 부재로 "2023년" 환각(→ `## 기준 정보` 주입 + 요약 문단 한정 후행 가드, 자동 치환 없음). 테스트 4건 신규. |
| 2026-08-25 | D-176 | **폼필 월 시리즈 기준월 — 절대 월 범위 정규식 + 앵커 LLM 기간 폴백 대칭 배선** — 금감원 CPU 양식 "1월부터 6월까지"가 2~7월로 채워진 라이브 실측: 연도 없는 월 범위 정규식 부재 + 앵커 산출 자리에 D-136 2단 폴백 미배선(stat_month 자리만 배선됐고 month_measures가 이를 덮어 무효). `_MONTH_RANGE_RE`/`_HALF_YEAR_RE` 신설(단일 월보다 우선), `recognize_month_series(parsed_time_range=)` 단일·멀티 대칭, `month_anchor_payload` 단일 출처(requested/source), 안내문 근거 구분 + `[기간 불일치]` 명시, 멀티 경로 인식 로그·입력 파싱 time_range 로그. 테스트 24건 신규. |
| 2026-08-25 | D-175 | **폴스타 심각도 라벨 정규화 + 워커 dead-letter + 설정 카탈로그 그룹 전수 등재** — 폐쇄망 실측: `alarm:raw`의 severity가 `"해제"/"주의"` 한글 라벨 → 워커 `int()` ValueError → ACK 폐기(전량 UI 미도달). domain `severity.py` 결정적 매핑을 워커·API 양쪽에 적용, 실패 건 `alarm:dead` Stream 보관(자동 재투입 없음), 템플릿 따옴표 정정. 부기: `polestar_rest`·`drm` 그룹이 `GROUP_ORDER` 미등재로 어드민 설정 UI에서 누락 → 등재 + 전수 등재 단언(277·21). |
