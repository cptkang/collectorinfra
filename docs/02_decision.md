...
### 후속 4 ("은행존" 표면어가 b0 alias와 부분매칭 안 됨 — 공동존 alias 보강의 b0 미전파)

버그(2026-07-16, multiintent 병합 후 실테스트): "**은행존**과 공동존 김포 폴스타의 모든 서버" /
"공동존 여의도와 **은행**의 모든 서버" 폼필이 둘 다 **공동존 DB만** 조회(gp만·yd만)하고 b0 누락.
후속 3의 hint 단위 배제는 정상 동작 — 문제는 **alias 매칭 단계**:

- b0 `aliases`가 `["polestar_b0", "은행 폴스타", "레거시 폴스타"]`뿐이라, LLM(규칙 10:
  `"은행"/"레거시"(은행존)`)이 내는 **"은행존"/"은행존 폴스타" 표면어가 어느 alias와도 부분매칭
  실패**("은행존"⊄"은행 폴스타" — 존이 붙어 별개 토큰). 후속 3 당시 테스트 표현이 "은행 폴스타"
  (alias 정확 일치)라 이 갭이 안 드러났다.
- 결정적 가드 `_ensure_location_hints`도 못 메움: "은행"이 이미 hint 문자열("은행존")에 부분문자열로
  포함돼 bare "은행" 보강을 스킵.
- 실측: `["은행존", "공동존 김포 폴스타"]`→`[gp]`, `["공동존 여의도", "은행존"]`→`[yd]` (버그 재현),
  `["은행 폴스타", …]`→`[b0, gp]` (정상) — alias 갭이 유일 원인.

수정: D-065 본문과 동일 패턴("여러 표면어는 각 DB alias에 단독 등재")을 **b0에 전파** —
`domain_config.py` b0 aliases에 `"은행"`/`"은행존"`/`"은행존 폴스타"` 추가. alias "은행"은
`alias in hint` 방향으로 "은행존" 계열 hint를 전부 커버하고, gp/yd는 `_DB_EXCLUDING_REGIONS`의
"은행" 배제로 이 hint에 안 걸린다(기존 회귀 전부 유지). 회귀:
`test_gongdongjon_routing.py::TestBankZoneSurfaceForms`(두 실테스트 케이스 재현 포함).
교훈: **위치 표면어 alias 보강은 한 존(gp/yd 공동존)만 하지 말고 전 존(b0 은행존)에 대칭 적용** —
동일 결함이 존만 바꿔 재발한다. LLM 프롬프트가 안내하는 표면어 형태("은행존")와 alias 표기("은행
폴스타")가 어긋나면 부분매칭이 조용히 실패하므로, 규칙 10에 등장하는 모든 표면어는 alias로 실측 검증.
...
## D-087. 생성 SQL의 자연어(한글) 토큰 잔존 결정적 차단 (validator 가드)

| 항목 | 내용 |
|------|------|
| **결정일** | 2026-07-20 |
| **상태** | 확정 (EX 평가 실측 버그 수정 — 재현 2회) |
| **관련 결정** | D-055(지시어/플레이스홀더 결정적 차단), D-047(LLM 출력의 결정적 교정), D-066(단일/멀티 경로 단일 출처), D-072(EX 평가 하네스 — 발견 경로) |

### 배경 / 문제

EX 평가 실측(2026-07-20, yd-006 "경고 이상 알람의 담당자 그룹까지 포함해 이력 조회")에서
생성 SQL이 `QueryExecutionError: 구문 오류, "해당" 부근`으로 실행 실패. 재실행 시 토큰이
"현재"로 바뀌며 동일 실패 — **LLM이 자연어 조각(지시어류)을 SQL 구조 영역에 남기는 비결정
오류**로, 프롬프트 강화로는 재발을 막을 수 없는 유형(D-055 "previous_server" 플레이스홀더와
동일 계열, 이번엔 SQL 본문). 기존 validator는 문법 파싱(sqlparse 관대)·금지어·테이블/컬럼
검증만 있어 이를 통과시키고 DB에서 터졌다.

### 결정

- `query_validator._find_bare_hangul_tokens(sql)`: 주석·문자열 리터럴(`''` 이스케이프 포함)·
  따옴표 식별자를 제거한 뒤 남는 한글 토큰을 검출. **따옴표 안 한글(폼필 별칭 "CPU 평균",
  값 리터럴 '서울')은 정당하므로 제외** — 금지 규칙의 범위를 좁게 못박음(D-055 교훈).
- 검출 시 검증 실패(오류 메시지에 토큰 명시 + 재작성 지시) → 기존 재시도 루프(max 3)가
  재생성을 유도. 단일 경로(query_validator 4.5단계)와 **멀티 DB 경로(`_validate_sql_simple`)
  양쪽에 동일 가드 공유**(D-066 비대칭 방지, 헬퍼는 query_validator 단일 출처).
- 검증: `test_query_validator_hangul.py` 6건(실측 토큰 2종·따옴표 안 허용·주석 제거·노드
  레벨 실패/통과). validator 계열 73건 통과, arch exit 0.
...
| 날짜 | 결정 ID | 변경 내용 |
|------|---------|----------|
| 2026-07-21 | D-076 | **(후속14) SMQ 용량 차원 누락 주입 가드 + gp-010 known_fail — 최종 마감 정정**: 마감 직전 재실측에서 2건 재흔들림. ① gp-009(7/6, 미일치 logicalcore) 3차 회귀 — SMQ 선택이 동일 질의에서 동시선택→단독선택→**미선택**으로 흔들림(치환 가드는 미선택을 못 잡음) → `normalize_smq`에 명시 요청 용량 차원 주입 가드(`_CPU_CAPACITY_RE`/`_MEM_CAPACITY_RE` — 질의가 CPU/메모리 용량을 명시했는데 해당 dim 부재 시 LOGICALCORE/TotalSize 주입, 패턴 A/B 한정·중복 주입 금지·미요청 시 불변, 회귀 6단언+컴파일 e2e). ② gp-010(4725/4728) — 월별 분해 폴백의 행 모집단 미세 유동(+3행=진행 중 달 월간 통계 조기 적재 서버 추정, 값 정확성은 직전 실측 확인) → known_fail(근본 해법: 트랙 C 월별 분해 지원, 백로그). **최종 상태 정정**: 총 26항목 = 22 PASS + 3 의도적 FAIL(yd-005·gp-010·gp-014) + gp-009 주입 가드 검증 대기 |
| 2026-07-21 | D-072 | **(후속) 3존 골드셋 수렴 완료 — 최종 상태**: yd 5 PASS+1 known_fail(yd-005 랭킹 폴백 형태 플립플롭), b0 5/5 PASS, gp 14 PASS+1 known_fail(gp-014 유사사양 자기조인 — 4회 실측 4형태의 조합 비결정, 개별 요소는 전사 검증상 전부 정상·조합 단계 0행 → 사용자 원칙대로 감시 전환. 부수 성과로 CTE 실결함 2건 발굴). 총 26항목: 24 PASS + 2 의도적 FAIL(감시). 이 스위트는 이후 회귀 게이트로 운용하고, 확장은 감사 로그 실사용 질의 샘플링(시험지 유출·순환 차단)과 의역 변형 추가로 진행(백로그). known_fail 2건의 근본 해법(트랙 C 정렬 지원·사양 동등 조인 패턴)도 백로그 |
| 2026-07-21 | D-087 | **(후속4) CTE 이름 추출 결함 2종 수정 + SMQ 코어 치환 가드 + gp 잔여 정비**: 재실측 12/15. ① gp-014 SKIP 재발("존재하지 않는 테이블: server_specs") — `_extract_cte_names`가 `^\\s*WITH` 앵커라 **SQL이 주석으로 시작하면 CTE 추출이 통째로 빔** + 파생 테이블 별칭(`(SELECT...) ref`)은 애초에 미제외(1차 실측의 'ref'가 이것) → 주석 제거 후 판정 + 닫는 괄호 뒤 식별자(문법상 항상 별칭) 제외 추가, 회귀 3건. ② gp-009 회귀(7/6, 미일치 logicalcore): SMQ가 같은 질의에서 PHYSICALCORE **단독** 선택으로 흔들림 — 기존 가드(동시 선택 시 제거)의 사각지대 → `normalize_smq`에 단독 선택 시 LOGICALCORE **치환** 추가('물리' 명시 시 불변). ③ gp-015(3/2, 미일치 ipaddress): 부모 승격·경고 이상·기간창 전부 수렴 확인 — 잔여는 질의 미명시 IP를 담은 골드 스코프 위반 → SELECT에서 제거(GROUP BY의 IPADDRESS는 동명 서버 병합 방지로 유지). ④ gp-010 subset(성능 통계의 지표 구성은 본질적 모호 — 사용자 판단, 값은 전부 일치 실측). 검증: 545 passed(기존 6 HEAD 동일)·dry-run 0 |
| 2026-07-21 | D-087 | **(후속3) gp 11/15→마감 직전 — 월별 분해 결정적 게이트 + 잔여 4건 처리**: 재실측 gp 11 PASS. ① gp-010(4725/1820): 프롬프트 단서(후속2)에도 트랙 C가 "서버별 월간 통계"를 패턴 B로 재과포획 → `compile_from_nl`에 결정적 게이트 `_MONTHLY_BREAKDOWN_RE`(월별|(?<!개)월간 — "3개월간"의 기간 표현은 부정 후방탐색으로 제외) 신설, 패턴 B+매칭 시 폴백 강제(프롬프트 유도 반복 실패→코드 결정화, D-047 계열). ② gp-014(44/43, 1/6): CTE 허용 후 첫 실행 성공 — 행 차이는 자기 자신 포함 여부(pred 제외가 의미 정합) → 골드에 자기 제외 추가+subset. ③ gp-013(1/1000): "서버들의 평균"이 서버별 리스트로 해석되는 모호성 → 질의를 "전체 서버를 통틀은 … 단일 값"으로 명확화. ④ gp-007 subset(사용자 결정: 값 일치 스코프 과잉은 제낌). ⑤ gp-015(알람 집계가 metric 테이블 참조 반복): 알람 템플릿에 "성능 통계 테이블은 알람 스키마에 부재 — 절대 참조 금지, 건수는 C-4 COUNT" 명시 — 프롬프트 마지막 시도, 미수렴 시 known_fail 전환 예정. 검증: text2sql+CTE 109 passed·dry-run 0 |
| 2026-07-21 | D-087 | **(후속2) CTE(WITH) 검증 원천 거부 수정 + gp 첫 실측 정비**: gp 첫 실측 4/15(FAIL 8·SKIP 3). **실결함**: ① sqlparse `get_type()`이 `WITH ... SELECT`를 UNKNOWN 반환 → "SELECT 문만 허용" 검증이 정당한 읽기 전용 CTE를 원천 거부(gp-014 재시도 소진 SKIP — 유사사양 자기조인류는 LLM이 CTE 선호) → `_get_statement_type` UNKNOWN 시 주석 제거 후 WITH 시작+DML 부재면 SELECT 재분류, 멀티 `_validate_sql_simple`도 WITH 허용(data-modifying CTE는 키워드 검사로 계속 차단), 회귀 7건. ② 트랙 C가 "지난 3개월 서버별 **월간** 통계"(gp-010)를 서버당 1행으로 과포획(후속9 "월 단위 기간=커버리지 안" 과교정) → SMQ 프롬프트에 "월별 행 분해(월간/추이) 질의는 커버리지 밖" 단서. **골드 정비**(b0 규약 미러): gp-001/002/004/009/010 질의 '전체' 명시(1820~4725행>기본 LIMIT 절단), gp-005 subset, gp-007 골드 server_name 제거(pred가 질의 명시 2컬럼만 정확 반환 — 골드가 스코프 규약 위반), gp-012 severity→표준 뷰 severity_grade 텍스트+gold_smq dims=[], gp-013 멀티DB 교차 비교→단일 DB 스칼라로 재정의(교차 판정은 orchestration 평가 백로그 — 단일 SQL로 표현 불가해 산문 생성→SKIP이 구조적). gp-015는 CTE 수정 후 재실측 대기. 검증: 533 passed(기존 6 HEAD 동일)·dry-run 0 |
| 2026-07-21 | D-087 | **(후속) cmm_resource 조회 dtime IS NULL 부재 결정적 차단**: b0-005 재실측 rows 376/475(+99대 과다) — 사용자 전사 WHERE 절로 원인 확정: resource_type·상한 게이트·_m 전환은 정상인데 **dtime IS NULL이 전무** → 삭제된 서버 혼입(이 항목만이 아니라 LLM이 필수 필터를 누락할 수 있는 전역 결함). 결정적 가드 신설: `query_gen_common.missing_dtime_filter`(주석·리터럴 제거 후 cmm_resource 참조 + dtime IS NULL 전무 판정 — 별칭 단위 강제는 알람 뷰 부모 SVR 조인·계층 조인 무필터가 정당해 오탐하므로 "전체에 한 번도 없음"만 차단), query_validator 4.6 + 멀티 `_validate_sql_simple` 공유(D-066) → 검증 실패로 재생성 유도. 기존 validator 테스트 픽스처 4건이 규약 위반 SQL(dtime 없음)이라 가드에 걸림 → 픽스처를 규약 정합(SQL에 dtime 추가 + 스키마 픽스처에 dtime 컬럼) — 가드 약화 아님(D-048.8 전수 갱신 계열). 회귀 `test_query_validator_dtime.py` 8건. 검증: 517 passed(기존 실패 6건 HEAD 동일)·arch 0 |
| 2026-07-21 | D-076 | **(후속13) b0 4/5 도달 — b0-005 잔여는 통계 테이블 선택(_d vs _m)**: 후속12 반입 후 b0-001~004 PASS. b0-005 rows 376/276 — pred SQL 실측(사용자 전사)으로 원인 확정: "적이 있는" 예외는 준수(기간 필터 없음)했으나 **`cmm_metric_stat_d`(일간, 보관 짧음)를 선택**해 검색 범위가 축소(골드는 _m 월간). 후속12의 "전 보관 기간 검색" 문구가 "전 기간=세밀한 일간"으로 오독될 여지 → 프롬프트·프로필 3종에 "반드시 _m(보관 최장, 월간 max가 피크 보존) 사용, _d/_h 금지" + 임계 비교 상한 게이트(<=1000) 지시 명문화. 폴백 항목 프롬프트 보강은 이번이 2회차 — 미수렴 시 known_fail(폴백 비결정 감시) 전환 예정 |
| 2026-07-21 | D-076 | **(후속12) b0 첫 실측 5건 전멸 원인 3계열 수정**: ① b0-001/003/004 rows 2337/1000 — b0는 서버 2337대라 pred 기본 LIMIT(1000)이 절단(yd/gp는 1000행 미만이라 미발현) → 질의에 "전체" 명시(resolve_query_limit 상향 키워드, 트랙 C·폴백 공유). ② b0-002 COUNT 값 불일치 — **프로필 few-shot 3곳이 구 관행 `LIKE 'platform.server%'` 잔존**(골드만 server.Server로 정비하고 예시를 안 고친 비대칭, yd서 0행 실측된 패턴이라 yd/gp 폴백의 잠재 0행 버그이기도 함) → 프로필 3종 예시 스윕. ③ b0-005 rows 376/131 — 직전월 기본 관행(후속6)이 "초과한 적이 있는"(전 기간 존재) 질의에까지 적용돼 pred가 직전월 한정 → 프롬프트·프로필에 "~한 적이 있는" 예외(전 보관 기간 검색) 추가, scoring=subset. 교훈: 존별 데이터 규모 차이(2337 vs <1000)가 공유 경로의 절단 결함을 노출 — 기본 LIMIT에 걸리는 골드는 질의에 전량 의도를 명시. 검증: 139 passed·dry-run 0·YAML 파싱 OK |
| 2026-07-21 | D-076 | **(후속11) 의도된 FAIL 정식화(known_fail) — yd-005 플립플롭 감시 전환**: 후속10 직후 재실측에서 yd-005 pred가 747(INNER)로 회귀(직전 실행은 788 LEFT) — 랭킹 질의는 트랙 C 커버리지 밖(정렬 미지원) LLM 폴백이라 형태가 실행마다 흔들리는 비결정임이 확정, 골드를 어느 쪽에 맞춰도 무의미. 사용자 원칙("의도한 false-negative라면 굳이 PASS로 맞출 필요 없음")을 하네스에 정식화: 골드 항목 `known_fail: <사유>` 선언 시 `[FAIL:의도]` 표기+사유 출력(집계는 일반 FAIL과 동일 — 지표 왜곡 없음), **PASS 전환 시 "감시 결함 수정됨 — 해제 검토" 안내**(의도된 FAIL의 PASS 전환이 개선의 증거). 골드 수정 기준 명문화: 골드는 제품 정본 대비 골드 자체가 결함일 때만 수정, 파이프라인 결함을 잡는 FAIL은 known_fail로 유지(세션 교체 후 골드가 pred에 맞춰지는 사고 방지). yd-005는 정본(788) 골드 유지+known_fail 선언, 근본 해법(패턴 B ORDER BY 확장)은 백로그. yd 최종: 5/6 PASS + 1 의도적 FAIL |
| 2026-07-21 | D-076 | **(후속10) yd 6/6 도달 — yd-005 골드를 D-068 정본에 정렬**: 후속9 배포 후 yd-001~004·006 전 PASS, yd-005만 rows 747/788 — 후속9 리스트 정본화(전 서버 LEFT JOIN few-shot)의 **의도된 파급**으로 pred가 전 서버 788행(통계 없는 41대 NULL 포함)으로 이동했고, 구식이 된 건 골드(INNER JOIN 747행) 쪽. 골드를 서버당 1행 피벗(LEFT JOIN·직전월·게이트·NULLS LAST)으로 교체. 랭킹류가 아닌 전체 집계(gp-013 스칼라 AVG)·존재 조회(b0-005)는 INNER 의미가 정당해 불변. 교훈: 프롬프트/few-shot 정본을 바꾸면 그 형태를 전제한 **골드 항목 전수를 같은 정본으로 재정렬**해야 함(파이프라인만 바꾸고 골드를 두면 개선이 FAIL로 위장) |
| 2026-07-21 | D-076 | **(후속9) 명시 건수 결정적 LIMIT + SMQ 기간 오분류 교정 + 리스트 few-shot 정본화**: 후속8 실측 2건 — ① yd-006 rows 100/1000·cols 9/9: 표준 뷰 컴파일은 발동(플립플롭 종결)했으나 `resolve_query_limit`이 "모든/전체"만 인식해 "…100건 조회"가 기본 LIMIT(1000)로 나감 → 명시 건수("N건"/"상위 N") 결정적 파싱 추가(단독 "N개"는 "개월"·"코어 4개인" 혼동으로 불인정 — 기존 경계 테스트 유지, 명시 건수가 '전체' 상향보다 우선). ② yd-004 rows 788/747·cols 7/10: 트랙 C 미발동 폴백 — SMQ 프롬프트의 "기간 범위→커버리지 밖" 규칙이 "지난달 1개월 통계"를 none으로 오분류(후속8 알람 예외 추가가 기간 화제를 부각한 부작용 추정). 커버리지 안 섹션 신설: 월 단위 기간은 시스템(stat_month)이 결정적 주입하므로 패턴 B로 변환(기간을 filters에 넣지 말 것). +폴백 자체 품질: 747행=few-shot 3방향 INNER JOIN이 통계 없는 서버 41대를 침묵 탈락 → 프로필 3종 "용량+사용률 리스트" few-shot을 D-068 정본(서버당 1행·cmm_resource 기준 LEFT JOIN·직전월·LOGICALCORE, b0는 DOUBLE/DECIMAL(31,2))으로 교체, Template B에 "리스트=서버당 1행·LEFT JOIN / 추이 명시 시에만 stat_date 분해" 규칙 추가. 검증: text2sql+parity+utils 229 passed·dry-run 0·프로필 YAML 파싱 OK |
| 2026-07-21 | D-076 | **(후속8) 알람 표준 뷰 결정적 컴파일 + 컬럼 스코프 과잉 subset 일반화**: 후속7 골드 교정 직후 실측에서 yd-006 pred가 이번엔 **CR직접 형태**로 생성돼 SVR 기반 3컬럼 미일치 — LLM 폴백이 부모조인/직접 형태를 실행마다 오가는 플립플롭이라 골드를 어느 쪽에 맞춰도 절반은 실패. 교리대로 결정화: 시맨틱 모델 4종 pattern_c에 severity_grade(CASE 텍스트)·occurred_at(TO_CHAR)·ALARM_ID 차원 + `default_dimensions`(제품 표준 알람 뷰 9컬럼) 신설, `_compile_c`가 빈 dimensions면 표준 뷰를 결정적 조립(장비 식별=SVR 고정), order_by에 CA.ID tiebreak(LIMIT 경계 동률 비결정 차단). NL→SMQ 프롬프트에 "컬럼 미명시 알람은 dimensions=[]"·"최근 발생 순 N건은 커버리지 안" 유도 추가(결정적 기본값이 받치는 하이브리드). +yd-003(커버리지 밖 avail_status)·yd-004(SMQ 식별 dimension 추가 선택)도 실행 간 부가 컬럼 비결정(값 일치) 실측 → scoring=subset 일반화(yd-004 핵심 감시(서버명 누락)는 subset에서도 실패로 검출). gp 대칭: gp-006/gp-009 subset, gp-008 호스트명 sbhdbo63→sbhdbo53(사용자 실서버 목록 정본). 검증: text2sql 101·DB2 표준뷰 스모크·dry-run 0 |
| 2026-07-21 | D-076 | **(후속7) 알람 뷰 장비 식별 = 부모 서버 기준 정합(골드+패턴 C)**: yd-006 subset 미일치 진단(신설 `column_subset_unmatched`)이 정확히 [ipaddress, hostname]을 지목 — 제품 알람 템플릿(C-1/C-2)은 장비명/IP/호스트명을 **부모 서버**(`COALESCE(PLATFORM_RESOURCE_ID, SERVICE_RESOURCE_ID, ID)` 조인 SVR)에서 가져오는데, 골드(yd-006·gp-012)가 알람 발생 자원(CR) 직접 컬럼으로 작성돼 하위 자원(CPU/메모리) 알람 행에서 값 불일치(**pred가 제품 정본, 골드가 결함** — 진단 컬럼명 2개가 원인을 즉시 특정한 사례). 골드 2건 SVR 조인 교정(server_name=장비명(부모)/resource_name=알람 자원 구분 — 중복 컬럼도 해소) + 시맨틱 모델 4종 pattern_c 대칭 수정(joins에 CMM_RESOURCE_SVR 추가, server_name/IPADDRESS/HOSTNAME→SVR, resource_name(CR.NAME) 신설)으로 트랙 C 알람 컴파일도 제품 뷰 정합. yd-005는 직전월 수렴 실측(747/747·값일치) 후 scoring=subset 선언(사용자 결정 — 순위 질의 LLM 폴백의 부가 컬럼 비결정). 검증: 패턴 C 컴파일 스모크(SVR 조인 확인)·text2sql 99·dry-run 0 |
| 2026-07-21 | D-076 | **(후속6) 트랙 C SMQ 선택 품질 결정적 교정 + 직전월 관행 유인 정리**: 트랙 C 첫 실전 컴파일(yd-004, 788/788·7컬럼)에서 LLM SMQ 선택 결함 2종 실측 — ① 서버 식별 dimension 누락(속성 LOGICALCORE/TotalSize만 선택, 기존 주입 게이트는 dimensions 완전 공백만 보정) ② "CPU 용량"에 LOGICALCORE·PHYSICALCORE 동시 선택. 교정(프롬프트 아닌 결정적 가드): `_has_identity_dim`(direct name/hostname/ipaddress 부재 시 default_dimensions 주입, measures 있을 때만 — 패턴 A 불변)·`normalize_smq`(질의에 '물리' 신호 없으면 PHYSICALCORE 제거). `default_dimensions`는 4개 모델 모두 `[name]`으로 축소(hostname 주입은 질의 미명시 컬럼 혼입 — 컬럼 스코프 규약 위반, b0-004 골드 5컬럼 정합). +LLM 폴백 경로 유인 정리: 프롬프트·프로필 3종의 "항상 MAX(stat_date) 서브쿼리" 문구가 진행 중 미완성 월 집계를 유도(yd-005 pred 747행 실측) → 직전월 기본 관행 명시+MAX(stat_date) 금지, Template B·프로필 few-shot hi 서브쿼리 PHYSICALCORE→LOGICALCORE(사용자 확정: VM 위주=논리코어), b0 프로필의 PG `INTERVAL` 예시 3건→DB2 방언(D-053 계열). +EX 하네스: `column_subset_unmatched`(FAIL 시 미일치 골드컬럼 진단 출력), subset 매칭에 골드 중복 컬럼의 pred 재사용 허용(yd-006 알람 뷰 장비명=리소스명이 1:1 강제로 오탐). 회귀: text2sql 99+column_subset 5 통과, 기존 실패 6건은 HEAD 동일(무관 확인) |
| 2026-07-20 | D-076 | **(후속5) 트랙 C 런타임 토글 불능 수정 + A/B 하네스 정합**: 폐쇄망 A/B(semantic_compose) 실측 +0.0%p·시맨틱 로그 전무의 원인 2중 — ① `AppConfig.text2sql/synonym`이 클래스 기본값 인스턴스라 **임포트 시점 env로 고정**, 하네스의 env 토글+cache_clear가 무효(운영 기동 로드는 무영향) → `Field(default_factory=...)`로 교정(양방향 토글 실측 확인). ② A/B baseline이 축 플래그를 명시 off 하지 않아 프로세스/.env 상태 상속 → `base_flags={axis: False}` 명시. +하네스 `--log-level INFO`(시맨틱 발동 로그 가시화)·파이프라인 검증 실패 시 실패 SQL 미실행(사유 노출). 회귀: utils/orchestration 247·관련 151 통과, arch 0. A/B 재실행 필요 |
| 2026-07-20 | D-087 | **생성 SQL 한글 토큰 잔존 결정적 차단**: EX 실측(yd-006)에서 생성 SQL 구조 영역에 자연어 조각("해당"→재실행 시 "현재", 토큰 가변)이 남아 구문 오류 — 프롬프트로 못 막는 비결정 오류라 validator 결정적 가드로 차단. `_find_bare_hangul_tokens`(주석·리터럴·따옴표 식별자 제거 후 한글 검출, 따옴표 안 별칭/리터럴은 허용) → 검증 실패로 재생성 루프 유도. 단일(query_validator)·멀티(`_validate_sql_simple`) 경로 공유(D-066). 회귀 6건 + validator 73건 통과. 상세 `## D-087` |
