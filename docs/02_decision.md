...
- **D-번호 채번**: 이 파일의 **①`## D-` 헤더 ②하단 「변경 이력」 표 ③아래 「채번 이력」 표**를 모두 grep하여 실제 최댓값+1 부여 (현재 최대 **D-194** → 다음 **D-195**. **D-158·D-163~D-168은 미등재 예약** — 재사용 금지. **D-176은 `plans/82`(존 동시 조회 — 실행 그룹 순차 실행·존별 산출물·솔루션 축 라우팅, D-143 후속3 개정) 예약** — 미등재·재사용 금지. **D-177~D-179는 2026-08-28 본문 등재 완료**(`plans/83`). **D-169~D-174는 2026-08-27, D-175·D-180~D-182는 2026-08-28, D-183은 2026-08-29 본문 등재 완료**. **D-184~D-188은 2026-08-31 원격 `multiintent` 병합 시 재부여** — 원격 브랜치가 같은 D-175~D-179를 다른 결정에 부여해 정면 충돌했고, 본 브랜치가 D-180~D-183까지 점유하고 있어 들어오는 쪽을 D-184~D-188로 옮겼다(참조 162라인/37파일 일괄 치환)). **계획서에만 적힌 예약은 효력이 없다** — grep 대상은 이 세 곳뿐이므로 예약은 ③에 행으로 등재할 것(D-161 참조).
...
| 번호 | 구분 | 내용 |
|---|---|---|
| D-194 | **채번 완료**(2026-09-02 본문 등재) | 장애진단 잔여 구현(`plans/50` v2.2) — 브리핑 계약 정본·공용 렌더러 · 사건 좌표계 도구 인자 · 결정적 사전수집+상관(`sre_agent`) · 가설 rank/confidence. 미등재·재사용 금지 |
...
## D-194. 장애진단 잔여 구현 — 브리핑 계약 정본 · 사건 좌표계 · 결정적 상관(`plans/50` v2.2)

- **결정일**: 2026-09-02 | **상태**: 확정 (구현 완료 · 실 조사 검증은 D-127 건별 승인 대기) | **번호**: `## D-` 헤더 최댓값 **D-193** · 「변경 이력」 표 최댓값 **D-193** · 「채번 이력」 표 예약(D-105·115·134·158·163~168·176) 대조 → **D-194**(등재 직전 재실측).
