...
| 번호 | 구분 | 내용 |
|---|---|---|
| D-198 | **채번 완료**(2026-09-07 본문 등재) | "이번 달" 성능 통계 = stat_d 당월 1일~어제 집계(사용자 제안 채택). 매월 1일은 빈 결과 안내 폴백, 진행월 집계 기준 결정적 각주 동반 |
| D-197 | **채번 완료**(2026-09-07 본문 등재) | '가동률' 의미 확정 — 가용성(uptime) 계열, CPU 사용률 매핑 금지(사용자 확정). pre-gate 구현·골드셋 yd-005 갱신 완료 |
| D-196 | **채번 완료**(2026-09-07 본문 등재) | EAV 단위 문자열("14.9 GB") 숫자 캐스트의 GB 기준 환산 정규화 — B-11 실측, 세 존 TB/GB/MB 혼재 |
| D-195 | **채번 완료**(2026-09-07 본문 등재) | SSE 무한대기 이중 가드 — FabriX 호출 총 소요 상한(F1) + 스트림 가드 타임아웃 검출·취소 분리(F2) |
...
## D-195. SSE 무한대기 이중 가드 — FabriX 호출 총 소요 상한 + 스트림 가드의 검출·취소 분리

- **결정일**: 2026-09-07 | **상태**: 확정 (구현 완료 · 폐쇄망 검증 대기)
...
## D-196. EAV 단위 문자열 숫자 캐스트의 GB 기준 환산 정규화
...
## D-197. '가동률' 의미 확정 — 가용성(uptime) 계열, CPU 사용률 매핑 금지
...
## D-198. "이번 달" 성능 통계 = cmm_metric_stat_d 당월 1일~어제 집계
...
## D-199. 알람 결정적 조립 커버리지 밖 신호 가드 — 집계 축·유형/메트릭 필터는 조립 포기

- **결정일**: 2026-09-08 | **상태**: 확정 (구현 완료 · 폐쇄망 검증 대기) | **번호**: 등재 최댓값 D-198 → **D-199**(등재 직전 `## D-` 헤더·「변경 이력」·「채번 이력」 표 재확인)
